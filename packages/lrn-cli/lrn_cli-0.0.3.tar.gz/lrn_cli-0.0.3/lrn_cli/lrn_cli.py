...
@click.option('--domain', '-d',
              help='Domain to use for web API requests',
              default='localhost')
# Environment
...
def data(ctx, endpoint_url, references=None, limit=None,
    ...
    consumer_key, consumer_secret, version, region, environment = _get_profile(
        ctx, DEFAULT_API_DATA_VERSION)

    action = _get_action(ctx)
    data_request = _get_request(ctx)
    endpoint_url = _build_endpoint_url(
        endpoint_url, DEFAULT_API_DATA_URL, version, region, environment)

    if len(references) > 0:
        if 'references' in data_request:
            logger.warning(
                'Overriding `references` in request with `--references` from the command line')
        data_request['references'] = references
    if limit:
        if 'limit' in data_request:
            logger.warning(
                'Overriding `limit` in request with `--limit` from the command line')
    ...
    if profile_name:
        ...
        if config_file:
            ...
            if profile_name not in config:
                logger.debug(
                    f'Profile {profile_name} not found in config {config_file.name}')
            else:
...
def _build_endpoint_url(endpoint_url, default_url, version,
    ...
    if not endpoint_url.startswith('http'):
        ...
        endpoint_url = default_url.format(
            region=region, environment=environment) + endpoint_url
    return endpoint_url
...
def _get_api_response(ctx, api, endpoint_url, default_url, default_version, user_id=None):
    ...
    endpoint_url = _build_endpoint_url(
        endpoint_url, default_url, version, region, environment)
    action = _get_action(ctx)
...
def _send_www_encoded_request(api, endpoint_url, consumer_key, consumer_secret, logger,
    ...
    security = _make_security_packet(
        consumer_key, consumer_secret, user_id, domain)
...
def _send_json_request(endpoint_url, consumer_key, consumer_secret,
    ...
    if not do_recurse:
        r = data_api.request(endpoint_url, security,
                             consumer_secret, request, action)
    else:
        meta = {
...
def _output_json(data, stream=None):
    ...
    if colorise:
        outJson = highlight(outJson, lexers.JsonLexer(),
