...
_si_prefix_symbols = "Y;Z;E;P;T;G;M;k;h;da;d;c;m;µ;n;p;f;a;z;y"
...
BaseUnit = namedtuple("BaseUnit", "symbol name quantity")
"""
...
DEFAULT_ATTRIBUTE_UNIT_SEPARATOR = AttributeUnitSeparators.WHITESPACE_BOXED
"""
...
def _read_raw_unit_table(text_buffer) -> List["AUnit"]:
    ...
    for raw_table_unit_definition in csv_reader:
        ...
        current_base_unit = BaseUnit(symbol=symbol, name=name, quantity=quantity)
...
class AUnit(object):
    ...
    def __init__(
        ...
        Examples:
            ...
            >>> raw_apple = BaseUnit("apple", "apple", "fruit")
    ...
    def to_raw_parts(self) -> List:
        ...
        Examples:
            ...
            Prefix(symbol='m', name='milli', decimal=0.001), 1.0]


            >>> doctest_print(SiUnits.meter.to_raw_parts(), max_line_width=50)
            [BaseUnit(symbol='m', name='meter', quantity='length'),
            Prefix(symbol='', name='', decimal=1), 1.0]
...
def create_unit(symbol: str, name: str, quantity: str, prefix: Optional[Prefix] = None):
    """
    ...
    """
    return AUnit(BaseUnit(symbol=symbol, name=name, quantity=quantity), prefix=prefix)


_base_si_unit_map = {
    "s": AUnit(BaseUnit("s", "second", "time")),
    "m": AUnit(BaseUnit("m", "meter", "length")),
    "kg": AUnit(BaseUnit("g", "gram", "mass"), prefix=SiPrefixes.kilo),
    "A": AUnit(BaseUnit("A", "ampere", "electric current")),
    "K": AUnit(BaseUnit("K", "kelvin", "thermodynamic temperature")),
    "mol": AUnit(BaseUnit("mol", "mole", "amount of substance")),
    "cd": AUnit(BaseUnit("cd", "candela", "luminous intensity")),
}

_si_unit_map = {
    "rad": AUnit(BaseUnit("rad", "radian", "plane angle")),
    "sr": AUnit(BaseUnit("sr", "steradian", "solid angle")),
    "Hz": AUnit(BaseUnit("Hz", "hertz", "frequency")),
    "N": AUnit(BaseUnit("N", "newton", "force, weight")),
    "Pa": AUnit(BaseUnit("Pa", "pascal", "pressure, stress")),
    "J": AUnit(BaseUnit("J", "joule", "energy, work, heat")),
    "W": AUnit(BaseUnit("W", "watt", "power, radiant flux")),
    "C": AUnit(BaseUnit("C", "coulomb ", "electric charge")),
    "V": AUnit(
        BaseUnit("V", "volt", "electrical potential difference (voltage)), emf")
    ),
    "F": AUnit(BaseUnit("F", "farad", "capacitance ")),
    "Ω": AUnit(BaseUnit("Ω", "ohm", "resistance, impedance, reactance")),
    "S": AUnit(BaseUnit("S", "siemens ", "electrical conductance")),
    "Wb": AUnit(BaseUnit("Wb", "weber", "magnetic flux ")),
    "T": AUnit(BaseUnit("T", "tesla", "magnetic flux density ")),
    "H": AUnit(BaseUnit("H", "henry", "inductance")),
    "°C": AUnit(BaseUnit("°C", "degree Celsius", "temperature relative to 273.15 K")),
    "lm": AUnit(BaseUnit("lm", "lumen", "luminous flux ")),
    "lx": AUnit(BaseUnit("lx", "lux", "illuminance ")),
    "Bq": AUnit(BaseUnit("Bq", "becquerel", "radioactivity (decays per unit time) ")),
    "Gy": AUnit(BaseUnit("Gy", "gray", "absorbed dose (of ionising radiation) ")),
    "Sv": AUnit(BaseUnit("Sv", "sievert", "equivalent dose (of ionising radiation)")),
    "kat": AUnit(BaseUnit("kat", "katal", "catalytic activity")),
}
...
_additional_units = {
    "g": AUnit(BaseUnit("g", "gram", "mass")),
    "t": AUnit(BaseUnit("t", "tonne", "mass")),
}
...
def prefix_base_unit(
    base_unit: BaseUnit, supported_prefixes: Iterable[str]
) -> List[AUnit]:
    """
    ...
    Examples:
        ...
        >>> from doctestprinter import doctest_iter_print
        >>> base_unit = BaseUnit("m", "meter", "length")
...
class _UnitLibrary:
    ...
    Examples:
        ...
        >>> mate = AUnit(BaseUnit("mat", "mate", "a tea"))

        A made up unit with same but uppercase leading letter and equal length
        to 'meter'.

        >>> big_m = AUnit(BaseUnit("M", "Big M", "Makes m big"))
...
def from_string(unit_text: str) -> "AnyUnit":
    """
...
class StyledDetection(object):
    def __init__(self, target_style: UnitStyle):
        """
        Sets the target format style of detected units.

        Args:
            target_style:
                Targeted unit style any detected unit receives.
        """
        self._target_style = target_style

    def from_string(self, unit_text: str) -> "AnyUnit":
        """
        Detects units from unit texts, returning AnyUnit.
        ...
        Examples:
            ...
            >>> import justunits
            >>> from justunits import StyledDetection, UnitStyle
            >>> my_target_style = StyledDetection(UnitStyle.FINE_POWERED_SUPERSCRIPT)
            >>> from doctestprinter import doctest_print
            >>> heat_capacity = my_target_style.from_string("kg/(s^2*K)*m^2")
            >>> doctest_print(repr(heat_capacity), max_line_width=60)
            (AUnit(kg 'kilogram' mass, 1e+03g), AUnit(s 'second' time; power=-2),
            AUnit(K 'kelvin' thermodynamic temperature; power=-1), AUnit(m
            'meter' length; power=2))

            >>> justunits.to_string(heat_capacity)
            'kg/(s²⋅K)⋅m²'

            >>> str(heat_capacity)
            'kg/(s²⋅K)⋅m²'
        """
        detected_units = from_string(unit_text=unit_text)
        detected_units.style = self._target_style
        return detected_units


class AnyUnit(Sequence):
    def __init__(self, *sub_units):
        ...
        Examples:
            ...
            **AnyUnit** are comparable and hashable. The order of units is neglected
            ...
            :class:`AnyUnit` is hashable like :class:`AUnit` to support usage as key
    ...
    def make_hash_values(units: Iterable[AUnit]) -> List[Union[str, int, float]]:
        ...
        .. doctest::
            ...
            >>> from justunits import AnyUnit, SiUnits
            >>> AnyUnit.make_hash_values((SiUnits.newton, SiUnits.meter))
            ('N', 1.0, 'm', 1.0)
        """
    ...
    def __eq__(self, other: "AnyUnit"):
        assert isinstance(other, AnyUnit)
    ...
    def with_prefix(self, prefix: SiPrefixes) -> "AnyUnit":
...
_splits_number_with_unit = re.compile(
    r"(?P<number>[0-9\+-,.eE]+)[\s]*(?P<unit>[0-9\w/\^]*)$"
)
...
def join_unit(
    ...
    unit: Union[str, AnyUnit, AUnit, UnknownUnit],
    ...
    if isinstance(unit, (AnyUnit, AUnit, UnknownUnit)):
