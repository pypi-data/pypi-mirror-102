...
def access_raw_label(frame):
    ...
    dataframe = pd.read_csv(config.path_color2class_table)
    list_rgb = []
    list_class_id = []
    ...
    # 创建空数组用于存放一通道的label
    label = np.empty([config.crop_size[0], config.crop_size[1]], dtype=int)
...
if __name__ == "__main__":
    train = LoadDataset([config.train_image, config.train_label], config.crop_size)
    val = LoadDataset([config.val_image, config.val_label], config.crop_size)
    if config.test_image is not None:
        test = LoadDataset([config.test_image, config.test_label], config.crop_size)
