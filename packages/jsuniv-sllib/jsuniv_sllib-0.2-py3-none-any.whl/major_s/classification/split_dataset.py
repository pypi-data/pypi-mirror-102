...
def run_for_whale():
    # 1.确定原图像数据集路径
    dataset_dir = os.path.join(config.dataset_image)

    # 2.确定数据集划分后保存的路径
    split_dir = os.path.join(config.split_dataset)
    train_dir = os.path.join(split_dir, "train")
    valid_dir = os.path.join(split_dir, "valid")
    test_dir = os.path.join(split_dir, "test")
    # 3.确定将数据集划分为训练集，验证集，测试集的比例
    train_pct = config.account_for_dataset[0]
    valid_pct = config.account_for_dataset[1]
    test_pct =  config.account_for_dataset[2]
    # 4.划分
...
def run(dataset_image,split_dataset,account_for_dataset):
    # 1.确定原图像数据集路径
    dataset_dir = os.path.join(dataset_image)

    # 2.确定数据集划分后保存的路径
    split_dir = os.path.join(split_dataset)
    train_dir = os.path.join(split_dir, "train")
    valid_dir = os.path.join(split_dir, "valid")
    test_dir = os.path.join(split_dir, "test")
    # 3.确定将数据集划分为训练集，验证集，测试集的比例
    train_pct = account_for_dataset[0]
    valid_pct = account_for_dataset[1]
    test_pct =  account_for_dataset[2]
    # 4.划分
    for root, dirs, files in os.walk(dataset_dir):
        for sub_dir in dirs: # 遍历0，1，2，3，4，5...9文件夹
            imgs = os.listdir(os.path.join(root, sub_dir)) # 展示目标文件夹下所有的文件名
