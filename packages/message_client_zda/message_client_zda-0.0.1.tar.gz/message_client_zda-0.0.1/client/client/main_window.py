...
from client_dist.client.client.delete_contact import DelContactDialog
from client_dist.client.client.main_window_conv import UiMainClientWindow
from server_dist.server.common.utils import ServerError
from server_dist.server.common.variables import MESSAGE_TEXT, SENDER
...
class ClientMainWindow(QMainWindow):
    ...
    def history_list_update(self):
        ...
        """
        my_list = sorted(self.database.get_history(self.current_chat),
    ...
    def set_active_user(self):
        ...
        try:
            ...
            self.current_chat_key = None
            self.encryptor = None
            CLIENT_LOG.debug(f'Не удалось получить ключ '
                             f'для {self.current_chat}')
        ...
        try:
            ...
            if err.errno:
                self.messages.critical(self, 'Ошибка',
                                       'Потеряно соединение с сервером!')
                self.close()
            self.messages.critical(self, 'Ошибка', f'Таймаут соединения! ')
