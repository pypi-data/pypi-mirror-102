...
class Component(metaclass=SimplifiedInterfaceMediaDefiningClass):
    ...
    def compile_instance_template(self, template_name):
        ...
        self._print_nodelist(component_template.template.nodelist, prefix="Before")
        ...
        self._print_nodelist(instance_template.nodelist, prefix="After")
    ...
    @staticmethod
    def _print_nodelist(nodelist, depth=0, prefix=""):
        if depth == 0:
            print("-" * ((80 - len(prefix) - 2) // 2), prefix, "-" * ((80 - len(prefix) - 2 + 1) // 2))

        for node in nodelist:
            print("\t" * (depth) + str(node))
            if hasattr(node, "nodelist") and node.nodelist:
                Component._print_nodelist(node.nodelist, depth=depth + 1)
