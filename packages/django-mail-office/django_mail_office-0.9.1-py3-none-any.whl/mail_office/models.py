...
class EmailMessageManager(models.Manager):
    ...
    def reset_tasks_to_initial_status(self) -> None:
        self.get_queryset().select_for_update().filter(
            status=EmailMessage.Statuses.in_sending
        ).update(status=EmailMessage.Statuses.new)

    @atomic
    def select_for_process(self, count: int) -> models.QuerySet:
        base_qs = (
            self.get_queryset()
                .select_for_update()
                .filter(
        ...
        new_tasks_list = list(base_qs.all()[:count].values_list('pk', flat=True))
        selected_tasks = (
    ...
    #     sending_messages = self.get_queryset().filter(
    #         status=EmailMessage.Statuses.new,
    #         send_time__lte=now()
    #     )
