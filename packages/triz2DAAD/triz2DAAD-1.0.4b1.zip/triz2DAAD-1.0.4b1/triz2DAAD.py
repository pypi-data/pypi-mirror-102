...
def imprimeDEF():
        ...
        if daadReady:
                print(';', file=f)
                print('; Definiciones específicas para DAAD Ready 0.3', file=f)
                print(';', file=f)
                if english:
...
def imprimePRO0_DR():
        ...
        print('', file=f)
        if english:
                print('; 16 bit mode. It uses PICTURE as a condition to select text window (WINDOW 1) ubication.', file=f)
        else:
                print('; Modo 16 bits. Utiliza PICTURE como condición para decidir donde colocar la ventana de texto (WINDOW 1).', file=f)
        print('', file=f)
        print('#ifdef "nativeraster"', file=f)
        print('>', file=f)
        ...
        print('', file=f)
        if english:
                print('; ZX Next mode. Just like "no retryImage" mode with an extra XNEXTCLS condact', file=f)
        else:
                print('; Modo ZX Next. Igual al modo "noretryImage" con un condacto XNEXTCLS extra.', file=f)
        print('', file=f)
        print('#ifdef "next"', file=f)
        print('>', file=f)
        print('_       _       CLEAR 20', file=f)
        ...
        print('', file=f)
        if english:
                print('; Rest of target platforms (including CPC, C64 and UNO if NOSPLIT mode is selected). If not dark, loads and displays the maluva graphic file. If it\'s dark OR there\'s no file, text window is located at 0,0. If neither the case, it goes to 14,0. ', file=f)
        else:
                print('; Resto de plataformas (incluyendo CPC, C64 y Uno si se elige modo NOSPLIT). Si no está oscuro, carga el fichero gráfico de maluva y lo muestra. Si está oscuro O no hay gráfico la ventana de texto se coloca en 0,0. Si no es ninguno de los dos casos, va a 14,0.', file=f)
        print('', file=f)
...
print()
print('Triz2DAAD versión 1.0.4b1 210420 (c) 2019-21 Pedro Fernández')     
...
parser.add_argument('-dr', '--daadready', help="Crea plantillas compatibles con DAADReady 0.3", action="store_true")
...
if daadReady:
        if english:
                print('-Exporting to a DAADReady 0.3 compatible template.')
        else:
                print('-Expoartando a una plantilla comnpatible con DAADReady 0.3')
        dsf=True
        if statusLine:
                if english:
