...
def evaluate_nll(input_file:str, poi_val:float, poi:str="", vmin:float=10., vmax:float=10., unconditional=False,
                 workspace:str='combWS', model_config:str='ModelConfig', data:str='combData', snapshot:str=None, 
                 ...
                 max_calls:int=-1, max_iters:int=-1, optimize:bool=True, offset:bool=False, outname:str=None, cache:bool=False):
    if cache:
        ...
        if cached_result is not None:
            ...
            return nll
    start = time.time()
    model = ExtendedModel(fname=input_file, data_name=data, binned_likelihood=binned, snapshot_name=snapshot, 
...
def scan_nll(input_file:str, poi_min:float, poi_max:float, poi_step:float, poi:str="", cache:bool=True,
             ...
             minimizer_type:str='Minuit2', minimizer_algo:str='Migrad', num_cpu:int=1, binned:bool=True,
             eps:float=1.0, strategy:int=0, fix_cache:bool=True, fix_multi:bool=True, mpsplit:int=3, 
             max_calls:int=-1, max_iters:int=-1, optimize:bool=True, offset:bool=False, parallel:int=8):
    ...
    if not poi:
        poi = ExtendedModel.get_parameters_of_interest(input_file, workspace, model_config)[0]
    ...
    if parallel:
        arguments = (repeat(input_file), points, repeat(poi), repeat(vmin), repeat(vmax),
                     ...
                     repeat(fix_multi), repeat(mpsplit), repeat(max_calls), repeat(max_iters), 
        ...
        results = []
        for point, uncond, outname_ in zip(points, uncondistional, outnames):
            nll = evaluate_nll(input_file, point, poi, vmin, vmax, uncond, workspace, model_config, data,
                               ...
                               num_cpu, binned, eps, strategy, fix_cache, fix_multi, mpsplit, max_calls,
