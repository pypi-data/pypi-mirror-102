...
class WsiUtils:
    ...
    @staticmethod
    def make_signed(i: int):
        return int.from_bytes(i.to_bytes(1, 'little'), 'little', signed = True)
        ...
        for i in range(len(unpack_result)):
            if unpack_result[i] not in [0, 255]:
                for j in range(i, len(unpack_result)):
                    unpack_result[j] = WsiUtils.make_signed(unpack_result[j])
                return unpack_result[i:]

    @staticmethod
    def int_to_byte(i: int):
        WsiUtils.check_type(i, int)
        return 0 if i == 0 else 255 if i == -1 else WsiUtils.make_signed(i)
