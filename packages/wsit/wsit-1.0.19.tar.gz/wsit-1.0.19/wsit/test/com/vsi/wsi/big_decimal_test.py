...
class TestBigDecimal(unittest.TestCase):
    ...
    def test_init(self):
        for int_value in TestBigDecimal.valid_values_zero_scale:
            ...
            self.assertTrue(big_decimal.get_scale() == 0)
            self.assertTrue(str(big_decimal).__eq__(str(int_value)))
    ...
    def test_init_with_scale(self):
        for tested_set in TestBigDecimal.valid_values_with_scale:
            ...
            self.assertTrue(str(big_decimal).__eq__(tested_set[2]))

    def test_init_from_str(self):
        for str_value in TestBigDecimal.valid_str_values:
            big_decimal = BigDecimal.init_from_str(str_value)
            self.assertTrue(str(big_decimal).__eq__(str_value))
