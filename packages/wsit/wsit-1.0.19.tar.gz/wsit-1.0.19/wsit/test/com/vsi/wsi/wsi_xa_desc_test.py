...
class TestWsiXADesc(unittest.TestCase):
    valid_values = {
    ...
    def test_err_to_string(self):
        for key in TestWsiXADesc.valid_values.keys():
            self.assertTrue(WsiXADesc.err_to_string(key).__eq__(TestWsiXADesc.valid_values.get(key))) 

    def test_err_to_string_e(self):
        err = TypeError("Type conversion error")
        for key in TestWsiXADesc.valid_values.keys():
            tested_str = TestWsiXADesc.valid_values.get(key) + ": " + err.args[0]
            self.assertTrue(WsiXADesc.err_to_string_e(key, err).__eq__(tested_str)) 
