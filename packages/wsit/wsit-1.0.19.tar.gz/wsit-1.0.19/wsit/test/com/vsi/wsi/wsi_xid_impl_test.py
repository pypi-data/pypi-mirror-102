...
class TestWsiXidImpl(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertTrue(wsi_xid.get_format_id() == tested_format_id) 
        for i in range(len(tested_gtrid)):
            self.assertTrue(tested_gtrid[i] == m_gtrid[i]) 
        for i in range(len(tested_bqual)):
            self.assertTrue(tested_bqual[i] == m_bqual[i]) 
