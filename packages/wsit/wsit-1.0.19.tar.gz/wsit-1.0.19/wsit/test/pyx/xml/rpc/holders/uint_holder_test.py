...
class TestUIntHolder(unittest.TestCase):
    ...
    def test_to_string(self):
        for tested_value in TestUIntHolder.valid_values:
            uint_holder = UIntHolder(tested_value)
            self.assertTrue(uint_holder.__str__().__eq__(str(tested_value))) 

    def test_equals(self):
        uint_holder_1 = UIntHolder()
        uint_holder_2 = UIntHolder()
        self.assertTrue(uint_holder_1.__eq__(None) is False) 
        self.assertTrue(uint_holder_1.__eq__(int("123")) is False) 
        self.assertTrue(uint_holder_1.__eq__(uint_holder_1) is True) 
        self.assertTrue(uint_holder_1.__eq__(uint_holder_2) is True) 
        uint_holder_2.set(123456)
        self.assertTrue(uint_holder_1.__eq__(uint_holder_2) is False) 
