...
class TestWsiException(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertTrue(wsi_exception.get_status() == 0) 
        self.assertTrue(wsi_exception.get_facility() == 0) 
        self.assertTrue(wsi_exception.get_vms_status() == 0) 
        self.assertTrue(wsi_exception.get_severity() == 0) 

        self.assertTrue(WsiException.WSIsK_WARNING == 0) 
        self.assertTrue(WsiException.WSIsK_SUCCESS == 1) 
        self.assertTrue(WsiException.WSIsK_ERROR == 2) 
        self.assertTrue(WsiException.WSIsK_INFO == 3) 
        self.assertTrue(WsiException.WSIsK_SEVERE == 4) 
        self.assertTrue(WsiException.WSIsK_FAC_IPC == 1) 
        self.assertTrue(WsiException.WSIsK_FAC_RTL == 2) 
        self.assertTrue(WsiException.WSIsK_FAC_MGR == 3) 
        self.assertTrue(WsiException.WSIsK_FAC_SVR == 4) 
    ...
    def test_init_from_er_msg(self):
        for tested_value in TestWsiException.str_valid_values:
            ...
            self.assertTrue(wsi_exception.get_message() == tested_value) 
            self.assertTrue(wsi_exception.get_status() == 0) 
            self.assertTrue(wsi_exception.get_facility() == 0) 
            self.assertTrue(wsi_exception.get_vms_status() == 0) 
            self.assertTrue(wsi_exception.get_severity() == 0) 
    ...
    def test_init_from_er_msg_except(self):
        for except_values in TestWsiException.exception_valid_values:
            for tested_value in TestWsiException.str_valid_values:
                ...
                self.assertTrue(except_values.__eq__(wsi_exception.get_detail())) 
                self.assertTrue(tested_value.__eq__(wsi_exception.get_message())) 
    ...
    def test_init_from_er_msg_ex_code_vms_code(self):
        for tested_value in TestWsiException.str_valid_values:
            for int_value in TestWsiException.int_valid_values:
                ...
                self.assertTrue(tested_value.__eq__(wsi_exception.get_message())) 
                self.assertTrue(int_value == wsi_exception.get_status()) 
                self.assertTrue(int_value == wsi_exception.get_vms_status()) 
                ...
                for facility_value in TestWsiException.facility_values:
                    for severity_value in TestWsiException.severity_values:
                        wsi_exception = WsiException.init_from_er_msg_ex_code_vms_code_severity_facility(str_value,
                                                                                                         ...
                                                                                                         severity_value,
                                                                                                         facility_value)
                        self.assertTrue(str_value.__eq__(wsi_exception.get_message())) 
                        self.assertTrue(int_value == wsi_exception.get_status()) 
                        self.assertTrue(int_value == wsi_exception.get_vms_status()) 
                        self.assertTrue(severity_value == wsi_exception.get_severity()) 
                        self.assertTrue(facility_value == wsi_exception.get_facility()) 
    ...
    def test_init_from_wsi_ex(self):
        for str_value in TestWsiException.str_valid_values:
            for int_value in TestWsiException.int_valid_values:
                ...
                self.assertTrue(str_value.__eq__(wsi_exception_other.get_message())) 
                self.assertTrue(int_value == wsi_exception_other.get_status()) 
                self.assertTrue(int_value == wsi_exception_other.get_vms_status()) 
                self.assertTrue(int_value == wsi_exception_other.get_severity()) 
                self.assertTrue(int_value == wsi_exception_other.get_facility()) 
    ...
    def test_is_connection_valid(self):
        ...
        wsi_exception = WsiException.init_from_er_msg_ex_code_vms_code_severity_facility(er_message, in_exc_code_test,
                                                                                         ...
                                                                                         in_severity_test,
                                                                                         in_facility_test)
        self.assertTrue(wsi_exception.is_connection_valid() is False) 
        ...
        wsi_exception = WsiException.init_from_er_msg_ex_code_vms_code_severity_facility(er_message,
                                                                                         ...
                                                                                         in_severity_test,
                                                                                         in_facility_test)
        self.assertTrue(wsi_exception.is_connection_valid() is True) 
