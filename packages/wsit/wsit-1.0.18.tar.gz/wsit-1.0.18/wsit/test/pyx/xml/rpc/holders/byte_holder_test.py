...
class TestByteHolder(unittest.TestCase):
    ...
    def test_to_string(self):
        for tested_value in TestByteHolder.valid_values:
            byte_holder = ByteHolder(tested_value)
            self.assertTrue(byte_holder.__str__().__eq__(str(tested_value))) 

    def test_equals(self):
        byte_holder_1 = ByteHolder()
        byte_holder_2 = ByteHolder()
        self.assertTrue(byte_holder_1.__eq__(None) is False) 
        self.assertTrue(byte_holder_1.__eq__(int("123")) is False) 
        self.assertTrue(byte_holder_1.__eq__(byte_holder_1) is True) 
        self.assertTrue(byte_holder_1.__eq__(byte_holder_2) is True) 
        byte_holder_2.set(123)
        self.assertTrue(byte_holder_1.__eq__(byte_holder_2) is False) 
