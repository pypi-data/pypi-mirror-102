...
class Connection:
    ...
    def __init__(self, dsn=None, user=None, password=None, host=None, database=None, tls=None, force=None):
        ...
        self._parse_args(dsn, user, password, host, database, tls)
        ...
        while True:
            ...
            if self.use_cbc:
                ...
                req.client_connection.userid = self.user
                req.client_connection.database = self.database
                req.client_connection.clientid = "pyocient"
                req.client_connection.version = "7.0.1"
                req.client_connection.majorClientVersion = version_major
                req.client_connection.minorClientVersion = version_minor
            else:
                req.type = req.CLIENT_CONNECTION_GCM
                req.client_connection_gcm.userid = self.user
                req.client_connection_gcm.database = self.database
                req.client_connection_gcm.clientid = "pyocient"
                req.client_connection_gcm.version = "7.0.1"
                req.client_connection_gcm.majorClientVersion = version_major
                req.client_connection_gcm.minorClientVersion = version_minor
            ...
            peer_key = load_pem_public_key(rsp.pubKey.encode(encoding='UTF-8', errors='strict'))
    ...
    def _client_handshake(self, initialization_vector, peer_key):
        ...
        # encrypted password
        hasher = hmac.HMAC(mac_key, hashes.SHA256())
        hasher.update(cipher)
        generated_hmac = hasher.finalize()

        return (cipher, generated_hmac, private_key.public_key())

    def _parse_args(self, dsn, user, password, host, database, tls): #pylint: disable=too-many-arguments
        """Internal routine to resolve function arguments and dsn"""
        ...
        if dsn is not None:
            ...
            if 'tls' in parsed.query:
                tlsval = parsed.query['tls']
                if tlsval.lower() == 'off':
                    self.tls = self.TLS_NONE
                elif tlsval.lower() == 'unverified':
                    self.tls = self.TLS_UNVERIFIED
                elif tlsval.lower() == 'on':
                    self.tls = self.TLS_ON
                else:
                    raise MalformedURL(f"Invalid tls value: {tlsval}")

            if 'force' in parsed.query:
                if parsed.query['force'].lower() == 'true':
                    self.force = True
            if 'handshake' in parsed.query:
                if parsed.query['handshake'].lower() == 'cbc':
                    self.use_cbc = True
                else:
                    self.use_cbc = False

        # Now override the DSN values with any values passed in
        if user:
            self.user = user
        if password:
        ...
        # finally, set the default value if we haven't found anything else
        if self.tls is None:
...
class Cursor:
    ...
    def _execute_query(self, operation, query_type: str = 'SELECT'):
        ...
        query_id = getattr(rsp, 'query_id', None)
    ...
    def _execute_list(self, operation: str = 'LIST ALL QUERIES'):
        ...
        self.list_result = factory.process(rsp)
...
def connect(dsn=None, user=None, password=None, host=None, database=None, tls=None, force=None):
    ...
    """
    return Connection(dsn, user, password, host, database, tls, force)
...
def main():
    ...
    parser = ArgumentParser(description=f"""Ocient Python client {version}.
                                           ...
                                           Supported parameters are `tls` and `force`
                                        """)
    ...
    outgroup.add_argument('-o','--outfile', type=FileType('w'), default='-',
                          help="Output file")
    outgroup.add_argument("-n", "--noout", action="store_true",
    ...
    parser.add_argument("sql", nargs='*',
                        help="SQL statements. Multiple statements may be provided")
    ...
    def doQuery(query):
        ...
        if not args.noout and cursor.description is not None:

            if cursor.explain_result:
                rows_as_dicts = [{'explain':json.loads(cursor.explain_result)}]
            else:
                colnames = [x[0] for x in cursor.description]

                # convert each row from a tuple to a dictionary
                # using the column names from the description field
                rows_as_dicts=[]
                for row in result:
                    rows_as_dicts.append(dict(zip(colnames, row)))

            print(json.dumps(rows_as_dicts, indent=4, default=_custom_type_to_json), 
                file=args.outfile)
