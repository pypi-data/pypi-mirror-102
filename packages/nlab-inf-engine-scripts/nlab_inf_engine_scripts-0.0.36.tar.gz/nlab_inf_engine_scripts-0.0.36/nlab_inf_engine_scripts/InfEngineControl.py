...
def main():
    ...
    group.add_argument(
        "--no-respawn-on-update",
        action="store_true",
        help=f"Disable auto respawn on dl update finish.",
    )
    ...
    try:
        if args.start:
            ...
            if args.no_respawn_on_update:
                respawn_dp = False
            else:
                respawn_dp = True

            server.update_data(
                dldata=args.dl_update, db_name=args.db_name, respawn_dp=respawn_dp
            )
            success()
        elif args.dl_delete:
