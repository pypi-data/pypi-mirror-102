...
class CAT(object):
    ...
    def multiprocessing(self, in_data, nproc=8, batch_size_chars=1000000, max_memory_percent=80, only_cui=False, addl_info=[]):
        ...
        max_memory_percent: if set it will limit RAM usage to a certain percentage
        ...
        for i in range(nproc):
            p = Process(target=self._mp_cons, kwargs={'in_q': in_q, 'out_dict': out_dict, 'pid': i, 'only_cui': only_cui,
                'addl_info': addl_info, 'max_memory_percent': max_memory_percent})
            p.start()
            procs.append(p)
    ...
    def _mp_cons(self, in_q, out_dict, pid=0, only_cui=False, addl_info=[], max_memory_percent=-1):
        cnt = 0
        out = []
        while True:
            if not in_q.empty():
                    ...
                    try:
                        if max_memory_percent > 0:
                            ...
                            while psutil.virtual_memory().percent > max_memory_percent:
                                cnt += 1
                                self.log.debug("Process: {}, waiting because current RAM usage at: {}".format(pid, psutil.virtual_memory().percent))
                                # Wait until size in memory is low
                                sleep(3)
                                if cnt % 200 == 0:
                                    self.log.info("Process: {}, waiting because current RAM usage at: {}, cnt: {}".format(
                                        pid, psutil.virtual_memory().percent, cnt))
