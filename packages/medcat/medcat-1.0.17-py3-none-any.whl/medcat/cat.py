                ...
                for id, text in data:
                    try:
                        if max_chars_in_memory > 0:
                            cnt = 0
                            while (out_dict['current_size_in_chars'] + len(text))) > max_chars_in_memory:
                                cnt += 1
                                ...
                                sleep(3)
                                if cnt % 200 == 0:
                                    self.log.info("Process: {}, waiting because current chars in memory: {}, cnt: {}".format(
                                        pid, out_dict['current_size_in_chars'], cnt))
                                if cnt % 2000 == 0:
                                    self.log.info("Breaking out: {} - a stuck process because current chars in memory: {}, cnt: {}".format(
                                        pid, out_dict['current_size_in_chars'], cnt))
                                    # Reset, probably something went wrong somewhere
                                    out_dict['current_size_in_chars'] = 0
                                    break
