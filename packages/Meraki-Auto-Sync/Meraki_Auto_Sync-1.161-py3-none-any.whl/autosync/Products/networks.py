...
class networks(object):
    ...
    async def Update_Network(self, master: object,
       ...
       #"No Seetins to Update here from master"
        if self.Network['timeZone'] == master.Network['timeZone']:
            print(
                    f'\t{lib.bc.OKBLUE} {netName} {lib.bc.OKGREEN}-Settings for  {task} Time Zone Matched Master{lib.bc.ENDC}')
        else:
            if appcfg.WRITE:
                print(
                        f'\t{lib.bc.OKGREEN}Updating Time Zone Settings in network {netName} {lib.bc.WARNING}{lib.bc.ENDC}')
                self.ReSync = True
                await db.networks.updateNetwork(net_id, timeZone=master.Network[
                    'timeZone'])
    ...
    async def Update_NetworkGroupPolicies(self,
        ...
        for localgp in self.NetworkGroupPolicies:
            mastergp = await lib.idFromName(master.NetworkGroupPolicies,
											localgp['name'])
            if mastergp is None:
                print(
                    f'{lib.bc.WARNING}{netName} - {task}\t\t{lib.bc.OKBLUE}Removing GP Policy named {localgp["name"]}{lib.bc.ENDC} not found in Master')
                await db.networks.deleteNetworkGroupPolicy(net_id, groupPolicyId=str(localgp['groupPolicyId']))
