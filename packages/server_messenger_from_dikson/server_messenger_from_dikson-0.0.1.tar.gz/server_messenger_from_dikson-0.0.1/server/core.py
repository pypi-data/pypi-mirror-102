...
class MessageProcessor(threading.Thread):
    """Основной класс сервера. Инициализация и запуск сокета. Подключение к базе данных.
    Прием и обработка сообщений от клиентов, отправка ответных сервисных сообщений и сообщений для пользователей
    по протоколу JIM. Возможность регистрации пользователей. Проверка на активного пользователя,
    корректность сообщений от клиентов"""
    ...
    # Запуск сокета
    def run(self):
        """Основной метод инициализации сокета."""
    ...
    def remove_client(self, client):
        """Метод для проверки отключенных (неактивных) клиентов (удаляет их)"""
    ...
    def process_message(self, message):
        """Метод, формирующий сообщения по протоколу JIM для клиентов, отправка сообщений."""
    ...
    def process_client_message(self, message, client):
        """Метод, реализующий обработку поступающих сообщений."""
    ...
    def autorize_user(self, message, sock):
        ...
        # Если имя пользователя уже занято то возвращаем код 400
        ...
        # Проверяем, что пользователь зарегистрирован на сервере.
            ...
            # Иначе отвечаем кодом 511 и проводим процедуру авторизации
            # Словарь - заготовка
            ...
            hash_pswd = hmac.new(self.database.get_hash(message[USER][ACCOUNT_NAME]), random_str, 'MD5')
            digest = hash_pswd.digest()
