...
class StreamConnection(threading.Thread, LogReporter):
    ...
    _DefaultPrefixStreamConnectionNameToProtocolToDataFormat = { 'streaming/pricing' : { 'OMM' : 'tr_json2', 
                                                                                    },
                                                                'streaming/trading-analytics' : { 'OMM' : 'tr_json2',
                                                                                                    'RDP' : 'rdp_streaming',
                                                                                                 }, 
                                                                'streaming/quantitative-analytics' : { 'RDP' : 'json',
                                                                                                    },
                                                                'streaming/benchmark' : { 'RDP' : None,
                                                                                        }
                                                            } 
    ...
    def __init__(self, name:str, 
        ...
        self._max_retry_connection = self._DefaultMaxRetryConnection if not self._session.server_mode else None
    ...
    def websocket_subprotocol(self):
        ...
        prefix_stream_connection_name, endpoint_name = os.path.split(self._connection_name)
        assert prefix_stream_connection_name in self._DefaultPrefixStreamConnectionNameToProtocolToDataFormat

        #   get the mapping from protocol to data format
        connection_protocol_to_dataformat = self._DefaultPrefixStreamConnectionNameToProtocolToDataFormat[prefix_stream_connection_name]
    ...
    def run(self):
        ...
        while not self.is_closing and self._do_reconnect:
            ...
            self._websocket = websocket.WebSocketApp(
                ...
                subprotocols=[self.websocket_subprotocol,] if self.websocket_subprotocol is not None else None,
                )
            ...
            if not self.is_closing and self._do_reconnect:
                ...
                if self._max_retry_connection is not None and self._num_retries >= (self._max_retry_connection * num_websocket_endpoints):
    ...
    async def _login_async(self):
        ...
        self.debug(f'Sending the login message. url={self._connection_config.url}, message={login_message}')
        with self._ws_lock:
    ...
    async def _close_async(self):
        ...
        if self._state != self.State.CLOSED:
            ...
            if self._is_closing:
                ...
                self.debug(f'The stream connection is closing from {self._connection_config.url}')
            ...
            self.debug(f'The stream connection is already closed from {self._connection_config.url}')
    ...
    def _ws_reconnect(self, failover_state, stream_state, data_state, state_code, state_text):
        ...
        if self._latest_failover_state is None \
            ...
            for stream in subscription_streams:
                ...
                self._loop.call_soon_threadsafe(functools.partial(stream._on_reconnect, 
