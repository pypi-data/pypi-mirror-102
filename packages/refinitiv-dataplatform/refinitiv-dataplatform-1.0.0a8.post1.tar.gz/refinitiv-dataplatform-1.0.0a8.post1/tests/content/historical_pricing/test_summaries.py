...
def test_get_data(open_session):
    with mock.patch('refinitiv.dataplatform.content.HistoricalPricing._get_summaries') as hp_mock:
        with mock.patch('refinitiv.dataplatform.legacy.tools.get_default_session') as m:
            rdp.historical_pricing.summaries.Definition('').get_data()
            assert hp_mock.call_count == 1

    with mock.patch('refinitiv.dataplatform.content.HistoricalPricing._get_summaries') as hp_mock:
        rdp.historical_pricing.summaries.Definition('').get_data(session=open_session)
        assert hp_mock.call_count == 1


def test_get_data_async(open_session):
    async def get_async():
        definition = rdp.historical_pricing.summaries.Definition('VOD.L')
