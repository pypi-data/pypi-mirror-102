...
def test_import_full_scores():
    try:
        from refinitiv.dataplatform.content.esg import full_scores
    except ImportError:
        assert False


def test_has_attributes(open_session):
    definition = rdp.esg.full_scores.Definition('', session=open_session)
    ...
    # assert definition._Definition__data_type == DataType.FullScores
...
def test_get_data_without_session_argument(open_session):
    definition = rdp.esg.full_scores.Definition('')
