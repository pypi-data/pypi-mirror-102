...
def test_success_response_is_success(response):
    ...
    assert success, [response.error_code, response.error_message, response.request_message.url]
...
def test_success_response_status(response):
    ...
    assert status['http_status_code'] == 200, status['error'] if status.get('error') else status
