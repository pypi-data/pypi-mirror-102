...
class WebSocket(object):
    ...
    def handleConnected(self):
        ...
        """
        logging.info(f'{self.address} connected at {time.time()}')

    def handleClose(self):
        """
            Called when a websocket server gets a Close frame from a client.
        """
        logging.info(f'{self.address} closed at {time.time()}')
...
class SimpleWebSocketServer(object):
    ...
    def get_server_data(cls):
        return SimpleWebSocketServer.__server_data

    @classmethod
    def get_scenario(cls):
        return SimpleWebSocketServer.__scenario

    @classmethod
    def set_scenario(cls, scenario):
        SimpleWebSocketServer.__scenario = scenario

    def __init__(self, host, port, websocketclass, config=None):
        self.events = {}
        ...
        self.selectInterval = self.config.get("select_interval", 0.1)
        self.connections = {}
        self.listeners = [self.serversocket]

        if SimpleWebSocketServer.__server_data is None:
            dirpath = os.path.dirname(__file__)
            filepath = os.path.join(dirpath, self.config.get("data_filename", "data.json"))
            with open(filepath) as f:
                raw_data = f.read()
            SimpleWebSocketServer.__server_data = json.loads(raw_data)
            SimpleWebSocketServer.__scenario = "happy_path"

    def set_scenario(self, scenario):
        if scenario in SimpleWebSocketServer.__server_data.keys():
            SimpleWebSocketServer.__scenario = scenario
        else:
            print(f'[WARN] {scenario} does not exists in server data.')
