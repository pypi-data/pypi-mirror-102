...
class Session(abc.ABC, OMMStreamObserver, StreamingChainObserver):
    ...
    def _get_streaming_connection_config_name_and_endpoint_name(self, streaming_connection_name:str):
        """ 
        Parameters
        ----------
        ...
        streaming_connection_config_endpoint_name = f'{streaming_connection_config_name}.endpoints.{endpoint_name}'

        #   done 
        return streaming_connection_config_name, streaming_connection_config_endpoint_name
    ...
    def get_streaming_protocols(self, streaming_connection_name:str):
        ...
        streaming_connection_config_name, streaming_connection_config_endpoint_name = self._get_streaming_connection_config_name_and_endpoint_name(streaming_connection_name)
        
        #   retrieve the protocols from config file
        streaming_connection_protocols = configure.config.get_list(f'{streaming_connection_config_endpoint_name}.protocols')
    ...
    def get_streaming_websocket_endpoint_url(self, streaming_connection_name:str):
        ...
        #       websocket url
        try:
            streaming_connection_endpoint_websocket_path = configure.config.get_str(f'{streaming_connection_config_endpoint_name}.websocket-url')
        except KeyError:
    ...
    def get_streaming_discovery_endpoint_url(self, streaming_connection_name:str):
        ...
        #       base streaming url
        try:
            streaming_connection_path = configure.config.get_str(f'{streaming_connection_config_name}.url')
        except KeyError:
            streaming_connection_path = None
        #       path
        try:
            streaming_connection_endpoint_path = configure.config.get_str(f'{streaming_connection_config_endpoint_name}.path')
        except KeyError:
            streaming_connection_endpoint_path = None
        self.debug(f'     building the streaming endpoint..........\n             base streaming path    = {streaming_connection_path}\n             endpoint path          = {streaming_connection_endpoint_path}')

        #   build the stream discovery endpoint url
        if streaming_connection_path is not None and streaming_connection_endpoint_path is not None:
        #   build the stream discovery endpoint
            return urljoin(urljoin(self._get_rdp_url_root(), streaming_connection_path), streaming_connection_endpoint_path)
        else:
    ...
    async def _create_and_start_stream_connection(self, stream_connection_name:str, 
        ...
        self.debug(f'{self.__class__.__name__}[{self._session_name}]._create_and_start_stream_connection(stream_connection_name={stream_connection_name},connection_protocol={connection_protocol_name})')
        ...
        stream_connection_config = await self._get_stream_connection_configuration(stream_connection_name)
        ...
        if connection_protocol_name == 'OMM':
            ...
            name = f'WebSocket {self.session_id} - OMM Protocol - {stream_connection_name}'

            #   create the stream OMM connection for pricing
            stream_connection = OMMStreamConnection(name, self, stream_connection_name, stream_connection_config, connection_protocol_name)

        elif connection_protocol_name == 'RDP':
            #   construct the websocket thread name
            name = f'WebSocket {self.session_id} - RDP Protocol - {stream_connection_name}'

            #   create the stream OMM connection for pricing
            stream_connection = RDPStreamConnection(name, self, stream_connection_name, stream_connection_config, connection_protocol_name)
    ...
    async def wait_for_streaming_reconnection(self, stream_connection_name: str, 
                                                    connection_protocol_name: str=None):
    ...
    async def _start_streaming(self, stream_connection_name:str, 
        ...
        try:
            ...
            raise SessionError(-1, f'ERROR!!! CANNOT establish the WebSocket connection for {stream_connection_name}')
    ...
    def _start_service_directory(self, stream_connection_name: str):
        ...
        if stream_connection_name not in self._service_directory:
            self._service_directories[stream_connection_name] = StreamingServiceDirectory(self, stream_connection_name)
        ...
        if status is Session.EventCode.StreamConnected:
            ...
            self.debug(f"Can't start service directory because {stream_connection_name} is closed")
            return False
    ...
    def _register_stream(self, stream):
        ...
        with self._stream_register_lock:
            ...
            if stream.api is None:
                raise SessionError('Error', f'Try to register but given stream[{stream.stream_id}] has api property is None.')
    ...
    def _unregister_stream(self, stream):
        with self._stream_register_lock:
            ...
            if stream.stream_id not in self._all_stream_subscriptions:
                raise SessionError('Error', f'Try to un-register unknown stream[{stream.stream_id}] '
                                       f'from session {self.session_id}.')

            if stream.api is None:
                raise SessionError('Error', f'Try to un-register but given stream[{stream.stream_id}] has api property is None.')
            
            if stream.api not in self._stream_connection_name_to_stream_ids_dict:
                raise SessionError('Error', f'Try to un-register unknown stream api[{stream.api}] of stream[{stream.stream_id}].')
    ...
    def report_session_status(self, session, event_code, event_msg):
        ...
        if _callback is not None:
            try:
                ...
                self._session.error(f'{self.__name__} on_event or on_state callback raised exception: {e!r}')
    ...
    ############################
    async def _http_request_async(self, url: str, method=None, headers=None,
        ...
        assert isinstance(kwargs['timeout'], int) or isinstance(kwargs['timeout'], float), 'ERROR!!! HTTP request timeout must be int or float'

        try:
            request_response = await self._http_session.send(_http_request, **kwargs)
        except httpx.RequestError as error:
            self.error(f'ERROR!!! An error occurred while requesting {error.request.url!r}.\n     {error!r}')
            raise error
