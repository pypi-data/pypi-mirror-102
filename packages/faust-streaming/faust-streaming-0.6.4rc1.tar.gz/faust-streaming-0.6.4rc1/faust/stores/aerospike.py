...
class AeroSpikeStore(base.SerializedStore):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            _key_index = LRUCache(limit=4096)
    ...
    def _get(self, key: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            gen = meta.get(self.GENERATION_KEY, 1)
    ...
    def _set(self, key: bytes, value: Optional[bytes]) -> None:
        try:
            ...
            self._key_index.pop(key)
        except RecordGenerationError as rge:
            self.log.error(
                f"Error in set for table {self.table_name} exception {rge} key {key} due to generation error generation {gen}"
            )
            raise rge
        except Exception as ex:
            self.log.error(
