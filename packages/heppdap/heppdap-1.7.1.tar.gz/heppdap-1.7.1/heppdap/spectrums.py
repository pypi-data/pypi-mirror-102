...
class Spectrumframe (wx.Frame):
    ...
    def __init__(self, title = "Spectrum", parent=None, data = df, bins = 100, mode = None, params = None, xmin = 0, xmax = 0.5, showparams = False ):
 #       app=wx.App(None)
        super().__init__(None, title=title)
        self.plotter = PlotNotebook(self, id=-1, pos=(100,100), size=(300,300))                
        axes = self.plotter.add(mode + "Hist").gca()
        axes.hist(data, range=[xmin, xmax], bins = bins, color = "Black", histtype="step")
        sigma = data.std()
