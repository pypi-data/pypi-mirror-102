...
class MainFrame(wx.Frame):
    ...
    def InitFrame(self):
        ...
        #Export Options        
        csvexportitem = exportMenu.Append(wx.ID_ANY, "CSV", "CSV")
        self.Bind(wx.EVT_MENU, self.panel.csv_export, csvexportitem)
        self.Bind(wx.EVT_MENU, self.panel.csv_export, id=ceid)
        excelexportitem = exportMenu.Append(wx.ID_ANY, "Excel", "Excel")
        self.Bind(wx.EVT_MENU, self.panel.excel_export, excelexportitem)
        self.Bind(wx.EVT_MENU, self.panel.excel_export, id=eeid)
        jsonexportitem = exportMenu.Append(wx.ID_ANY, "JSON", "JSON")
        self.Bind(wx.EVT_MENU, self.panel.json_export, jsonexportitem)
        self.Bind(wx.EVT_MENU, self.panel.json_export, id=jeid)
...
#          """
    ...
    def plotspec(self, event):
        ...
        titleprompt = wx.TextEntryDialog(self, "Enter Graph Title", "Configure Graph")
        titleprompt.params = wx.CheckBox(titleprompt, id=wx.ID_ANY, pos = (10,60), label = "Display Parameters")

        if titleprompt.ShowModal() == wx.ID_OK:
            title = str(titleprompt.GetValue())
            showparams = titleprompt.params.IsChecked()
        else:
            return

        hist = spectrums.Spectrumframe(title, data = amps, bins = bins, mode = characteristic.title(), params=params, xmin = xmin, xmax = xmax, showparams = showparams)
        return
    ...
    def csv_export(self, event):
        with wx.FileDialog(self, "Save CSV file", wildcard="TXT files (*.csv)|*.csv",
    ...
    def json_export(self, event):
        with wx.FileDialog(self, "Save JSON file", wildcard="TXT files (*.json)|*.json",
                       style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT) as fileDialog:
        ...
        # save the current contents in the file
            ...
            try:
                    for index, event in enumerate(self.data.events):
                        ...
                        fullevent = pd.concat(event_data, axis = 1)
                        fullevent.reset_index()
                        all_events.append(fullevent)
 
                    keys = list(range(1, index + 1))

                    all_events = pd.concat(all_events, keys = keys)
                    with open(pathname, 'w') as f:
#                        all_events.to_json(f, orient = "table")
#                         all_events.rename_axis("Time (ns)", axis = "index", inplace=True)
                         f.write(all_events.to_string())
