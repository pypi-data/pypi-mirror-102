...
class hmmer():
    ...
    def runhmm(self, hmmmold, path):
        ...
        m1 = hmmsearch + ' --cut_tc --domtblout one.out ' + hmmmold + ' ' + self.pep
        ...
        # x = input()
        a, b = Clustalw_cline()
        # elif (self.comparison == diamond):
        #     ml4 = self.diamond_path + ' makedb --in ' + in_file +' prodb'
        #     ml5 = self.diamond_path + ' blastp --db prodb -q ' + in_file + ' -o ' + out_file
        #     d = os.system(ml4)
        #     d = os.system(ml5)
        os.remove ('one.pep')
        ...
        d = os.system(m2)# 格式转换
        # print(m2)
        m3 = hmmsearch + ' --cut_tc --domtblout ./out_list/' + name0 + '.out ./out_hmm/' + name0 + '.hmm ' + self.pep
        # x = input()##################
    ...
    def changefile(file,list, hmmpath):
        for i in list:
            name = str(i)[:-4] + '.hmm'
            d = os.system("hmmbuild %s %s" % (name, i))
    ...
    def run(self):
        # pa = os.getcwd()
        if os.path.isdir('./out_hmm') and os.path.isdir('./out_pep') and os.path.isdir('./out_list') and os.path.isdir('./out_aln'):
            pass
        else:
            d = os.mkdir("out_hmm")
            d = os.mkdir("out_pep")
