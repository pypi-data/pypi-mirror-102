...
class FECNNLayer(Layer):
    ...
    def __init__(self, filters=(14, 16,), kernel_width=(7, 7,), pooling_width=(2, 2), **kwargs):
        ...
        super(FECNNLayer, self).__init__(**kwargs)
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(1, len(self.filters) + 1):
            ...
            ext_feature_list.append(
                tf.reshape(pooling_result, (-1, pooling_result.shape[1]*pooling_result.shape[-1], embedding_size)))
...
class ANLayer(Layer):
    """Attentonal Network models without linear term and bias.
      ...
      Input shape
        - A list of 3D tensor with shape: ``(batch_size,num_feature_interaction,embedding_size)``.
    ...
    def build(self, input_shape): # creating layer weights: 레이어를 초기화
        ...
        if not isinstance(input_shape, list) or len(input_shape) < 2:
            # input_shape = input_shape[0]
            # if not isinstance(input_shape, list) or len(input_shape) < 2:
            raise ValueError('A `Attentional Network` layer should be called '
                             'on a list of at least 2 inputs')
        ...
        if len(shape_set) > 1:
            raise ValueError('A `Attentional Network` layer requires '
                             'inputs with same shapes '
                             'Got different shapes: %s' % (shape_set))
        ...
        if len(input_shape[0]) != 3 or input_shape[0][1] != 1:
            raise ValueError('A `Attentional Network` layer requires '
                             'inputs of a list with same shape tensor like\
                             (None, num_feature_interaction, embedding_size)'
                             'Got different shapes: %s' % (input_shape[0]))
