...
||||||| merged common ancestors
        self.map = Map(location=center, zoom_start=zoom)
=======
        self.map = Map(location=center, zoom_start=zoom,tiles='Stamen Terrain')
>>>>>>> conda-changes
        return self.map
    ...
    def plot_pair_with_line(self, data, loc_col, ctry_col, lat1, lng1, lat2, lng2, clr1="red", clr2="green",clrLine='orange'):
        """
        Create markers with lines between them: (lat0,lng0) to (lat1,lng1).
        ...
        :type ctry_col: str.
        :param lat1: name of latitude 1 column.
        :type lat1: str.
        :param lng1: name of longitude 1 column.
        :type lng1: str.
        :param lat2: name of latitude 2 column.
        :type lat2: str.
        :param lng2: name of longitude 2 column.
        :type lng2: str.
        :param clr1: color of first marker.
        :type clr1: str
        :param clr1: color of second marker.
        :type clr1: str
        :param clrLine: color of polyline.
        :type clrLine: str
        :return: all of the created markers and polylines between the markers.
        :rtype: list of folium.Marker and folium.Polyline.
        """
        MapItems = []

        df = read_data(data, {loc_col, ctry_col, lat1, lng1, lat2, lng2,})

        for (index, row) in df.iterrows():
            if pd.notnull(row[lat1]) and pd.notnull(row[lng1]) and pd.notnull(row[lat2]) and pd.notnull(row[lng2]):
                lat1_val = row[lat1]
                lng1_val = row[lng1]
                lat2_val = row[lat2]
                lng2_val = row[lng2]
                ...
                marker1 = self.plot_point(lat=lat1_val, lng=lng1_val,
                                         desc='%s, %s : (%.2f, %.2f)' % (location[0], location[1], lat1_val, lng1_val), clr=clr1)
                MapItems.append(marker1)

                marker2 = self.plot_point(lat=lat2_val, lng=lng2_val,
                                          desc='%s, %s : (%.2f, %.2f)' % (location[0], location[1], lat2_val, lng2_val),
                                          clr=clr2)
                MapItems.append(marker2)

                points = [[lat1_val, lng1_val], [lat2_val, lng2_val]]

                LineConnectingMarker = PolyLine(points, color=clrLine)
                MapItems.append(LineConnectingMarker)
    ...
    def plot_correct_data(self, data, loc_col, ctry_col, lat_col, lng_col, clr='green', as_cluster=False):
        ...
        gdf['ISO2'] = coco.convert(list(gdf[ctry_col]), to='ISO2',not_found=None)
