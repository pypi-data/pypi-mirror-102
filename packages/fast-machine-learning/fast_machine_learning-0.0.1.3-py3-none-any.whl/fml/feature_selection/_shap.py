...
class Shap:
    ...
    def fit(self, model, X, Y, explainer="auto", feature_names=None, shap_kwargs=dict(), model_kwargs=dict()):
        ...
        if model in additional_model_kwargs.keys():
            model_kwargs.update(additional_model_kwargs[model])
        ...
        if Explainer == Permutation:
            ...
            model = model(**model_kwargs).fit(X, Y)
        self.explainer = Explainer(model, X, **shap_kwargs)
...
if __name__ == "__main__":
    ...
    s = Shap().fit(XGBClassifier, X, Y, feature_names=load_breast_cancer().feature_names)
