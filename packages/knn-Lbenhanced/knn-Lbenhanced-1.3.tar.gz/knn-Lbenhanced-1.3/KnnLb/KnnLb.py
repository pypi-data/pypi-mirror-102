...
class SequenceStats:
    ...
    def __init__(self, dataset=None, window=1):
...
class KnnLb(object):
    ...
    def predict(self, x):
        sequence_stats_cache = SequenceStats([x])
        dm = self._dist_simple_matrix_lb(x, self.x, sequence_stats_cache, self.window, self.V)
    ...
    def score(self, x, y):
        self.train_cache = SequenceStats(x)
        labels, probas = self.predict_proba(x)
        correct = 0
