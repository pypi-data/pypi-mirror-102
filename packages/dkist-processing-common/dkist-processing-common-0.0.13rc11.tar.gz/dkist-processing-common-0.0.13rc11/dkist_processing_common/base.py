...
from dkist_processing_common._util.graphql import RecipeRunQuery
...
class TaskBaseExt(TaskBase, ABC):
    ...
    @property
    def input_paths(self) -> Generator[Path, None, None]:
        """
        Get all of the filepaths for objects stored in the recipe_run_id/input dir
        Returns
        -------
        A generator of Path objects, each corresponding to one file in the input dir
        """
        return [p for p in Path(self.input_dir).iterdir() if p.suffix in [".fits", ".FITS"]]
    ...
    def read_input_fits(
        self, datatype: str, modstate: Optional[int] = None
    ) -> Generator[Path, None, None]:
        """
        Get paths for all data tagged with "INPUT" and the datatype requested
        """
        if modstate:
            return self.read_fits(tags=["INPUT", datatype.upper(), f"MODSTATE {modstate}"])
    ...
    def write_translated_input_fits(
        self, hdul: fits.HDUList, filename: str, tags: Optional[List] = []
    ):
        """
        Write a translated input file from the summit to disk
        Parameters
        ----------
        hdul:
            translated HDUList to write to file
        filename: str
            filename to use in the write
        tags: Optional[List]
        ...
        None
        """
        self._scratch.write_fits(data=hdul, relative_path=filename, tags=["INPUT"] + tags)
...
class ScienceTaskL0ToL1(TaskBaseExt, ABC):
    """
    ...
    def input_lamp_gain_frames(self, modstate: Optional[int] = None) -> Generator[Path, None, None]:
        """
        Get the list of input lamp gain frames
        """
        return self.read_input_fits(datatype="LAMP GAIN", modstate=modstate)
    ...
    def write_lamp_gain(
        ...
        """
        Write the calibrated lamp gain frame, including a modulation state if necessary
        """
        if modstate:
            tags = tags or []
            ...
            self.write_intermediate_fits(
                ...
                datatype="LAMP GAIN",
                tags=tags,
                filename=f"GAIN_MODSTATE_{modstate}.fits",
            )
        else:
            self.write_intermediate_fits(data=data, datatype="LAMP GAIN", tags=tags)
    ...
    def intermediate_lamp_gain(self, modstate: Optional[int] = None) -> Path:
        """
        Get the calibrated gain frame
        """
        return self.read_intermediate_fits(datatype="LAMP GAIN", modstate=modstate)
    ...
    @staticmethod
    def fits_to_numpy(
        fits_filepaths: Union[Iterable[Path], Path]
    ) -> Generator[np.ndarray, None, None]:
        """
        Yields numpy arrays from an iterable of paths to fits files

        Calls `CommonFitsData.from_file` under the hood.

        Parameters
        ----------
        fits_filepaths
            generator of pathlib.Path objects corresponding to the fits files

        Yields
        -------
        A single `numpy.ndarray` for each filepath
        """
        fits_filepaths = [fits_filepaths] if isinstance(fits_filepaths, Path) else fits_filepaths
        for fits_filepath in fits_filepaths:
            # It's OK to always use CommonFitsData here because we're throwing away the header metadata
            fits_obj = CommonFitsData.from_file(fits_filepath)
            yield fits_obj.data
