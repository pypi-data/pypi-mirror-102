...
class Query(object):
	...
	def get_dataframe(self, year, field, unit, update):
		...
		return df
	def get_stock(self, security, start, end, field_list, unit, fq=None, get_today=False):
		# 获取单只股票数据
		...
		if unit != '1m':
			...
			if unit in ['5m','15m','30m','60m','120m']:				
				...
				raise Exception('only 5m,15m,30m,60m,120m,1d,1W,1M unit is available, but got {}'.format(unit))
