...
class LocalClient:
    ...
    def read(self, url):
        ...
        r = resource["resource"]
        if r["type"] == "entry":
            r["author"] = self.get_version(r["author"])["resource"]
        return resource
    ...
    def create(self, resource_type, visibility="public", **resource):
        ...
        published = resource.get("published", web.utcnow())
        path = "/"
        mentions = []
        if resource_type == "card":
            ...
            timeslug = web.timeslug(published)
            if post_type == "note":
                ...
                # mentions.append(events[0]["url"])
            path += f"{timeslug}/{web.textslug(textslug)}"
            # author = {"type": "card", **self.read("")["resource"]}
            # author.pop("visibility", None)
            author_id = tx.db.select("resources", where="url = ?",
                                     vals=["/people/me"],
                                     order="modified DESC")[0]["version"]
            resource.update(published=published, url=path, author=author_id)
        resource["type"] = resource_type
        resource["visibility"] = visibility
        resource_kwargs = dict(url=path, modified=published, resource=resource,
...
@server.route(r"authorship")
class Authorship:
    """."""

    def _get(self):
        return templates.authorship()
