...
class OWTestAndScore(OWWidget):
    ...
    def update_stats_model(self):
        ...
        param_keys = self.score.keys()
        for key, slot in self.learners.items():
            name = learner_name(slot.learner) #random forest
            names.append(name)
            ...
            if stats is not None: #한 행 만드는 작업
                for i in range(len(param_keys)): #파라미터
                    row2 = row.copy()
                    tem = QStandardItem()
                    tem.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
                    tem.setData(list(param_keys)[i], Qt.DisplayRole)
                    row2.append(tem)

                    for idx in range(0,5):  # 값, 평가지표
                        item = QStandardItem()
                        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)

                        item.setData(float(self.score[list(param_keys)[i]][idx]), Qt.DisplayRole)
                        row2.append(item)  # 한 행이 다 만들어졌으면
                    model.setHorizontalHeaderLabels(["Model", "n_estimator", "AUC", "CA", "F1", "Precision", "Recall"])
                    model.appendRow(row2)
                    '''
                    for stat, scorer in zip(stats, self.scorers): #값, 평가지표
                        item = QStandardItem()
                        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
                        if stat.success:
                            item.setData(float(self.score[list(param_keys)[i]][0]), Qt.DisplayRole)
                            print(idx)
                            idx+=1
                        else:
    ...
    def black_box_function(self, n_estimators):
        ...
        s[n_estimators] = Orange.evaluation.scoring.AUC(res)[0] #auc값을 기준으로 판단

        if n_estimators not in self.score.keys() or self.score[n_estimators][0] < s[n_estimators]: #초기값
            self.score[n_estimators] = [s[n_estimators], Orange.evaluation.scoring.CA(res)[0],
                                        Orange.evaluation.scoring.F1(res, average='weighted')[0],
                                   Orange.evaluation.scoring.Precision(res, average='weighted')[0],
                                   Orange.evaluation.scoring.Recall(res, average='weighted')[0]]
    ...
    def opt(self):
        ...
        optimizer.maximize(init_points=5, n_iter=10, acq='ei', xi=0.6)
