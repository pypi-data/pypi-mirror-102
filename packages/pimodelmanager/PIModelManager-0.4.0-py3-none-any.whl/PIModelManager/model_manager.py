        ...
        if model["model_type"] == "spacy":
            ...
            if name and version:
                if cls.__pip_package_is_available(name, version):
                    ...
                    model_manager_logger.info(f"Downloading package {url}")
                    ...
                    model_manager_logger.info(f"Return code: {output.returncode}")
                    if(output.returncode != 0):
                        raise Exception(f"Error when installing package {package}")
                    else:
                        model_manager_logger.info(f"Successfully installed package: {package}")
    ...
    def __download_model_dataservices(cls, model: Dict[str, str]) -> None:
        ...
        if len(matched_models) == 0:
            ...
            if temp_path.exists():
                ...
                model_manager_logger.info(f"Downloading model in {temp_path}")
    ...
    def delete_models(cls) -> None:
        ...
        if("allennlp" in model_types):
            if not cls.__pip_package_is_available("allennlp"):
                model_manager_logger.warning("You have allennlp model, but allennlp package is missing. Not possible to delete cached allennlp models.")
            else:
                try:
                    from allennlp.common.file_utils import remove_cache_entries
                    cleaned_bytes = remove_cache_entries(patterns=[])
                    model_manager_logger.info(f"Removed {cleaned_bytes} bytes of cached allennlp models.")
                except ImportError:
                    model_manager_logger.warning("Could not import allennlp. Not possible to delete cached allennlp models.")
