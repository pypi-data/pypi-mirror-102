        ...
        if model["model_type"] == "spacy":
            ...
            if name and version:
                if cls.__pip_package_is_available(name, version):
                    ...
                    #subprocess.check_call([sys.executable, "-m", "pip", "install", url, "-y"])
                    output = subprocess.run(
                        ["python", "-m", "pip", "install", url, "-y"], 
                        stdout=subprocess.PIPE, 
                        stderr=subprocess.STDOUT
                    )
                    model_manager_logger.info(f"Return code: {output.returncode}")
                    if(output.returncode != 0):
                        raise Exception(f"Error when installing package {package}: {output.stderr.decode('utf-8')}")
                    else:
                        model_manager_logger.info(f"Output: {output.stdout.decode('utf-8')}")
