...
class ServiceConf():
    ...
    # def __get_keyvalue(self, key, default=None):
    #     '''
    #     获取kvstore的value
    ...
    #     :return:
    #     '''
    #     return ServiceConf.get_keyvalue(key,default,self.consul)
...
class RedisConf():
    ...
    def __init__(self,tag,near=False,no_exist_except=True):
        ...
        self._ip = sc.address
        self._port = sc.port
        self._db = sc.get_keyvalue('redis_%s_db' % tag, 0,sc.consul)
...
class DatabaseConf():
    def __init__(self,dbname='maxbus'):
        ...
        self._port = sc.port
        self._dbdriver = 'mssql+pymssql' if sc.get_keyvalue('%s_dbdriver' % dbname,consul=sc.consul) == 'mssql' else 'mysql+mysqldb'
