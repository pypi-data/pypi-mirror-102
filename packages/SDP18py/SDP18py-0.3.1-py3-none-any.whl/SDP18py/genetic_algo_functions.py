...
def pareto_ranking(scores1, solns, n):
	...
	:param scores1: (lists) of scores, have to correspond with solns index
	:param solns: (list) population of solutions
	:param n: (int) max number of ranks wanted
	:return: (dict) the solutions scores with the diction key as rank and value as scores
	"""
...
def select_two_parent(ranks, list_scores, list_soln):
	"""
	Selection of 2 parents initialization (binary tournament selection method)
	:param ranks:
	:param list_scores: (list) of scores
	:param list_soln:  (list) of population solutions
	:return: (ndarry) 2 solutions corresponding to the 2 parents
	"""
...
def tournament(tournament_population, crowding_dist, list_scores):
	"""
	Binary tournament selection method, always take better pareto rank. If same rank, use crowding distance as tiebreaker
	:param tournament_population: (ndarray) tournament population of solutions
	:param crowding_dist: (list) of crowding distance for each solution in tournament population
	:param list_scores: (list) of tournament population scores
	:return: (list) scores of 2 finalist
	"""
...
def GA_crossover(p1, p2, sch_list, empty_schedule):
	"""
	For 2 parents to create an offspring which inherits characterisitics of both parents
	:param p1: (ndarray) parent 1
	:param p2: (ndarray) parent 2
	:param sch_list: (list) of surgeries that was to be scheduled
	:param empty_schedule: (ndarray) the initial schedule before the surgeries were fitted in
	:return: (ndarray) a child solution
	"""
...
def GA_mutate(child, clashes, MAS_allowed, OT_indexes, day_of_week_indexes):
	"""
	Fixes the solution by randomly slotting in clashed slots from parents
	:param child: (ndarray) solution that was created by crossover
	:param clashes: (list) of clashes due to crossover
	:param MAS_allowed: (dict) the MAS restrictions
	:param OT_indexes: (dict) the OTs in which the index in the array represents
	:param day_of_week_indexes: (dict) the day of the week in which the index in the array represent
	:return: (ndarray) 'fixed' child solution
	"""
	if len(clashes)==0:
		return child
	else:
