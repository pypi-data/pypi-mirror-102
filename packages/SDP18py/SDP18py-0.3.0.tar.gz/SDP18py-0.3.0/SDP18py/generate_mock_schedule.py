...
'''
Use this file to generate a mock csv file of surgeries, it will prompt for planning horizon and percentage full
'''
def get_slots_per_day(days, perc):
	"""
	Because not all days have same percentage full, we use exponentially decaying function to resemble real scenario
	:param days: (int) number of days of planning horizon
	:param perc: (int) overall percentage full across planning horizon
	:return: (ndarray) slots filled in each day
	"""
	slot_in_a_day = 23*44
...
def random_sum_to(n):
	"""
	Get a random sum to the number to fill, and also a random sum to the number of empty slots for that day
	:param n: (int) the number of slots to fill
	:return: (tup) returns a tuple of 2 list
	"""
	sum_n = 0
	m = 44 * 23 - n
	random_list_n = []
...
def fill_surg_or_breaks(lensurg, lenbreaks):
	"""
	A binary selector to choose whether to fill the schedule with a surgery or a break
	:param lensurg: (int) count of surgeries left to fill
	:param lenbreaks: (int) count of empty slots left to fill
	:return: (bool) fill surg or a break
	"""
	# print(lensurg)
	# print(lenbreaks)
