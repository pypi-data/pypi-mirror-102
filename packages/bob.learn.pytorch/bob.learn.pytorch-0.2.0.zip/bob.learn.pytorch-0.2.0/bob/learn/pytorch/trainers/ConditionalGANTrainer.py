...
class ConditionalGANTrainer(object):
  ...
  """
    ...
    """
        ...
        self.fixed_one_hot = torch.FloatTensor(
    ...
    def train(
        self, dataloader, n_epochs=10, learning_rate=0.0002, beta1=0.5, output_dir="out"
    ):
    ...
    """
        ...
        optimizerD = optim.Adam(
            self.netD.parameters(), lr=learning_rate, betas=(beta1, 0.999)
        )
        optimizerG = optim.Adam(
            self.netG.parameters(), lr=learning_rate, betas=(beta1, 0.999)
        )
        ...
        for epoch in range(n_epochs):
            for i, data in enumerate(dataloader, 0):
                ...
                noise = torch.FloatTensor(batch_size, self.noise_dim, 1, 1).normal_(
                    0, 1
                )
                label = torch.FloatTensor(batch_size)

                # create the one hot conditional vector (generator) and feature maps (discriminator)
                one_hot_feature_maps = torch.FloatTensor(
                    batch_size,
                    self.conditional_dim,
                    self.image_size[1],
                    self.image_size[2],
                ).zero_()
                one_hot_vector = torch.FloatTensor(
                    batch_size, self.conditional_dim, 1, 1
                ).zero_()
                ...
                logger.info(
                    "[{}/{}][{}/{}] => Loss D = {} -- Loss G = {} (time spent: {})".format(
                        epoch,
                        n_epochs,
                        i,
                        len(dataloader),
            ...
            # do checkpointing
            torch.save(
                self.netG.state_dict(), "%s/netG_epoch_%d.pth" % (output_dir, epoch)
            )
            torch.save(
                self.netD.state_dict(), "%s/netD_epoch_%d.pth" % (output_dir, epoch)
            )
