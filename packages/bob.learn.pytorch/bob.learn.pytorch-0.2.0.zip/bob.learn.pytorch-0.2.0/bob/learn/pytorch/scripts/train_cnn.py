...
def main(user_input=None):
    ...
    args = docopt(
        __doc__ % completions, argv=arguments, version="Train a CNN (%s)" % version,
    )
    ...
    if hasattr(configuration, "dataset"):
        dataloader = torch.utils.data.DataLoader(
            configuration.dataset, batch_size=batch_size, shuffle=True
        )
        num_classes = numpy.max(configuration.dataset.id_labels)
        logger.info(
            "There are {} training images from {} categories".format(
                len(configuration.dataset), num_classes
            )
        )
    else:
