...
def main(user_input=None):
    ...
    args = docopt(
        __doc__ % completions, argv=arguments, version="Train a MCCNN (%s)" % version,
    )
    ...
    adapted_layers = get_parameter(
        args, configuration, "adapted_layers", "conv1-block1-group1-ffc"
    )
    adapt_reference_channel = get_parameter(
        args, configuration, "adapt_reference_channel", False
    )
    ...
    if hasattr(configuration, "network"):
        trainer = MCCNNTrainer(
            configuration.network,
            batch_size=batch_size,
            use_gpu=use_gpu,
            adapted_layers=adapted_layers,
            adapt_reference_channel=adapt_reference_channel,
