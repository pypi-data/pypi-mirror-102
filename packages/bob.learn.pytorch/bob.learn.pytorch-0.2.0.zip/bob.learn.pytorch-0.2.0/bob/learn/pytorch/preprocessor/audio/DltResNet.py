...
class DltResNetExtractor(Preprocessor):
  ...
  """
    ...
    """
        ...
        model_type = "emb"
        if use_res:
            self.network = resnet_arc(
                num_classes=num_classes, tp=model_type, bn_dim=bn_dim
            )
        else:
            self.network = drn_arc(
                num_classes=num_classes, tp=model_type, bn_dim=bn_dim
            )
            ...
            if not test:
                if use_res:
                    model_path = pkg_resources.resource_filename(
                        __name__, "data/rn34.pth.tar"
                    )
                    if rc["bob.learn.pytorch.extractor.audio.rn_modelpath"] != None:
                        model_path = rc[
                            "bob.learn.pytorch.extractor.audio.rn_modelpath"
                        ]
                else:
                    model_path = pkg_resources.resource_filename(
                        __name__, "data/drn34.pth.tar"
                    )
                    if rc["bob.learn.pytorch.extractor.audio.drn_modelpath"] != None:
                        model_path = rc[
                            "bob.learn.pytorch.extractor.audio.drn_modelpath"
                        ]
    ...
    def extract_embeddings(
        self, model, specs, L, batch_size=1, cuda_flag=0, min_length=30
    ):
    ...
    """
        ...
        for i in range(embedding_count):
            ...
            if (
                ...
                or i == embedding_count - 2
                or i == embedding_count - 1
            ):
                with torch.no_grad():
                    in_batch = numpy.array(batch)
                    if cuda_flag == 1:
                        input_var = torch.autograd.Variable(
                            torch.Tensor(in_batch).unsqueeze(1).cuda(non_blocking=True)
                        )
                    else:
                        input_var = torch.autograd.Variable(
                            torch.Tensor(in_batch).unsqueeze(1)
                        )
    ...
    """
        ...
        input_feat = c(data)
        features = self.extract_embeddings(
            self.network, input_feat, 0, batch_size=1, cuda_flag=self.cuda_flag
        )
        return features
