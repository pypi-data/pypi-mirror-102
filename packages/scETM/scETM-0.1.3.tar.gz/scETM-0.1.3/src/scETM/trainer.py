...
import pandas as pd
import time
from typing import Iterable, List, Mapping, Sequence, Union, Tuple
import psutil
import logging
...
from scETM.eval_utils import calculate_kbet, clustering, draw_embeddings, calculate_entropy_batch_mixing
    ...
    def train(self,
        ...
        while self.epoch < n_epochs:
            ...
            if self.epoch >= next_ckpt_epoch or self.epoch >= n_epochs:
                logger.info('=' * 10 + f'Epoch {self.epoch:.0f}' + '=' * 10)

                # log memory cost
                logger.info(repr(psutil.Process().memory_info()))
                # log current lr and kl_weight
                if self.lr_decay:
                    logger.info(f'{"lr":12s}: {self.lr}')
                logger.info(f'{"kl_weight":12s}: {self._get_kl_weight(self.epoch, n_epochs):12.4f}')
                ...
                if eval:
                    ...
                    if eval_result_log_path is not None:
                        with open(eval_result_log_path, 'a+') as f:
                            # ckpt_dir, epoch, test_nll, best_ari, k_bet, ebm, time, seed
                ...
                if next_ckpt_epoch and save_model_ckpt:
                    # checkpointing
        ...
        del recorder
        logger.info("Optimization Finished: %s" % self.ckpt_dir)
        if isinstance(sampler, MultithreadedCellSampler):
            sampler.join(0.1)

@log_arguments
def evaluate(adata: anndata.AnnData,
    ...
    Args:
        ...
        batch_col: a key in adata.obs to the batch column.
        draw: whether to save the embeddings plots.
        color_by: a list of adata.obs column keys to color the embeddings by.
        plot_fname: file name of the generated plot.
        plot_ftype: file type of the generated plot.
        plot_dir: directory to save the generated plot.
        min_dist: the min_dist argument in sc.tl.umap.
        spread: the spread argument in sc.tl.umap.
    ...
    if batch_col and adata.obs[batch_col].nunique() > 1:
        ebm = calculate_entropy_batch_mixing(adata, use_rep=embedding_key, batch_col=batch_col, n_neighbors=n_neighbors, calc_knn=False)
        logger.info(f'{embedding_key}_BE: {ebm:7.4f}')
        k_bet = calculate_kbet(adata, use_rep=embedding_key, batch_col=batch_col, n_neighbors=n_neighbors, calc_knn=False)[2]
        logger.info(f'{embedding_key}_kBET: {k_bet:7.4f}')
    else:
        ebm = k_bet = None
    ...
    if draw:
        ...
        draw_embeddings(adata=adata, color_by=color_by, min_dist=min_dist, spread=spread,
            ckpt_dir=plot_dir, fname=f'{plot_fname}.{plot_ftype}')
...
class _stats_recorder:
    ...
    def log_and_clear_record(self):
        for key, val in self.record.items():
            logger.info(f'{key:12s}: {np.mean(val):{self.fmt}}')
        self.record = defaultdict(list)
