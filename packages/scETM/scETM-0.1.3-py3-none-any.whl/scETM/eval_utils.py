...
def calculate_nearest_neighbors(
    ...
    if nsamples < n_neighbors:
        logger.warning(f"Warning: in calculate_nearest_neighbors, number of samples = {nsamples} < n_neighbors = {n_neighbors}!\n Set n_neighbors to {nsamples}.")
...
def calculate_kbet(
    ...
    assert adata.obs[batch_col].dtype.name == "category"
...
def draw_embeddings(adata: ad.AnnData,
    ...
    Args:
        ...
        spread: The effective scale of embedded points. In combination with
            ...
            points are.
        save: whether to save the plot to ckpt_dir.
        ckpt_dir: where to save the plot. Only used if save = True.
        fname: file name of the saved plot. Only used if save = True.
        show: whether to show the plot (plt.show()). Useful for visualizing
            the plot in an interactive environment.
        dpi: the dpi of the saved plot. Only used if save = True.
    """

    sc.tl.umap(adata, min_dist=min_dist, spread=spread)
    fig = sc.pl.umap(adata, color=color_by, show=show, return_fig=True)
    if save:
        fig.savefig(
