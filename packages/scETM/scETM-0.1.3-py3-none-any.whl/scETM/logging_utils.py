...
def log_arguments(f):
    ...
    def log_arguments_wrapper(*args, **kwargs):
        ...
        logger.info(f'{name}({args_kwargs_str})')
        result = f(*args, **kwargs)
        return result
    return log_arguments_wrapper

def initialize_logger(ckpt_dir=None, level=logging.INFO) -> None:
    """Initialize the scETM logger.
    ...
    logger_ = logging.getLogger('scETM')
    logger_.setLevel(level)
    if logger_.hasHandlers():
        for handler in logger_.handlers:
            if isinstance(handler, logging.FileHandler):
                logger_.warning(f'Reinitializing... The file handler {handler} will be closed.')
