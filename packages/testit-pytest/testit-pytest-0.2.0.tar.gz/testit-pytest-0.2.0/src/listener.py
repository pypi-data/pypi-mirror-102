...
class TestITListener(object):
    ...
    def attribute_collector(cls, item, nextitem, tree_setup_steps, tree_teardown_steps, index):
        if hasattr(item.function, 'test_externalID'):
            ...
            if hasattr(item, 'test_session_setup_steps'):
                tree_setup_steps['session'] = item.test_session_setup_steps
                tree_setup_steps['session_results'] = item.test_session_setup_results_steps
            elif 'session' not in tree_setup_steps:
                tree_setup_steps['session'] = []
                tree_setup_steps['session_results'] = []
            if hasattr(item, 'test_module_setup_steps'):
                tree_setup_steps['module'] = item.test_module_setup_steps
                tree_setup_steps['module_results'] = item.test_module_setup_results_steps
            elif 'module' not in tree_setup_steps:
                tree_setup_steps['module'] = []
                tree_setup_steps['module_results'] = []
            if hasattr(item, 'test_class_setup_steps'):
                tree_setup_steps['class'] = item.test_class_setup_steps
                tree_setup_steps['class_results'] = item.test_class_setup_results_steps
            elif 'class' not in tree_setup_steps:
                tree_setup_steps['class'] = []
                tree_setup_steps['class_results'] = []
            if hasattr(item, 'test_method_setup_steps'):
                tree_setup_steps['method'] = item.test_method_setup_steps
                tree_setup_steps['method_results'] = item.test_method_setup_results_steps
            elif 'method' not in tree_setup_steps:
            ...
            if hasattr(item, 'test_session_teardown_steps'):
                tree_teardown_steps['session'] = item.test_session_teardown_steps
                tree_teardown_steps['session_results'] = item.test_session_teardown_results_steps
            elif 'session' not in tree_teardown_steps:
                tree_teardown_steps['session'] = []
                tree_teardown_steps['session_results'] = []
            if hasattr(item, 'test_module_teardown_steps'):
                tree_teardown_steps['module'] = item.test_module_teardown_steps
                tree_teardown_steps['module_results'] = item.test_module_teardown_results_steps
            elif 'module' not in tree_teardown_steps:
                tree_teardown_steps['module'] = []
                tree_teardown_steps['module_results'] = []
            if hasattr(item, 'test_class_teardown_steps'):
                tree_teardown_steps['class'] = item.test_class_teardown_steps
                tree_teardown_steps['class_results'] = item.test_class_teardown_results_steps
            elif 'class' not in tree_teardown_steps:
                tree_teardown_steps['class'] = []
                tree_teardown_steps['class_results'] = []
            if hasattr(item, 'test_method_teardown_steps'):
                tree_teardown_steps['method'] = item.test_method_teardown_steps
                tree_teardown_steps['method_results'] = item.test_method_teardown_results_steps
            elif 'method' not in tree_teardown_steps:
                tree_teardown_steps['method'] = []
                tree_teardown_steps['method_results'] = []
            ...
            if hasattr(item.function, 'test_links'):
                if parametrize_id:
                    for ID in parametrize_id:
                        mark = item.own_markers[ID]
                        for link in item.function.test_links:
                            data['links'].append({})
                            if link['url'][link['url'].find('{') + 1:link['url'].rfind('}')] in item.own_markers[ID].args[0]:
                                data['links'][-1]['url'] = link['url'].split('{')[0] + mark.args[1][index][mark.args[0].split(', ').index(link['url'][link['url'].find('{') + 1:link['url'].rfind('}')])] + link['url'].split('}')[1]
                            else:
                                data['links'][-1]['url'] = link['url']
                            if link['title'] and link['title'][link['title'].find('{') + 1:link['title'].rfind('}')] in mark.args[0]:
                                data['links'][-1]['title'] = link['title'].split('{')[0] + mark.args[1][index][mark.args[0].split(', ').index(link['title'][link['title'].find('{') + 1:link['title'].rfind('}')])] + link['title'].split('}')[1]
                            else:
                                data['links'][-1]['title'] = link['title']
                            if link['type'] and link['type'][link['type'].find('{') + 1:link['type'].rfind('}')] in mark.args[0]:
                                data['links'][-1]['type'] = link['type'].split('{')[0] + mark.args[1][index][mark.args[0].split(', ').index(link['type'][link['type'].find('{') + 1:link['type'].rfind('}')])] + link['type'].split('}')[1]
                            else:
                                data['links'][-1]['type'] = link['type']
                            if link['description'] and link['description'][link['description'].find('{') + 1:link['description'].rfind('}')] in mark.args[0]:
                                data['links'][-1]['description'] = link['description'].split('{')[0] + mark.args[1][index][mark.args[0].split(', ').index(link['description'][link['description'].find('{') + 1:link['description'].rfind('}')])] + link['description'].split('}')[1]
                            else:
                                data['links'][-1]['description'] = link['description']
                else:
            ...
            if hasattr(item.function, 'test_labels'):
                if parametrize_id:
                    ...
                    if param_id and item.function.test_labels[0][1:-1] in item.name[item.name.find('[') + 1:item.name.rfind(']')].split('-')[param_id]:
                        for label in result:
            ...
            if hasattr(item.function, 'test_workItemsID'):
                if parametrize_id:
                    ...
                    if param_id and item.function.test_workItemsID[0][1:-1] in item.name[item.name.find('[') + 1:item.name.rfind(']')].split('-')[param_id]:
            ...
            return data, tree_setup_steps, tree_teardown_steps, index + 1 if nextitem and item.originalname == nextitem.originalname else 0
        return None, tree_setup_steps, tree_teardown_steps, 0
    ...
    def pytest_sessionfinish(self, session):
        ...
        for item_id in range(len(session.items)):
            data_item, tree_setup_steps, tree_teardown_steps, index = self.attribute_collector(session.items[item_id], session.items[item_id + 1] if item_id + 1 < len(session.items) else None, tree_setup_steps, tree_teardown_steps, index)
            if data_item:
