...
# 画出分类边界图
def plot_classifier(classifier, x, y, method='tsne', figsize=None, save_path=None):
    '''
    :param classifier: 分类器
    :param x: 数据
    :param y: 标签
    :param method: 数据预处理方法，['tsne','mds','pca','kernelpca']
    :param figsize: 图片大小，如果为None，则为默认大小
    :param save_path: 图片保存路径，如果为None，则不保存
    :return: None
    '''
    if x.shape[1] > 2:
        if method == 'tsne':
            ...
            x = MDS(2).fit_transform(x)
        elif method == 'kernelpca':
            x = KernelPCA(2).fit_transform(x)
        else:
    ...
    plt.pcolormesh(x1_values, x2_values, mesh_output, cmap=plt.cm.Paired)
...
# def plot_classifier(classifier, x, y, axes=[-1.5, 2.5, -1, 1.5]):
#     if x.shape[1]>2:
#         x = PCA(2).fit_transform(x)
#     classifier.fit(x, y)
#     plt.plot(x[y == 0][:, 0], x[y == 0][:, 1], 'bs')
...
def Friedman_test(x, alpha=0.05, ranked=False, use_f_distribution=False, verbose=False):
    ...
    if not ranked:  # 输入为得分
        ...
        if verbose:
            print('输入得分排名为：')
            for xx in x:
                print('\t'.join(xx.astype(str).tolist()))
    R = np.mean(x, axis=0)
