...
class ColorPickerSynchronizer:
    def __init__(self, left_color_pickers: List[ColorPicker], right_color_pickers: List[ColorPicker],
                 center_color_pickers: List[ColorPicker], all_color_pickers: List[ColorPicker]):
        self.left_color_pickers: List[ColorPicker] = left_color_pickers
        self.right_color_pickers: List[ColorPicker] = right_color_pickers
        self.center_color_pickers: List[ColorPicker] = center_color_pickers
        self.all_color_pickers: List[ColorPicker] = all_color_pickers

    def sync_left_color_pickers(self, current_color):
        for color_picker in self.left_color_pickers:
            color_picker.value = current_color.new

    def sync_right_color_pickers(self, current_color):
        for color_picker in self.right_color_pickers:
            color_picker.value = current_color.new

    def sync_center_color_pickers(self, current_color):
        for color_picker in self.center_color_pickers:
            color_picker.value = current_color.new

    def sync_all_color_pickers(self, current_color):
        for color_picker in self.all_color_pickers:
            color_picker.value = current_color.new
...
class SkeletonVisualizer:
    ...
    def __display_video_player(self, fps: int, frames: np.ndarray) -> None:
    ...
    def __display_color_changer(self):
        ...
        if len(skeleton_color_changer_tabs) > len(self.skeletons):
            color_changer_widget.set_title(len(self.skeletons), 'Pred skeleton colors')
            color_changer_widget.set_title(len(self.skeletons) + 1, 'GT skeleton colors')
    ...
    def __create_color_changer_tab(self, skeleton: DrawableSkeleton) ->\
        ...
        left_color_picker: ColorPicker = ColorPicker(
            description='Left color:', value=self.__get_as_html_color(skeleton.get_left_objects()[0].color))
        right_color_picker: ColorPicker = ColorPicker(
            description='Right color:', value=self.__get_as_html_color(skeleton.get_right_objects()[0].color))
        center_color_picker: ColorPicker = ColorPicker(
            description='Center color:', value=self.__get_as_html_color(skeleton.get_center_objects()[0].color))
    ...
    def __create_color_synchronizer_tab(
        ...
        color_picker_synchronizer: ColorPickerSynchronizer = ColorPickerSynchronizer(
            left_color_pickers=[color_picker[0] for color_picker in color_pickers],
            right_color_pickers=[color_picker[1] for color_picker in color_pickers],
            center_color_pickers=[color_picker[2] for color_picker in color_pickers],
            all_color_pickers=[color_picker[3] for color_picker in color_pickers]
        )
