"""
 * $Id: 1.0+10 utils.py fbf3e027e655 2021-04-19 od $
 *
...
class Client(object):
    ...
    def __init__(self, data: Dict = None, metainfo: Dict = None,
                 ...
                 descr: str = '', http_status:int = 0, conf: Dict = {}):
    ...
    @staticmethod
    def __get(url: str, section: str = PARAMETER, conf: Dict = {}) -> "Client":
    ...
    @staticmethod
    def __post0(url: str, multipart: Dict, headers: Dict, conf: Dict = {}):
        ...
        while retry > 0:
            try:
                response = requests.post(url, files=multipart, allow_redirects=allow_redirects, headers=headers,
                                         timeout=to)
                response.raise_for_status()
                response_json = response.json()
                break
    ...
    @staticmethod
    def get_catalog(url: str, conf: Dict = {}) -> List['Client']:
        """ Get the list of services fron an endpoint"""

        response, http_status =  Client.__get0(url, conf)
        if http_status != 200:
            raise Exception("Connection Error " + http_status)

        response_json = response.json()
        return [Client(metainfo=service) for service in response_json]

    @staticmethod
    def get_capabilities(c, conf: Dict = {}) -> "Client":
    ...
    def execute(self, url: str = None, sync: bool = True, files: List[str] = None, headers: Dict = None) -> 'Client':
        ...
        response_json, http_status = Client.__post0(url, files=multipart, headers=headers, conf=self.conf)

        # print(response_json)
        # async handling
        data = {} if not self.RESULT in response_json else response_json[self.RESULT]
        return Client(data, parent=self, metainfo=response_json[self.METAINFO], http_status=http_status)
    ...
    def execute_async(self, url: str = None, files: List[str] = None,
        ...
        callback(r, progress)
        while r.get_status() == Client.STATUS_RUNNING and r.get_http_status() == 200:
            sleep(np)
