"""
 * $Id: 1.0+9 utils.py ee3c2ac46588 2021-04-14 od $
 *
...
class Client(object):
    ...
    def __get(url: str, section: str = PARAMETER) -> "Client":
        ...
        response = requests.get(url, allow_redirects=True, timeout=4.0)
    ...
    def download_data_files(self, files: List[str] = None, dir: str = ".") -> None:
        ...
        for name in files:
            ...
            val = self.get_data_value(name)
            print("  --> " + name)
            r = requests.get(val, allow_redirects=True, timeout=4.0)
