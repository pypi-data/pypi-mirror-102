...
try:
    ...
    from __init__ import __serverName__, __version__
    ...
    from data2es.__init__ import __serverName__, __version__
    ...
    def loadConfig(self, filename):
        try:
            ...
            for i in inputs:
                ...
                if provider == 'mysql':
                    self.data_provider = MysqlProvider(host=host, port=port, user=user, password=password, db=db, sql=sql,
                                                       canSaveId=self.record_last_run, idFile=self.last_run_metadata_path, idName=self.tracking_column)
            ...
            while(True):
                ...
                if len(datas) > 0:
                    ...
                    for url in self.finished_urls:
                        WebhookClient.sendFinished(
                            url, start_id, self.data_provider.lastId)
                    return
        except Exception as e:
    ...
    def run(self):
        if self.trigger == 'schedule':
            ...
            server = WebhookServer(
                self.transmit_data, self.webhook_token, int(self.webhook_port))
            server.run()
            pass
        else:
...
def run(isDaemon=False):
    ...
    parser = argparse.ArgumentParser(
        prog='%s' % __serverName__, usage='%s [-c]' % __serverName__)
    parser.add_argument("-c", "--config", help="Specify configuration file path.",
                        metavar='CONFIGFILE_PATH', required=True)
