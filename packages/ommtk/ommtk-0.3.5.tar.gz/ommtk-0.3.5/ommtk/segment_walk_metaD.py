...
class SegmentMappingSimHREMD(SegmentMappingSim):
    ...
    def __init__(self, max_production_time=200 * unit.nanosecond, swap_freq = 2 * unit.nanosecond,
                 convergence_window=2 * unit.nanosecond, dG_displacement_criteria = 1, min_coverage_criteria=.25,
                 **kwargs):
    ...
    def cycle_replicas(self, segment_coords):
        ...
        while continue_simulating:
            ...
            if log_p_accept<=0 * unit.kilojoules_per_mole \
                ...
                print('successfully swapped coordinates')
                accepted +=1
                attempted +=1
            else:
                print('failed swap attempt')
                attempted+=1
            ...
            # dG_trace is written every .01 * ns / step_length steps
            window_steps = int(int(self.convergence_window / self.step_length)/int(.01 * unit.nanosecond/self.step_length))
            # print('window steps, len(dG-trace)',window_steps,len(dG_trace))
            if len(dG_trace)>window_steps:
                dG_std = np.std(dG_trace[:-window_steps])
                print('dG_std over the last {}: {}'.format(self.convergence_window,dG_std))
                m_ave = moving_average(dG_trace[:-window_steps], np.max([1,int(len(dG_trace[:-window_steps])/10)]))
            ...
            if coverage > self.min_coverage_criteria and \
                ...
                print('convergence detected! Coverage: '
                      '{}, dG_std: {}, dG_displacement: {}'.format(coverage,dG_std,dG_displacement))
