...
Example
----------
>>> from Method_bounceAnalyzer import bounceAnalyzer
...
class bounceAnalyzer:
    ...
    keepBounceDetermineSize:
        int, 最近是否发生跳变的状态记录的尺寸, 不可大于self.bounceStatusRecorderSize, optional, default self.bounceStatusRecorderSize
    keepBounceDetermineThreshold:
        float, 当监测到窗口（keepBounceDetermineSize）内发生了跳变的情况的百分比, 触发条件是：
        发生个数>=keepBounceDetermineThreshold × bounceStatusRecorderSize, optional, default 0.4
    ...
    def updateAndDetermine(self, _sampleCache):
        ...
        :param _sampleCache: float, 新传入的数据
        """
        # 更新缓存记录
