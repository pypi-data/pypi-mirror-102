...
def make_fake_api_client(path, overrides=None):
    """
    ...
    if overrides is None:
        overrides = {}
    api_client = docker.APIClient(version="19.03.12")
    mock_attrs = {
        ...
        "nodes.return_value": fake_api.get_fake_nodes(path)[1],
        "tasks.return_value": fake_api.get_fake_tasks(path)[1],
        "services.return_value": fake_api.get_fake_services(path)[1],
...
def make_fake_client(path, overrides=None):
    """
    Returns a Client with a fake APIClient.
    """
    client = docker.DockerClient()
    client.api = make_fake_api_client(overrides, path)
    return client
