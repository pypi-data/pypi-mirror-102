...
class Transformers:
    ...
    def __init__(
        ...
        try:
            ...
            model_list = "\n".join(format_backend(MODEL))
            msg = (
                f"Couldn't find model type {model_type} in backend "
                f"Choose model inside list {model_list}"
            )
    ...
    @classmethod
    def list_backend(cls) -> None:
        """Get all possible backend for the model"""
        log.info(
            "Use ESM backend in this list \n\n%s", "\n".join(format_backend(MODEL))
        )
    ...
    def _predict(
        ...
        result = []
        total_iter = len(sequences) // batch_size
        for batch in tqdm(
            self._generate_chunks(batch_tokens, batch_size), total=total_iter
        ):
            with torch.no_grad():
                output = self.model(
...
def format_backend(backend_list: List[str]) -> List[str]:
    ...
    FORMAT_BACKEND = ["  *" + " " * 3 + model for model in backend_list]

    return FORMAT_BACKEND
