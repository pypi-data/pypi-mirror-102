    ...
    def update_grid_range_slider(self, *args):
        ...
        if hasattr(self, "positive_class_map_filter"):
            if not self.empty_metadata:
                ...
                num_filtered_items = self.class_map_subset_filter.sum()
        ...
        # Reduce max limit depending on whether or not items marked
        #   with certain attributes (review, deleted, completed) are to
        #   be shown or not
        if self.hide_review:
            num_filtered_items -= self.num_under_review

        if self.hide_deleted:
            num_filtered_items -= self.num_deleted

        if self.hide_completed:
            num_filtered_items -= self.num_completed

        max_limit = min(len(self), num_filtered_items)

        self.grid_range_slider.value = (min_value, max_value)

        # HACK: Sometimes somewhere this fixes stuff lol...
