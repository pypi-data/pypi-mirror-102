...
class StringTimeFormat(TimeFormat):
    type_str = "StringTimeFormat"

    """
    TimeFormat subclass that allows to inject a custom string date pattern.
    The pattern defined in this class follows the same rules
    in the ToDate transformation
    """

    def __init__(self, pattern: str):
        TimeFormat.__init__(self, StringTimeFormat.type_str)
        self.pattern = pattern

    def __repr__(self):
        return f"StringTimeFormat(pattern: {self.pattern})"
    ...
    @staticmethod
    def _from_payload(json: dict) -> Union[DtlError, "StringTimeFormat"]:
        pattern = json.get("pattern")
        if pattern is None:
            return _property_not_found("pattern", json)
        return StringTimeFormat(pattern)
...
def _time_format_from_payload(json: dict) -> Union[DtlError, "TimeFormat"]:
    ...
    elif type_str == StringTimeFormat.type_str:
        time_format = StringTimeFormat._from_payload(json)
    else:
