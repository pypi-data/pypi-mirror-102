...
if "zoom.us" in pyperclip.paste():
    ...
    if ":" in link_time:
        starting_time = link_time.split(":")
    else:
        starting_time = link_time.split()
    if len(starting_time[1]) == 1:
        link_time = starting_time[0] + ":" + "0" + starting_time[1]
    if len(starting_time[0]) == 1:
        link_time = "0" + link_time
    elif len(starting_time[0]) != 1 and len(starting_time[1]) != 1:
        link_time = starting_time[0] + ":" + starting_time[1]
        print("in else")

    print(starting_time)
    print(link_time)

    while str(datetime.now())[11:-10] != link_time:
        time.sleep(1)
