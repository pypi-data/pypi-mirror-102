...
@click.option(
    "-c", "--count", default=None, help="number of conversations to list"
)
@click.option("-s", "--start", default=None, help="start conversations at")
def grab_conversations(
    ...
    convo_data = wrapper.get_conversations(
        user_id=user_id, start=start, size=count
    )
