...
class SessionReader:
    def __init__(self, filepath):
        ...
        success = SessionReader.__ReadMagic(self.file)
        assert success
    ...
    @staticmethod
    def Unpack(filepath, visitor:SessionReaderVisitor):
        datFilepath = pl.Path(filepath).with_suffix(".dat")
        ...
        sessionMetadata = SessionReader.UnpackMetadata(datFilepath)
        numChannels = sessionMetadata["deviceInfo"]["numChannels"]

        with open(recFilepath,'rb') as file:
            if not SessionReader.__ReadMagic(file):
