...
bool Generation::check_json() {
    ...
    auto& edge_schame = json_obj[schema::json_edge];
    std::unordered_set<std::string> edge_set;
    for (auto& one_edge : edge_schame) {
        // label
        ...
        if (one_edge.find(schema::json_comm) != one_edge.end()) {
            ...
            if (comm_obj.find(schema::json_comm_overlap) != comm_obj.end()) {
                if (!comm_obj[schema::json_comm_overlap].is_number()) {
                    std::cerr << "[Generation::check_json] Lack Error: JSON['" << schema::json_edge << "'][i]['" << schema::json_comm << "'] (overlap number)." << std::endl;
                    ans = false;
                }
            }
        }
...
void Generation::generate_plan() {
    ...
    for (auto& one_edge : edge_schema) {
        ...
        // std::string i_dist_type = in_dist[schema::json_dist_type];
