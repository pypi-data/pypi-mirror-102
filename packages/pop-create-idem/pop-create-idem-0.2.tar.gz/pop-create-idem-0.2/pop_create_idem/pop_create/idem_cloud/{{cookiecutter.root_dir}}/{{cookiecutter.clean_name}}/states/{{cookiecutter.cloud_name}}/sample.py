...
async def present(hub, ctx, name: str, **kwargs):
    ...
    .. code-block:: sls
        ...
        my_state_name:
          {{cookiecutter.cloud_name}}.sample.present:
            name: item_name
            acct_profile: default
            my_kwarg: my_value
    """
    try:
        if name in await hub.exec.{{cookiecutter.cloud_name}}.sample.list(ctx):
            comment = f"{name} is already exists"
            my_instance, before = await hub.exec.{{cookiecutter.cloud_name}}.sample.get(ctx, name)
        else:
            before = {}
            if ctx.test:
                ...
                hub.exec.{{cookiecutter.cloud_name}}.create(ctx, name, **kwargs)
                comment = f"Created {name} with kwargs: {pprint.pformat(kwargs)}"
            # Now that it exists, retrieve it
            my_instance = await hub.exec.{{cookiecutter.cloud_name}}.sample.get(ctx, name)
        ...
        after = await hub.exec.{{cookiecutter.cloud_name}}.sample.get(ctx, name)
...
async def absent(hub, ctx, name: str, **kwargs):
    ...
    .. code-block:: sls
        ...
        my_state_name:
          {{cookiecutter.cloud_name}}.sample.absent:
            name: item_name
            acct_profile: default
            my_kwarg: my_value
    """
    try:
        if name not in await hub.exec.{{cookiecutter.cloud_name}}.sample.list(ctx):
            comment = f"{name} is already absent"
            before = {}
        else:
            before = await hub.exec.{{cookiecutter.cloud_name}}.sample.get(ctx, name)
            if ctx.test:
                # If `idem state` was run with the `--test` flag, then don't actually make any changes
                comment = (
                ...
                hub.exec.{{cookiecutter.cloud_name}}.delete(ctx, name, **kwargs)
