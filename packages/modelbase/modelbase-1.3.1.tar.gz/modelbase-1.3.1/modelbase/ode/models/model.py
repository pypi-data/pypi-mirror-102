...
class Model(
    ...
    def _element_difference(self, other, attribute):
        self_collection = getattr(self, attribute)
        other_collection = getattr(other, attribute)
        difference: list[str] = sorted(set(other_collection).difference(self_collection))
        if not difference:
            return None
    ...
    def _element_intersection(self, other, attribute):
        self_collection = getattr(self, attribute)
        other_collection = getattr(other, attribute)
    ...
    def add(
        ...
        if parameters is not None:
            self.add_parameters(parameters, meta_info=None)
    ...
    def update(
        ...
        if compounds is not None:
            self.update_compounds(compounds)
        if parameters is not None:
    ...
    def remove(
        ...
        if stoichiometries is not None:
            self.remove_stoichiometries(stoichiometries)
        if functions is not None:
