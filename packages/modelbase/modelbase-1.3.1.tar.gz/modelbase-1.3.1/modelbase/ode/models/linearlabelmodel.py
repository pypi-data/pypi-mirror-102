...
class LinearLabelModel(RateMixin, StoichiometricMixin, CompoundMixin, BaseModel):
    ...
    def __add__(self, other):
        m = self.copy()
        m.add_compounds(other.compounds)
        m.add_rates(other.rates)
        m.add_stoichiometries(other.stoichiometries)
        return m

    def __iadd__(self, other):
        self.add_compounds(other.compounds)
        self.add_rates(other.rates)
        self.add_stoichiometries(other.stoichiometries)

    def __sub__(self, other):
        m = self.copy()
        m.remove_compounds(set(self.compounds).intersection(other.compounds))
        m.remove_rates(set(self.rates).intersection(other.rates))
        m.remove_stoichiometries(set(self.stoichiometries).intersection(other.stoichiometries))

    def __isub__(self, other):
        self.remove_compounds(set(self.compounds).intersection(other.compounds))
        self.remove_rates(set(self.rates).intersection(other.rates))
        self.remove_stoichiometries(set(self.stoichiometries).intersection(other.stoichiometries))
    ...
    def _get_fluxes(self, *, fcd: dict, v_ss: dict, external_label=1):
    ...
    def _get_rhs(self, t, y_labels):
        ...
        fluxes = self._get_fluxes(fcd=y_labels, v_ss=self._v_ss, external_label=self._external_label)
