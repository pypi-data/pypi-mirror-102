...
def add(directory, alias, description):
    ...
    wdescription(f"{texpath()}/{alias}", '' if description == None else description)
...
def create(alias, name, directory):
    ...
    if not exists(f"{abspath(expanduser(os.path.join(directory, name)))}"):
        os.mkdir(f"{abspath(expanduser(os.path.join(directory, name)))}")
