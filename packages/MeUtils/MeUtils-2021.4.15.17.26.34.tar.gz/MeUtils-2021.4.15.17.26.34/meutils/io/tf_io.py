...
def to_hdfs(
        df, file, batch_size=None,  # 拆成很多小文件
    ...
    if batch_size:
        def _writer(args):
            i, df = args
            writer(df, f"{cache_dir}/{file}-{i}")

        dfs = df_split(df, batch_size=batch_size)
        enumerate(dfs) | xProcessPoolExecutor(_writer, 8) | xlist # 多进程加速
    else:
        writer(df, f"{cache_dir}/{name}")

    cp(cache_dir, dir, with_success=with_success)  # 多线程cp
