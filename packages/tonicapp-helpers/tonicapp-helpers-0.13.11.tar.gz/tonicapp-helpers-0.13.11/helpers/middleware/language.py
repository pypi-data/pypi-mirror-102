...
class LanguageMiddleware(object):
    ...
    def __call__(self, request):
        ...
        if '"count":0' in language_response.content:
            logger.warning(f'No keys were found in language service with this params: {params}.')
            return response

        dict_response = language_response.content.split('"results":')[1]
        dict_response = literal_eval(dict_response[:-2])
        for result in dict_response:
            content_string = content_string.replace(result['key'], result['value'])

        response.content = content_string[2:-1]
        return response
