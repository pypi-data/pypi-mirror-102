...
class mkrunner(object):
    ...
    def __run_step_request(self, step: TStep) -> StepData:
        ...
        body = "params"
        if not parsed_request_dict["params"]:
            body = "data"


        code_function = "data_sign"
        if "function" in parsed_request_dict[body]:
            code_function = parsed_request_dict[body]["function"]
            del parsed_request_dict[body]["function"]

        if "api_sign" not in parsed_request_dict[body] and "code_secret" in parsed_request_dict[body]:

            import json

            app_secret = parsed_request_dict[body]["code_secret"]

            del parsed_request_dict[body]["code_secret"]

            api_sign = self.__project_meta.functions[code_function](json.dumps(parsed_request_dict[body]), app_secret)
            parsed_request_dict[body]["api_sign"] = api_sign
