...
__TEMPLATE__ = jinja2.Template(
    """# NOTE: Generated By mkrunner v{{ version }}
# FROM: {{ testcase_path }}
...
@allure.parent_suite("{{ suites_dir_1 }}")


{% if suite_dir_2 %}
@allure.suite("{{ suite_dir_2 }}")
@allure.sub_suite("{{ suites_dir_name }}")
{% elif suites_dir_name %}
@allure.suite("{{ suites_dir_name }}")
{% endif %}
...
def convert_testcase_path(testcase_abs_path: Text) -> Tuple[Text, Text]:
    ...
    for index, i in enumerate(testcase_abs_path.split("/")):
        if i in ["test_cases", "test_suites","api"]:
            pytest_dir_name = testcase_abs_path.split("/")[index + 1]
            data = testcase_abs_path.split("/" + i)[1]
            abs_psth = data.strip(data.split("/")[-1])
            abs_psth = abs_psth.strip("/")
    ...
    testcase_python_abs_path = project_meta.RootDir + "/pytest_file/cases/" + abs_psth + "/" + f"{file_name}_test.py"
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    if dir_path:
        ...
        #zsw testsuites
        # testcase_python_abs_path = project_meta.RootDir + "/case_files/" + pytest_dir_name + "/" + f"{file_name}"
        # testcase_python_abs_path = project_meta.RootDir + "/pytest_file/" + os.path.basename(dir_path) + f"/{file_name}"
        testcase_python_abs_path = project_meta.RootDir + "/pytest_file/suites/" + dir_path.split("test_suites")[1] + f"/{file_name}"
    ...
    if dir_path:
        #zsw tests_suits 根据目录分级
        suites_dir_name = os.path.basename(dir_path)
        suites_dir_data = dir_path.split("test_suites/")[1].split("/")
        suites_dir_1 = suites_dir_data[0]
        if len(suites_dir_data)> 2:
            if suites_dir_data[0] == suites_dir_1 and suites_dir_data[2] == suites_dir_name:
                suite_dir_2 = suites_dir_data[1] +"_BySuite"
            else:
                suite_dir_2 = None
        else:
            suite_dir_2 = None
    else:
        if len(testcase_path.split("/"))>3:
            suites_dir_1 = testcase_path.split("/")[1]
            suites_dir_name = testcase_path.split("/")[2] +"_ByCase"
            suite_dir_2 = None
        else:
            suites_dir_1 = testcase_path.split("/")[1]
            suite_dir_2 = None
            suites_dir_name = None


    data = {
        "suites_dir_1": suites_dir_1,
        "suite_dir_2": suite_dir_2,
        "suites_dir_name": suites_dir_name,
