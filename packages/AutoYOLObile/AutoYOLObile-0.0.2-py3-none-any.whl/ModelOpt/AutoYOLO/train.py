...
wdir = upperpath+'weights' + os.sep  # weights dir
last = wdir + 'last.pt'
best = wdir + 'best.pt'
...
def train(hyp):
    ...
    if opt.admm:
        ...
        for i in range(opt.rho_num):
            ...
            ADMM = admm.ADMM(model, file_name=upperpath+"prune_config/" + opt.config_file + ".yaml" , rho=current_rho)
            ...
            torch.save(
                ...
                "{}model_pruned/yolov4_{}_{}_{}.pt".format(upperpath,
    ...
    if opt.masked_retrain:
        ADMM = admm.ADMM(model, file_name=upperpath+"prune_config/" + opt.config_file + ".yaml", rho=initial_rho)
        if not opt.resume:
            # possible weights are '*.pt', 'yolov3-spp.pt', 'yolov3-tiny.pt' etc.
            print("\n>_ Loading file: {}model_pruned/yolov4_{}_{}_{}.pt".format(upperpath, initial_rho * 10 ** (opt.rho_num - 1), opt.config_file, opt.sparsity_type))
            chkpt = torch.load("{}model_pruned/yolov4_{}_{}_{}.pt".format(upperpath, initial_rho * 10 ** (opt.rho_num - 1), opt.config_file, opt.sparsity_type), map_location=device)
            # chkpt = torch.load(weights, map_location=device)
            # load model
            try:
                # chkpt['model'] = {k: v for k, v in chkpt['model'].items() if model.state_dict()[k].numel() == v.numel()}
