...
requests_get = sync_to_async(requests.get)
requests_post = sync_to_async(requests.post)
requests_put = sync_to_async(requests.put)
requests_delete = sync_to_async(requests.delete)
...
async def check_pipe(pipe, host, token, populate, cl, limit=10):
    ...
    await new_pipe(checker_pipe, host, token, replace=True, check=False, populate=populate, cl=cl, skip_tokens=True)
    headers = {'Authorization': f'Bearer {token}'}
...
async def new_pipe(p, host, token, replace=False, check=True, populate=False, wait_populate=False, cl=None, skip_tokens=False, tag='', skip_table_checks=False):  # noqa: C901
    headers = {'Authorization': f'Bearer {token}'}
    exists = False
    materialized = False
    ...
    if r.status_code == 200:
        ...
        if replace:
            ...
            if check and not populate:
                await check_pipe(p, host, token, populate, cl)
            ...
            if populate:
                ...
                if wait_populate:
                    with click.progressbar(label="Populating ", length=100, show_eta=False, show_percent=True, fill_char=click.style("█", fg="green")) as progress_bar:
                        ...
                        res = await cl.wait_for_job(job_id, status_callback=progressbar_cb)

                        if res['status'] != 'done':
                            click.echo(FeedbackManager.error_while_populating(error=res['error']))

        r = await requests_put(f"{host}/v0/pipes/{p['name']}/endpoint?{urlencode(cli_params)}", headers=headers, data=r.json()['id'])
        if r.status_code != 200:
            raise Exception(FeedbackManager.error_creating_node(pipe=p['name'], error=r.json()['error']))
    ...
    if p['tokens'] and not skip_tokens:
        ...
        for tk in p['tokens']:
            ...
            t = await cl.get_token_by_name(token_name)
            if t:
                break
        if not t:
            token_name = tag + tk['token_name']
...
async def new_ds(ds, host, token, replace=False, check=True, skip_table_checks=False):
    # print(f"""curl -X POST -H "Authorization: Bearer $TOKEN" {host}/v0/pipes/?name={name}&sql=select+1""")
    headers = {'Authorization': f'Bearer {token}'}
    name = ds['params']['name']
    ...
    r = await requests_get(f"{host}/v0/datasources/{name}?{urlencode(cli_params)}", headers=headers)

    if r.status_code == 200:
        if replace:
            # removed replacing by default. When a datasource is removed data is
            # removed and all the references needs to be updated
            click.echo(FeedbackManager.warning_datasource_already_exists(datasource=name))
            if os.getenv('TB_I_KNOW_WHAT_I_AM_DOING') and click.prompt('Please type data source name to be replaced') == name:  # TODO move to CLI
                r = await requests_delete(f"{host}/v0/datasources/{name}?{urlencode(cli_params)}", headers=headers)
                if r.status_code != 204:
                    raise Exception(FeedbackManager.error_removing_datasource(datasource=name))
            return
        else:
            raise click.ClickException(FeedbackManager.error_datasource_already_exists(datasource=name))

    params = ds['params']
    params['skip_table_checks'] = 'true' if skip_table_checks else 'false'
    params['cli_version'] = VERSION

    r = await requests_post(f"{host}/v0/datasources?{urlencode(params)}", headers=headers, data='')
    ...
    if r.status_code != 200:
        ...
        raise Exception(FeedbackManager.error_creating_datasource(error=error_message))


async def exec_file(r, host, token, override, check, debug, populate, wait_populate, cl, tag, skip_table_checks=False):
    if debug:
...
async def folder_push(token, host, tag='', filenames=None, dry_run=False, check=False, push_deps=False,  # noqa: C901
                      ...
                      skip_table_checks=False, tables_helper=None):
    cl = TinyB(token, host)
    datasources = await cl.datasources()
    pipes = await cl.pipes()
                ...
                if not dry_run:
                    if name not in remote_resource_names or resource_versions.get(name, '') != latest_datasource_versions.get(name, '') or force:
                        if name not in resource_versions:
                            ...
                            click.echo(FeedbackManager.info_creating_resource(name=name, version=version))
                        else:
                        ...
                        try:
                            await exec_file(to_run[name], host, token, force, check, debug, populate, wait, cl, tag, skip_table_checks)
                            click.echo(FeedbackManager.success_create(name=name))
                        except Exception as e:
            ...
            for group in toposort(dep_map):
                for f in group:
                    ...
                    if name not in processed:
                        if name in to_run:
                            await check_data(cl, to_run[name], debug, folder)
                            processed.add(name)
            for f in to_run:
                if f not in processed:
                    await check_data(cl, to_run[f], debug, folder)
        else:
