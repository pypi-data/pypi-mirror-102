...
class Project(PathExAttMap):
    ...
    def _get_table_from_samples(self, index):
        ...
        for sample in self.samples:
            sd = sample.to_dict()
            ser = pd.Series({k: v for (k, v) in list(sd.items()) if not k.startswith("_")})
    ...
    def __str__(self):
        ...
        if CONFIG_KEY not in self:
            return msg
        msg = "{}\nSections: {}".format(
            msg, ", ".join([s for s in list(self[CONFIG_KEY].keys())])
        )
        if (
            ...
            msg = "{}\nAmendments: {}".format(
                msg, ", ".join(list(self[CONFIG_KEY][PROJ_MODS_KEY][AMENDMENTS_KEY].keys()))
            )
        if self.amendments:
