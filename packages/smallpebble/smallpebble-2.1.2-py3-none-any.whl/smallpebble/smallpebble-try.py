...
def cross_entropy(y_pred: Variable, y_true, axis=-1) -> Variable:
    """Cross entropy.
    Args:
...
def np_add_at(a, indices, b):
    ...
    if np.array_library.__name__ == "numpy":
        np.add.at(a, indices, b)
    elif np.array_library.__name__ == "cupy":
        np.scatter_add(a, indices, b)
    else:
        raise ValueError("Expected np.__name__ to be `numpy` or `cupy`.")
