...
from cannerflow.exceptions import handle_exception
from cannerflow.request import CannerRequest
from cannerflow.dto import SqlQueryResult, SqlQueryStatus
from cannerflow.adapters.query_reader import ParquetQueryReader, ApiFetchQueryReader

# from cannerflow.query_reader import
...
class Query(object):
    ...
    def row_count(self) -> Optional[int]:
        """
        row count will shows the correct value until `status` shows `FINISHED`.
        If `status` not be `FINISHED`, the row count return `None`.
        """
        return None if self._status != SqlQueryStatus.FINISHED else self._row_count
    ...
    def ___create_sql_query(
        ...
        if result.status == SqlQueryStatus.FAILED:
            print(result)
            raise Exception("Execute sql failed")
        return result
