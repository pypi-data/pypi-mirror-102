...
def matrix_rank(evals, N, tol=None):
    """Matrix rank, computed from the eigenvalues directly
...
def matrix_entropy(evals, N):
    """Matrix entropy of real, computed using the eiganvalues evals and matrix rank"""

    rank = matrix_rank(evals, N) #np.linalg.matrix_rank(W)
