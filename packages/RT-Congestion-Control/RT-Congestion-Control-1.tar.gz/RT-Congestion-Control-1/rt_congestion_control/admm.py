...
class Admm():
    ...
    def create_agents(self, time_series):
        ...
        for k in self.net.flex_data.index:
            ...
            self.agents.append(Agent(name=None,
                                     ...
                                     rho=self.rho,
                                     partners="all",
                                     time_series=self.net.data[self.net.data["dataid"]==ID].sort_values(by='localminute')))
    ...
    def run(self, warm_start, primal_residual = 1e-1, dual_residual = 1e-1, max_it=500, scale_residual=True, SO_strategy = True, plot =False):
        """Run the simulation. \n
        Input :
            - warm_start (bool) : perform a warm start. If True the simulation start with the ADMM converged.
            - primal_residual (float, 1e-5) : Primal residual stopping criteria #BUG
            - dual_residual (float, 1e-5) : Dual residual stopping criteria (set to 0 to force max_it) #BUG
            ...
            - scale_residual (bool, True) : If True, the primal and dual residual are scale by : sum(Power^2) #BUG
    ...
    def ADMM(self, memory, warm_start=False, max_it=None):
        ...
        for t in range(n_step):
            ...
            # # parallel loop using Joblib #BUG
            # # (This is not what slows down the code, osqp minimisation is very fast)
            # Parallel(n_jobs=-2, backend="threading")(delayed(self.agents[k].update_trade_and_price)(P[:,k], grid_cost) for k in range(len(self.agents)))
            # trade, min_p_mw, max_p_mw, polycost = zip(*Parallel(n_jobs=-2, backend="threading")(delayed(self.agents[k].run)(warm_start) for k in range(len(self.agents))))
            # trade = np.asarray(trade)
            # min_p_mw = np.asarray(min_p_mw)
            # max_p_mw = np.asarray(max_p_mw)
            # polycost = np.asarray(polycost)
