"""
   ...
   1.txt2images,文字转逐字图象函数。
   2.makegif，合帧，制作gif图像函数。
"""
__author__ = '李兴球'
__date__ = '2021/4/16'
...
def txt2images(string,width=480,height=360,background=None,margin=68,
               fontsize=18,fgcolor=(10,0,100,255),bgcolor=(0,0,0,0)):
    
    """文本转逐字图像,string:字符串，width:宽度，height:高度,background:背景图或颜色,
    ...
    fnt = ImageFont.truetype("msyh.ttf",fontsize)
...
def txt2image(txt,fontfile="simkai.ttf",fontsize=64,
              color=(255,255,255,255),stroke=None):
    """
...
def main():  
    ...
    frames = txt2images(string,fgcolor='yellow',bgcolor=(128,0,128))
    g = Sprite(cv,frames)
    while True:
       g.nextshape()
       cv.update()
