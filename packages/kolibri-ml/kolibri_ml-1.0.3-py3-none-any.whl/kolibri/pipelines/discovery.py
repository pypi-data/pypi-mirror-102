...
This module contains functions to load component and pipelines
annotations, as well as to configure how Component finds the
...
_PRIMITIVES_PATHS = [
    os.path.join(os.getcwd(), 'kolibri_components'),
    os.path.join(sys.prefix, 'kolibri_components'),
]
...
def add_components_path(path):
    """Add a new path to look for components.

    The new path will be inserted in the first place of the list,
    so any component found in this new folder will take precedence
    over any other component with the same name that existed in the
    system before.
    ...
    if added:
        LOGGER.debug('New components path added: %s', path)
...
def add_pipelines_path(path):
    ...
    The new path will be inserted in the first place of the list,
    so any component found in this new folder will take precedence
    over any other pipelines with the same name that existed in the
    system before.
...
def _load_entry_points(entry_point_name, entry_point_group='mlblocks'):
    ...
    An example of such an entry point would be::
        ...
        entry_points = {
            'mlblocks': [
                'components=some_module:SOME_VARIABLE'
            ]
        }
...
def get_components_paths():
    """Get the list of folders where components will be looked for.

    This list will include the values of all the entry points named ``components``
    published under the entry point group ``mlblocks``.

    Also, for backwards compatibility reasons, the paths from the entry points
    named ``jsons_path`` published under the ``kolibri_components`` group will also
    be included.
    ...
    Returns:
        list:
            The list of folders.
    """
    paths = _load_entry_points('components') + _load_entry_points('jsons_path', 'kolibri_components')
...
def _load(name, paths):
    ...
    Args:
        ...
        paths (list):
            list of paths where the components will be looked for.
...
def load_component(name):
    """Locate and load the component JSON annotation.

    All the component paths will be scanned to find a JSON file with the given name,
    and as soon as a JSON with the given name is found it is returned.
    ...
    Raises:
        ValueError:
            A ``ValueError`` will be raised if the component cannot be found.
    """
    component = _load(name, get_components_paths())
...
def load_pipeline(name):
    """Locate and load the pipelines JSON annotation.

    All the pipelines paths will be scanned to find a JSON file with the given name,
    and as soon as a JSON with the given name is found it is returned.
    ...
    Raises:
        ValueError:
            A ``ValueError`` will be raised if the pipelines cannot be found.
    """
    pipeline = _load(name, get_pipelines_paths())
    if pipeline is None:
        raise ValueError("Unknown pipelines: {}".format(name))
...
def find_components(pattern='', filters=None):
    """Find components by name and filters.

    If a patter is given, only the components whose name matches
    the pattern will be returned.

    If filters are given, they should be a dictionary containing key/value
    filters that will have to be matched within the component annotation
    ...
    Args:
        pattern (str):
            Regular expression to match agains the component names.
        filters (dict):
    ...
    Returns:
        list:
            Names of the matching components.
    """
    filters = filters or dict()
    return _find_annotations(get_components_paths(), load_component, pattern, filters)
...
def find_pipelines(pattern='', filters=None):
    ...
    If filters are given, they should be a dictionary containing key/value
    filters that will have to be matched within the pipelines annotation
    for it to be included in the results.
    ...
    Args:
        pattern (str):
            Regular expression to match agains the pipelines names.
        filters (dict):
            Dictionary containing the filters to apply over the matchin
            pipelines.
