...
class DownloaderBase(object):
    def __init__(self,  download_dir=DATA_PATH):
        ...
        try:
            self.local_db = dict(pickle.load(open(os.path.join(DATA_PATH, ".index"), "rb", encoding='utf-8')))
        except IOError as e:
            print(e)
            self.local_db = {}
    ...
    def download(self, file_path):
        ...
        if server_file_entry is not None:
            ...
            if server_file_checksum != local_file_checksum or not os.path.exists(os.path.join(self.download_dir, file_path)):
                ...
                pickle.dump(list(self.local_db.items()), open(os.path.join(DATA_PATH, ".index"), "wb", encoding='utf-8'))
