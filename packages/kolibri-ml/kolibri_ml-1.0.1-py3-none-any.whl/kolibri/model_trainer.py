...
class ModelTrainer(object):
    ...
    config={
        ...
        'max-time-for-optimization': 14400,
        'random-state': 42,
        'ml-task': None,
        'explain-level': 2,
...
#        self.config
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if self.config['evaluate-performance']==True and self.config['save-evaluation-output']==True:
            self.performance_data=pd.DataFrame(np.column_stack((X, y, self.pipeline.estimator.validatation_data)))
            self.performance_data.columns=['content', 'class', 'prediction', 'probability']
    ...
    def objective(self, X, y):
        objective=PipelineObjective(X, y, self.pipeline.get_hyperparameters(), None, eval_metric=self.config['opt-metric-name'],n_jobs=-1, random_state=42)
        return objective
    ...
    def optimize(self, X, y):
        try:
            ...
            self.hyperparameters = optimizer.optimize(
                ...
                learner_params=self.pipeline.get_hyperparameters()
            )
