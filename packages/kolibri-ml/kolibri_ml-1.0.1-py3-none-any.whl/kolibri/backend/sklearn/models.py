...
from kolibri.backend.sklearn.meta.ecoc_model import EcocModel

from kdmt.file import read_json_file
from kdmt.objects import class_from_module_path
models=read_json_file("/Users/mohamedmentis/Dropbox/My Mac (MacBook-Pro.local)/Documents/Mentis/Development/kolibri-ml/kolibri/backend/sklearn/models.json")

n_estimators = 10

def get_model(model_name, weights=None, voting_type='soft'):
    if isinstance(model_name, list):
        models_ = [models.get(model, None) for model in model_name]
        if weights is None:
...
def get_model_(model_name):
    ...
    model_data=models.get(model_name, None)
    if model_data:
        params=model_data["parameters"]
        defaults={p:params[p]["value"] for p in params}
        return model_data
