...
import pickle
with open("train_data_2021-04-18 142321.pickle","rb") as fr:
...
def show_acc():
    ...
    for i, v in enumerate(result["test_acc_list"]):
        plt.text(i, v + 0.0005, float(v),  # 좌표 (x축 = v, y축 = y[0]..y[1], 표시 = y[0]..y[1])
                 fontsize=9,
                 color='blue',
...
def show_img_predict():
    net = MultiLayerNet()
    net.load_model("weight_idol_f.npz")
    ...
    img_list = []
    predict_list = []
    for fname in test_iu_fnames[:15]:
    ...
    plot_grid(np.asnumpy(img_list), predict_list, 3, 5)
    plt.show()
