...
class Oracle:
    ...
    def push_many(self, table, dataset, batch_size=50000):
        ...
        def process_data(values):
            return [tuple([v if not (pandas.isna(v) or v is np.nan) else None for v in line]) for line in values]

        data = Parallel(n_jobs=multiprocessing.cpu_count())(delayed(process_data)(dataset.values[i:i + batch_size]) for i in
                                                     tqdm(range(0, len(dataset), step=batch_size), total=len(dataset)))
