...
class DataModule():
    ...
    def _split_pair_name(self, pair_name):
        """
            ...
            return: ("BINANCE.ETHUSDT", "BINANCE", "ETHUSDT", "ETH") # 返回一个元组
        """
        # 1. 核验pair名称
        ...
        exchange_name, symbol = pair_name.split(".")
        asset_name = symbol[:-4]
        return (pair_name, exchange_name, symbol, asset_name) # ("BINANCE.ETHUSDT", "BINANCE", "ETHUSDT", "ETH") 返回一个元组
    ...
    def add_pairs(self, pair_lst=["BINANCE.BTCUSDT", "BINANCE.ETHUSDT"]):
        """
            ...
            self.pairs.update(
                {
                    ...
                    pair_name : {
                        ...
                        "symbol" : symbol, # ETHUSDT; (用大写来作为symbol的一般通用形式) (pair_name是大小写不敏感的, 但是symbol最好还是大写)
...
class ShowModlue():
    ...
    5.【最大允许跌幅】: 【{(usdt_wallet_balance-maintMargin)/newest_notional_marketValue-0.02:.2%} 】
    -----------------------
    ...
    def _check_pair_pos(self, pair_name):
        ...
        # 核验余额
            # i. 该币种的现货余额
        symbol_account = self.oms.accounts.get(f"{exchange_name}.{asset_name}") # 这里是ETH不是ETHUSDT, 所以把后面4个字符删掉
        if symbol_account:
    ...
    def req_mytrade(self, pair_name="BINANCE.ETHUSDT", type="spot", df_type="3", start_time="today", end_time="now", direction=None):
        """
            function:
        ...
        time_df = df.drop_duplicates("orderId")[["orderId", "time"]].set_index("orderId").copy() # 索引列是orderId, 值列是时间 (只有一列)
    ...
    def show_trade(self, pair_name="BINANCE.ETHUSDT", offset="OPEN", start_time="today", end_time="now"):
    ...
    def show_trades(self, pair_lst=["BINACE.ETHUSDT"], offset="OPEN", start_time="today", end_time="now"):
        self.show_dict = {}
        for pair_name in pair_lst:
            df = self.Show.show_trade(pair_name=pair_name, offset=offset, start_time=start_time, end_time=end_time)
            self.show_dict.update({pair_name:df})
...
class TradeModule():
    ...
    def get_pair_volume(self, pair_name="BINANCE.ETHUSDT"):
        """
    ...
    def two_sides_send_order(self, pair_name, offset, volume):
        """
            ...
            usage:
                eg1:
                    # 双边下单
                    sc.two_sides_send_order(pair_name="BINANCE.ETHUSDT", offset="OPEN", volume=1)
    ...
    def send_order(
            self, pair_name="BINANCE.ETHUSDT", type="spot", offset="OPEN", direction="BUY",
        ...
        """
            ...
            usage:
                eg1:
                    # 单边下单
                    sc.send_order(pair_name="BINANCE.ETHUSDT", type="futures", offset="OPEN", direction="BUY", volume=1)
    ...
    def fast_buy_sell(self, pair_name="BINANCE.ETHUSDT", multiplier=1, time_sleep=10):
        """
            # '秒抢费率'
...
class Robot():
    def __init__(
        ...
        pair_name="BINANCE.ETHUSDT", offset="OPEN", multiplier=1,
                    ...
                    if offset == "OPEN":
                        ...
                        if (spread_rate_open >= self.target_spread_rate) and (self.trading_count >= 1):
                            ...
                            d = {
                                ...
                                "target_spread_rate" : self.target_spread_rate, "capture_spread_rate": spread_rate_open,
                                }
                        ...
                        if (spread_rate_close <= self.target_spread_rate) and (self.trading_count >= 1):
                            ...
                            d = {
                                "offset" : offset, "time": datetime.now(),
                                "target_spread_rate" : self.target_spread_rate, "capture_spread_rate": spread_rate_close,
                                }
                            self.Data.record_dict[pair_name].append(d)
                            self.trading_count -= 1 # 成功交易一笔后, 就把trading_count递减1, 减为0的时候停止交易.. (可以自行在控制台开启)
                            time.sleep(1) # 拉开两笔交易的间隔时间, 避免0.1s的时间太短而重复发送下单信号, 导致滑点过大...
...
def init_main_engine(engines=[CtaEngine], gateway_settings_lst=[binance_settings_lsh, binances_settings_lsh]):
    """
...
def init_spread_client(engines=[], gateway_settings_lst=[binance_settings_lsh, binances_settings_lsh]):
    """
    ...
    """
    # 1. 初始化main_engine
    main_engine = init_main_engine(engines=engines, gateway_settings_lst=gateway_settings_lst)

    # 2. 实例化sc对象
    sc = SpreadClient(main_engine=main_engine)
    sc.user = gateway_settings_lst[0].get("user")
...
# lzc = init_spread_client(gateway_settings_lst=[binance_settings_lzc, binances_settings_lzc])
# sc = lzc

lsh = init_spread_client(gateway_settings_lst=[binance_settings_lsh, binances_settings_lsh])
sc = lsh

# wyh = init_spread_client(gateway_settings_lst=[binance_settings_wyh, binances_settings_wyh])
# sc = wyh
#
#
...
#         engines=[CtaEngine], gateway_settings_lst=[binance_settings_lsh, binances_settings_lsh]
#     )
