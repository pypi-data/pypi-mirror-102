...
class BinanceRestApi(myRequest):
    ...
    def __init__(self, api_type="spot", settings=settings_lsh):
    ...
    def req_historicalTrades(self, binance_symbol="BTCUSDT"): # 使用币安官网的命名
        """
            functions:
    ...
    def req_markprice(self, binance_symbol="BTCUSDT", is_obj=False):
        """
            function:
    ...
    def req_hist_funding_rate(self, binance_symbol="BTCUSDT", start_time=None, end_time=None, limit=1000, is_obj=False):
        """
            function:
                获取每个币种的历史'资金费率'
            args:
        ...
        verify = False
        query_dict = {
            "symbol" : binance_symbol.upper(), # 币安要求大写
            "limit" : limit,
        }
        #     # 币安接口中的startTime必须是 'ms'为单位的int型 (eg: 1234567890123)
        # start_time = start_time if start_time else int(get_timestamp("today").timestamp()*1000)  # 如果没有传入start_time, 则用今天0点的时间戳传入
    ...
    # @timer # 计时器: 用于检测该请求的耗时
    def send_order(
            self, binance_symbol="BTCUSDT", offset="OPEN", direction="BUY", order_type="LIMIT",
...
bra_spot = BinanceRestApi(api_type="spot", settings=settings_lsh)
bra_futures = BinanceRestApi(api_type="futures", settings=settings_lsh)
