...
class Requests:
    ...
    async def get_turn_ratio(self, fromroad: str = None, toroad: str = None) -> dict:
        params = {}
    ...
    async def get_data_point_indicator(self, indicator: str, fromdatetime: datetime,
        ...
        if datatypeeids is not None:
            params[RequestParam.datatype] = self.__encode_list_params(datatypeeids)
        return await self._get_data_point_indicator(indicator, fromdatetime, todatetime, datapointeids, period,
                                                    sampling, window, RequestPath.datapoint, params)
    ...
    async def get_data_point_imputed_indicator(self, indicator: str, fromdatetime: datetime,
        ...
        if datatypeeids is not None:
            params[RequestParam.datatype] = self.__encode_list_params(datatypeeids)
        return await self._get_data_point_indicator(indicator, fromdatetime, todatetime, datapointeids, period,
                                                    sampling, window, SEPARATOR.join((RequestPath.datapoint,
                                                                                      RequestPath.imputed)), params)

    async def _get_data_point_indicator(self, indicator: str, fromdatetime: datetime, todatetime: datetime,
                                        locationeids: list, period: str, sampling: str, window: str, applyto: str,
    ...
    async def get_road_indicator(self, indicator: str, fromdatetime: datetime, todatetime: datetime,
        ...
        return await self.__get_data(fromdatetime, todatetime, roadeids, period, sampling, window,
                                     SEPARATOR.join((RequestPath.road, RequestPath.indicator, indicator)), {})
