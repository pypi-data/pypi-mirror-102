...
class YahtzeeSinglePlayerV2:
    ...
    def _make_reward(self, action):
        ...
        if action >= 13 and action <= 18:
            try:
                # Add 35 to indicate the potential bonus to be earned from scoring top half > 62
                return (self.ytz.c_player.t_scorecard[action - 13][0] / \
                    self.ytz.c_player.t_scorecard[action - 13][1].count(1)) + 35
        ...
        # Chance
            ...
            return 12 if sum(self.ytz.c_player.dice) <= 12 else 3
