...
class HTTP3Connection:
    ...
    def _request (self, method, urls, data = None, headers = {}, allow_push = True, repeat = 1):
        issingle = False
        if isinstance (urls, str):
            issingle = True
            urls = [urls]
        rs = [ Request (method.upper (), self.endpoint + url, data, headers, allow_push) for url in urls ]
        self.request (rs)
        return rs [0].response if issingle else [ r.response for r in rs ]

    def get (self, urls, headers = {}, allow_push = True, repeat = 1):
        return self._request ('GET', urls, None, headers, allow_push, repeat)

    def post (self, urls, data, headers = {}, allow_push = True, repeat = 1):
        return self._request ('POST', urls, data, headers, allow_push, repeat)
...
class Session (h2client.Session):
    def __init__ (self, endpoint):
        ...
        self.s = HTTP3Connection ('https://' + parts.netloc)

    def get (self, urls, headers = {}):
        return self.s.get (urls, headers = headers)

    def post (self, urls, data, headers = {}):
        return self.s.post (urls, data, headers)
