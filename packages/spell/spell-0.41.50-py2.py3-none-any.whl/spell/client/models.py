...
class ModelsService:
    ...
    def get(self, name, version=None):
        ...
        Returns:
            A :py:class:`SpellModel` object if ``version`` is ``None``. Otherwise, returns a
            :py:class:`ModelVersion` object instead.
    ...
    def new(self, name, resource, version=None, files=[], description=None):
        ...
        Parameters:
            name (str): model name
            resource (str): path to a top-level resource, e.g. ``runs/168``.
            files (list of str, optional): specific files or folders within the run which should
            be included in the model artifact. By default, this parameter is left blank, and all
            files written to disk by the run are included in the model artifact. If this parameter
            is set, all files not included in ``files`` are excluded. Each entry should be a valid
    ...
    def archive(self, name, version=None):
        """Archive a model or model version.
...
class Model(SpellModel):
    ...
    def __init__(self, api, model):
        self._api = api
        self.__set_from_api_run_object(api, model)

    def __set_from_api_run_object(self, api, model):
...
class ModelVersion(SpellModel):
    ...
    Attributes:
        ...
        resource (str): path to the top-level resource associated with this mode, e.g.
        ``runs/168``
        files (list of str, optional): specific files or folders within the run included in the
        model artifact. If set to empty list `[]`, all files from the resource are included in the
        model, otherwise, only ones included in `files` are.
    """
    ...
    def download(self, dest=None, quiet=False):
        ...
        Raises:
            :py:class:`~spell.api.exceptions.ClientException` if an error occurs.
        """
        # necessary because the CLI and Python API handle logging differently.
        def echo(msg):
            print(msg)
        ...
        if dest is None:
            dest = "./"
        self._api.download_model_version(self.model_version, dest, echo=echo, quiet=quiet)
