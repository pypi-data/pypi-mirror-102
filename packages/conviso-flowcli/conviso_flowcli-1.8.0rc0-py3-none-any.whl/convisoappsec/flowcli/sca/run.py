...
def perform_command(
    ...
    try:
        ...
        click.secho('\U0001F4AC Preparing Environment', bold=True, blink=True)
        scabox = Box(
            token=token, 
        ...
        click.secho('\U0001F4AC Starting SCA', bold=True, blink=True)
        scabox.run()
        click.secho('\U0001F4AC Processing Results', bold=True, blink=True)
        if send_to_flow:
        ...
        click.secho('\U00002705 SCA Scan Finished', bold=True, blink=True)
