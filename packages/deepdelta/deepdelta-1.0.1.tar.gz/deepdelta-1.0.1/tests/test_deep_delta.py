...
dict_list = [
    {'departmentId': 1, 'department': 'Sales',
        'employeeId': 123, 'name': 'Ali', 'gender': 'male'},
    {'departmentId': 2, 'department': 'IT',
        'employeeId': 235, 'name': 'Tom', 'gender': 'male'},
    {'departmentId': 1, 'department': 'Sales', 'employeeId': 135,
        'name': 'Blinder', 'gender': 'female'},
    {'departmentId': 1, 'department': 'Sales',
        'employeeId': 178, 'name': 'Clair', 'gender': 'female'},
    {'departmentId': 3, 'department': 'Unknown',
        'employeeId': 326, 'name': 'Bruce', 'gender': 'male'},
    {'departmentId': 3, 'department': 'Support',
        'employeeId': 377, 'name': 'Ben', 'gender': 'male'},
    {'departmentId': 1, 'department': 'Sales',
        'employeeId': 138, 'name': 'Frank', 'gender': 'male'},
]
...
dict_list2 = [
    ...
    {'departmentId': 3, 'department': 'support',
        'employeeId': 377, 'name': 'Ben', 'gender': 'male'},
    {'departmentId': 2, 'department': 'IT',
        'employeeId': 235, 'name': 'Tommy', 'gender': 'male'},
    {'departmentId': 1, 'department': 'Sales',
        'employeeId': 123, 'name': 'Ali', 'gender': 'male'},
    {'departmentId': 1, 'department': 'Sales', 'employeeId': 135,
        'name': 'Blinder', 'gender': 'female'},
    {'departmentId': 1, 'department': 'sales',
        'employeeId': 138, 'name': 'Frank', 'gender': 'male'},
    {'departmentId': 3, 'department': 'Manage',
        'employeeId': 327, 'name': 'Harry', 'gender': 'male'},
]
...
def test_compare_with_options_case_ignored():
    result = default_comparator.compare(
        dict_list, dict_list2, DeltaConfig.CaseIgnored | DeltaConfig.IdAsKey)
    pprint(result)
    assert len(result) == 3
...
shopping_list = [
    {'name': 'bread', 'expired': datetime(
        2010, 10, 7), 'price': 2.98, 'brand': 'SoftLake', 'amount': 2},
    {'name': 'milk', 'expired': datetime(
        2010, 10, 8), 'price': 4, 'brand': 'DiaryFarm', 'amount': 1},
]

basket = [
    {'Name': 'BREAD', 'Expired': '2010-10-07', 'price': '2.99',
        'brand': 'SOFTLake', 'amount': '2.000003'},
    {'Name': 'MILK', 'Expired': '2010 Oct 9',
        'price': '3.98', 'brand': 'DiaryFarm', 'amount': '1'},
]


def test_compare_with_default_settings():
    result = DeepDelta.compare(
        shopping_list, basket, DeltaConfig.KeyCaseIgnored)
    pprint(result)
...
def test_compare_with_exclusive_key_strings():
    result = DeepDelta.compare(
        shopping_list, basket, DeltaConfig.KeyCaseIgnored, None, None, 'BRAND', 'PRICE')
    pprint(result)
...
def test_compare_with_named_comparator():
    ...
    result = DeepDelta.compare(
        shopping_list, basket, DeltaConfig.KeyCaseIgnored, None, named)
    pprint(result)
    assert set(result['0'].keys()) == {'name'} and set(
        result['1'].keys()) == {'name', 'expired', 'price'}
    result = DeepDelta.compare(
        basket, shopping_list, DeltaConfig.KeyCaseIgnored, None, named)
    pprint(result)
...
def test_missing_as_none():
    ...
    delta = DeepDelta.compare(
        d1, d2, DEFAULT_DELTA_CONFIG ^ DeltaConfig.MissingAsNone)
    pprint(delta)
    assert len(delta) == 1
...
def test_convert_to_compare():
    ...
    delta = DeepDelta.convert_to_compare(
        products1, products2, comparators, lambda name, case_ignored: 'no' in name)
...
def test_named_comparator():
    ...
    def compare_p(l, r, p): return DeepDelta.convert_to_compare(
