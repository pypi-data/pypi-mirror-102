...
category_dict = {'animal_distances': {'icon': os.path.join(simba_assets_path, 'animal_distances.jpg')},
             'intruder_movement': {'icon': os.path.join(simba_assets_path, 'intruder_movement.jpg')},
             'intruder_shape': {'icon': os.path.join(simba_assets_path, 'intruder_shape.jpg')},
             'resident_intruder_movement': {'icon': os.path.join(simba_assets_path, 'resident_intruder_movement.jpg')},
             'resident_intruder_shape': {'icon': os.path.join(simba_assets_path, 'resident_intruder_shape.jpg')},
             'resident_movement': {'icon': os.path.join(simba_assets_path, 'resident_movement.jpg')},
             'resident_shape': {'icon': os.path.join(simba_assets_path, 'resident_shape.jpg')}}
...
img = 255 * np.ones([1280,1280, 3], dtype=np.uint8)
baseline_scale_img = cv2.imread(scale_dict['baseline_scale'])
baseline_scale_top_left = (100,500)
...
baseline_scale_middle = ((int(400 + baseline_scale_img.shape[1] / 2)), 85)
...
for value, tick in enumerate(side_scale_y_tick_cords):
    ...
    #black_pixels = np.where((icon_img[:, :, 0] == 0) &(icon_img[:, :, 1] == 0) & (icon_img[:, :, 2] == 0))
    #icon_img[black_pixels] = [255, 255, 255]
    icon_top_left = (tick[0] - int(icon_img.shape[0] / 2), tick[1])
    icon_bottom_right = (icon_top_left[0] + icon_img.shape[0], tick[1] + icon_img.shape[1])
...
color_bar_middle = ((int(270 + baseline_scale_img.shape[1] / 2)), color_bar_bottom_right[0] + 50)
