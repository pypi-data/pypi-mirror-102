...
class BaseBackend(object):
    ...
    def _to_bytes(self, data, key='', expired=None, noc=0, ncalls=0):
        ...
        **Parameters**
            ...
            :param key: If the key is provided (is not empty) and
                        `pycryptodome` is installed, cached data
                        will be encrypted. It is empty by default.
            :param expired: exact date when the cache will be expired;
                            `None` by default
        ...
        if not can_encrypt and key:
            warnings.warn("Pycrypto not installed. "
    ...
    def get_data(self, data_key, key=''):
        ...
        if extracted != -1:
            try:
                data, expired, noc, ncalls = self._from_bytes(extracted,
                                                              key=key)
                flag = True
            except ValueError:
                return None, flag
...
class BaseLimitedBackend(BaseBackend):
    ...
    ==========
        ...
        :param algorithm : string, cache-clearing algorithm; the cache is
                           clearing when it is almost exhausted; available
                           values are `lfu` and `mfu`: stands for the least
                           frequently used and most frequently used caching
                           algorithms respectively; default value is {}.
    ...
    def get_data(self, data_key, *args, **kwargs):
        result = super(BaseLimitedBackend, self).get_data(data_key,
                                                          *args, **kwargs)
        if result[-1]:
