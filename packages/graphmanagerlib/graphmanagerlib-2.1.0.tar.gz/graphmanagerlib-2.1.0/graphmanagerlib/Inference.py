...
from Data_for_gcn import load_train_test_split, load_predictionData
...
save_modelPath = os.path.join("C:\\AsianGCN\\saved_model\\saved_model.pt")
model = torch.load(save_modelPath)
...
def predictionsUnique(saved_model_folder, prediction_dataset_folder, document_to_predict,labels_tab):
    predictions_data = load_predictionData(save_fd=prediction_dataset_folder)

    model = torch.load(saved_model_folder)
    ...
    G, df, individual_data, img = make_info(document_to_predict)
