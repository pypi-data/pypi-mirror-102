...
def initialize_user():
    ...
    if not os.path.exists(USER_JSON_PATH):
        ...
        if 'CI' in os.environ and os.environ['CI'] is not None:
            ...
            # Then, we check if the user is on the server. If they are are on the server (and signed in
            # with their email, then we can get their email as their JUPYTERHUB_USER. If they are not on 
            # the server, then we set their email as empty for now
            if is_on_kuberentes_mito():
                user_email = os.getenv('JUPYTERHUB_USER')
                # If they don't have an @ in their email, its not
                # an email, doh
