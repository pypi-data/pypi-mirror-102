...
class DetectDuplicatesTask(Task):
    ...
    def _run(self):
        ...
        errors_to_hide = set(self.ingest_config.detect_duplicates_override)
        for dup_check_fn, fn_errors in self.checks.items():
            errors_to_detect = fn_errors - errors_to_hide
            if len(errors_to_detect) > 1:
                dup_check_fn(errors_to_detect)
            elif len(errors_to_detect) == 1:
                dup_check_fn()
    ...
    def _check_new_session_container_study_instance_uids(self):
        ...
        for item in self.db.find_all_items_with_uid(
            ...
            if not item.item_id in item_ids:
                self._add_error(item.item_id, errors.StudyInstanceUIDExists)
    ...
    def _check_new_acquisition_container_study_instance_uids(self):
        ...
        def add_error(item):
            self.error_item_ids.append(item.item_id)
            if not item.item_id in item_ids:
                self._add_error(item.item_id, errors.SeriesInstanceUIDExists)
                item_ids.add(item.item_id)
                self.error_container_ids.add(item.acquisition_container_id)
    ...
    def _add_error(self, item, error_type) -> None:
        ...
        self.insert_errors.push(
            T.Error(item_id=item_id, code=error_type.code).dict(exclude_none=True)
        )
