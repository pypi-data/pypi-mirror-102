...
class TestClient:
    ...
    async def test_get_balance(self, test_init):
        ...
        rune_balance = await self.client.get_balance(contract_address=ETH_RUNE.ticker)
        rune_balance_address = await self.client.get_balance(address=self.test_address,
                                                       contract_address=ETH_RUNE.ticker)
    ...
    async def test_transfer_rune(self, test_init):
        ...
        receipt = await self.client.transfer(dest_addr, 1, contract_address=ETH_RUNE.ticker)
    ...
    async def test_read_contract(self, test_init):
        assert await self.client.read_contract(ETH_RUNE.ticker, "balanceOf", self.client.get_address()) / 10**18 \
               == await self.client.get_balance(contract_address=ETH_RUNE.ticker)
    ...
    #     old_balance = self.client.get_balance(contract_address=ETH_RUNE.ticker)
    #     func_to_call = "giveMeRUNE"
    #     self.client.set_gas_strategy("fast")
    #     tx_receipt = self.client.write_contract(ETH_RUNE.ticker, func_to_call, erc20=False)
    #     new_balance = self.client.get_balance(contract_address=ETH_RUNE.ticker)
