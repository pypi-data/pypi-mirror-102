...
def _upload_firmware_stm32h735():
    ...
    firmware_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'firmware/stm32h735g-dk-demo.elf')
    command = '%s --connect port=SWD ap=0 -w %s -hardRst' % (programmer_path, firmware_path)
    log.debug(command)
