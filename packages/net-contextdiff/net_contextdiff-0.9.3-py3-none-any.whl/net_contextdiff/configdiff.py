...
class DiffConvert:
    ...
    as the 'cfg' and 'diff' parameters to the action methods.
    ...
    'cfg' and 'diff' parameters to the action methods; this variable
    defaults to None and must be defined by a child class
...
class DiffConfig:
    ...
    def convert(self, from_cfg, to_cfg):
        ...
        for cvtr in self._cvtrs:
            ...
            remove_matches = cvtr.full_matches(remove_cfg_full)
            update_matches = cvtr.full_matches(update_cfg)
            ...
            for match in all_matches:
                ...
                if match in remove_matches:
                    ...
                    if cvtr.context_removed(remove_cfg, match):
                        if self._debug_convert:
                            print(
                    ...
                    if self._debug_convert >= 2:
                        print(
                            "-> from configuration:",
                            yaml.dump(cvtr_cfg, default_flow_style=False),
                            "-> remove configuration:",
                            yaml.dump(cvtr_diff, default_flow_style=False),
                            sep="\n", file=sys.stderr)
