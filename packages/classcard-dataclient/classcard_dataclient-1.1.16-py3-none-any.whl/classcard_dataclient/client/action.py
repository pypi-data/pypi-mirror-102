...
class DataClient(object):
    ...
    def create_teacher(teachers):
        ...
        for index, t in enumerate(teachers):
            teacher_sso_data = t.sso_data
            if t.uid:
                teacher_sso_data.pop("password")
                code, rlt = update_teacher_req(t.uid, teacher_sso_data, school_id=t.school)
            elif t.number in teacher_map:
                teacher_sso_data.pop("password")
                code, rlt = update_teacher_req(teacher_map[t.number], teacher_sso_data, school_id=t.school)
            else:
