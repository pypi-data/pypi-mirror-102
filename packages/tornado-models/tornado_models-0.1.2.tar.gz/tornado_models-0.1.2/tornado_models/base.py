...
def authenticated_async(f):
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        if self.current_user is None:
            ...
            self.write_json(dict(success=False, errorCode=401, errorMessage='登录超时, 请重新登录', showType=4, data=''))
        elif self.current_user is False:
            self.set_status(403, '禁止访问')
            self.write_json(dict(success=False, errorCode=403, errorMessage='Forbidden', showType=9, data=''))
        else:
...
class BaseRequestHandler(RedisMixin, SessionMixin, RequestHandler):
    ...
    def forbidden(self):
        self.set_status(403, '禁止访问')
        ret_data = dict(success=False, errorCode=403, errorMessage='Forbidden', showType=9, data='')
