...
class saveModel():
    ...
    def saveNames(self):
        ...
        groups_grp.create_dataset("groups", data=np.array(self.groups_names, dtype='S80'))        

        # Save views names
        views_grp = self.hdf5.create_group("views")
        views_grp.create_dataset("views", data=np.array(self.views_names, dtype='S80'))

        # Save samples names
        samples_grp = self.hdf5.create_group("samples")
        for g in range(len(self.groups_names)):
            samples_grp.create_dataset(self.groups_names[g], data=np.array(self.samples_names[g], dtype='S80'))

        # Save feature names
        features_grp = self.hdf5.create_group("features")
        for m in range(len(self.data)):
            features_grp.create_dataset(self.views_names[m], data=np.array(self.features_names[m], dtype='S80'))

        # Save covariate names
        if not self.covariates_names is None:
            covariates_grp = self.hdf5.create_group("covariates")
            covariates_grp.create_dataset("covariates", data=np.array(self.covariates_names, dtype='S80'))
