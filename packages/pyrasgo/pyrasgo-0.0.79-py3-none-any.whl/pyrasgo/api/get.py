...
class Get(Connection):
    ...
    def feature_set_yml(self, id: int) -> FeatureSet:
        ...
        try:
            response = self._get(f"/feature-sets/{id}", api_version=1).json()
            return api.v1.FeatureSetYML(**response)
        except:
            raise APIError(f"FeatureSet {id} does not exist or this API key does not have access.")
    ...
    @track_usage
    def source_table(self, table_name: str, record_limit: int, database: Optional[str] = None, schema: Optional[str] = None) -> pd.DataFrame:
        """
