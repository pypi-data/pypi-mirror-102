...
import dslibrary.utils.connect
from .utils.dbconn import sql_enquote_id
from .utils.file_utils import write_stream_with_read_on_close
...
class DSLibrary(object):
    ...
    def __init__(self, spec: dict=None):
        ...
        """
        self._spec = spec or {}
        jsonschema.validate(self._spec, LOCAL_SPECS_SCHEMA)
    ...
    def open_resource(self, resource_name: str, mode: str='rb', **kwargs) -> io.RawIOBase:
        ...
        """
        open_args = dict(kwargs)
        if "://" in resource_name:
            # an external (untracked) resource
            path = resource_name
        elif resource_name.startswith("./"):
            # a local file - the "./" bypasses lookup of named inputs
            path = resource_name
        else:
            # a named resource
            resource_spec = (self._spec.get("inputs" if 'r' in mode else "outputs") or {}).get(resource_name)
            if resource_spec:
                # options come from mapping, and override supplied options
                open_args.update(resource_spec)
                path = open_args.pop("uri", resource_name)
            else:
                # if there is no mapping we look for a local file
                path = resource_name
        ...
        if use_mlflow:
            ...
            return write_stream_with_read_on_close(w_mode=mode, r_mode='r', on_close=lambda fh: mlflow.log_artifact(fh.name, resource_name))
        ...
        return self._opener(path, mode, **open_args)
    ...
    def get_sql_connection(self, resource_name: str, for_write: bool=False, **kwargs):
        ...
        resource_spec = (self._spec.get("outputs" if for_write else "inputs") or {}).get(resource_name)
        kwargs.update(resource_spec)
        # connect to data
        uri = kwargs.pop("uri", None)
        return dslibrary.utils.connect.connect_to_database(path=uri, for_write=for_write, **kwargs)

    def get_nosql_connection(self, resource_name: str, **kwargs):
        """
        ...
        :param resource_name:  Which database to access.
            This can be the name of an input or output resource, or a URI referencing an external data source.
        """
        # TODO design and implement this
        raise DSLibraryException("not yet implemented")

    def get_timeseries_db_connection(self, resource_name: str, for_write: bool=False, **kwargs):
        """
        Get a standard interface to a time series database.
        :param resource_name:   Name of an input or output data connection.  The supplied specifications will map this
                                to a target database.
        :param for_write:       True to enable write access.
        :param kwargs:      Additional connection configuration, credentials, etc.
        :return:    A connection, as a yet-to-be-determined interface that supports a range of popular time series
                    databases.
        """
        # TODO design and implement this
        raise DSLibraryException("not yet implemented")
    ...
    def log_param(self, key: str, value):
        ...
        self.write_resource(PARAMS_ALIAS, df, append=True, uri="params.json", format="json")
    ...
    def log_metric(self, metric_name: str, metric_value: float, step: int=0) -> None:
        ...
        df = pandas.DataFrame(data=[row])
        self.write_resource(METRICS_ALIAS, df, append=True, uri="metrics.json", format="json")
    ...
    def get_metrics(self, metric_name: str = None, uri: str = None, time_range: (list, tuple) = None, limit: int = None) -> typing.Iterable:
        """
        Retrieve the values of previously stored metrics.  Implementations might limit which metrics are returned
        ...
        for row in df.itertuples(index=False, name=None):
            metric = DSLibrary.Metric(run_id=row.run_id, uri=row.uri, user=row.user, time=row.time, name=row.name, value=row.value, step=row.step)
            yield metric
    ...
    def iter_scoring_requests(self, callback=None):
        """
        Iterates through multiple requests.  Each iterated value is {} defining the request.

        :param callback:       Called while waiting for requests, and can raise an exception to stop the iteration.
        """
        return []
    ...
    def _write_content(self, opener, resource_name: str, content, append: bool=False, **kwargs):
        ...
        resource_spec = kwargs
        resource_spec.update((self._spec.get("outputs") or {}).get(resource_name) or {})
        resource_name_xlt = resource_spec.pop("uri", None) or resource_name
        file_extension = os.path.splitext(resource_name_xlt)[1].strip(".")
        format = resource_spec.pop("format", None) or file_extension
        # NOTE: specs take precedence over kwargs, making model-supplied values effectively only defaults
        format_options = dict(resource_spec.get("format_options") or {})
        # detect SQL target
        ...
        # TODO support for time series databases
        ...
        if hasattr(content, PANDAS_WRITE_FORMATS[df_format]):
            ...
            if append:
                try:
                    ...
                    with opener(resource_name_xlt, 'rb') as f_r:
                        chk_args = {"nrows": 0}
                        if format == "json":
                            chk_args["lines"] = True
                        df0 = PANDAS_READ_FORMATS[df_format](f_r, **chk_args)
                        # verify column names match
                        if set(df0.columns) != set(content.columns):
                            raise DSLibraryException(f"Cannot append to {resource_name}, column names do not match")
                except FileNotFoundError:
                    append = False
            mode = "w"
            if append:
            ...
            with opener(resource_name_xlt, mode) as f_w:
        ...
        # save matplotlib figures as images
            ...
            if use_mlflow:
                mlflow.log_figure(content, resource_name_xlt)
                return
            buf = io.BytesIO()
        ...
        if isinstance(content, io.IOBase):
            ...
            if use_mlflow:
                ...
                mlflow.log_text(content, resource_name_xlt)
            else:
            ...
            if use_mlflow and isinstance(content, dict):
                mlflow.log_dict(content, resource_name_xlt)
                return
    ...
    def load_dataframe(self, resource_name: str, **kwargs):
        ...
        :param resource_name:   The name of an input resource, or of a local file, or the URI of an external resource.
                For sql- and nosql- type data sources, ":<table_name>" is expected.
        """
        # map requested resource through input spec
        resource_spec = (self._spec.get("inputs") or {}).get(resource_name) or {}
        resource_name_xlt = resource_spec.get("uri") or resource_name
        # mix in format arguments supplied in input specification
        format_options = dict(resource_spec)
        format_options.pop("uri", None)
        kwargs.update(format_options)
        # detect SQL
        if kwargs.get("sql") or kwargs.get("sql_table"):
            sql = kwargs.pop("sql", None)
            table = kwargs.pop("sql_table", None)
            if table:
        ...
        if kwargs.get("nosql_collection"):
            collection = kwargs["nosql_collection"]
            # TODO code get_nosql_connection() then implement accordingly
            raise DSLibraryException("Loading from nosql is not supported yet.")
        # TODO support read from time series database
        ...
        format_options = kwargs.pop("format_options", None) or {}
    ...
    def load_pickled_model(self, part: str=None):
        ...
        """
        return pickle.load(self.open_model_binary(part=part, mode='rb'))

    def save_pickled_model(self, model_object, part: str = None):
        """
        Use pickle to save a model.
        """
        return pickle.dump(model_object, self.open_model_binary(part=part, mode='wb'))
