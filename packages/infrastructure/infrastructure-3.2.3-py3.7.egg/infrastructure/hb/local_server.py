...
# @Last Modified time: 2021-04-13 14:07:33
...
class LocalServer(object):
	...
	def cpFilePreprocess(self,alreadyRecord,countNum=None):
		"""
		复制单次记录到待合并目录
		"""
		# alreadyRecord = JavaCoverageMergeRecord
		if countNum == None:
			countNum = self.alreadyRecord.id
		...
		if not os.path.exists("{jacocoIt}".format(jacocoIt=self.jacocoIt.format(
				week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name,recordID=self.alreadyRecord.id,index=countNum))):
		...
		command = "cp {jacocoIt} {mergeFile}/{index}.exec".format(jacocoIt=self.jacocoIt.format(
				week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name,recordID=countNum,index=self.alreadyRecord.id),mergeFile=mergeFile,
