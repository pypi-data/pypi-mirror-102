...
class freetail_module(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def __init__(self, waveform, 
        ...
        tx_waveform_file: If specified, the transmit waveform will be saved to this file in csv format
        """
        ...
        # If requested, write transmit waveform to a file
        if len(tx_waveform_file) > 0:
            np.savetxt(tx_waveform_file, np.stack((np.arange(len(padded_waveform))/samp_rate, padded_waveform), axis=1),
            delimiter=', ', header='Time, Waveform', comments='')
