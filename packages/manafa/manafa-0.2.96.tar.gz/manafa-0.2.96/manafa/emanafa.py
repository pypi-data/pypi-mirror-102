...
class EManafa(Service):
	...
	def __extractPowerProfile(self, filename):
		...
		res, suc, _ = execute_shell_command("adb pull /system/framework/framework-res.apk %s" % self.resources_dir)
		if res==0:
...
if __name__ == '__main__':
	...
	if not has_device_conn and invalid_file_args:
		log("Fatal error. No connected devices and result files submittted for analysis", LogSeverity.FATAL)
		exit(-1)
