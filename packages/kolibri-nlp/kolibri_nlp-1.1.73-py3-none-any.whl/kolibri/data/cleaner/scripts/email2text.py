...
function_re = ''
filename_job_functions=filename_disclaimer=filename_salutation=filename_email_closing=None
pattern_salutation=pattern_disclaimer=functions=disclaimers=disclaimer_openings=salutation_opening_statements=None
try:
...
#            "\*-+\s+Sent\s+:.*\s+Received\s+:.*\s+Reply to\s:.*\s+Attachments\s+:.*\s+\*-*",
    ...
    def _get_salutation(self):
        ...
        if pattern_salutation is not None:
...
#        regex = r"From\s*:[\w @\.:,;\&\?\\\(\)'\"\*[\]<>#\/\+-]+?(Subj(ect)?)\s?:|From\s*:[\w @\.:,;\&\(\)'\"\*[\]<>#\/\+-]+?(Sent|Date)\s?:(\s*\d+(\s|\/)(\w+|\d+)(\s|\/)\d+(\s|\/)?(\d+:\d+)?)?|From\s*:[\w @\.:,;\&\(\)'\"\*[\]<>#\/\+-]+?(Sent\s+at)\s?:(\s*\d+\s\w+\s\d+\s?\d+:\d+)?|From\s*:[\w @\.:,;\&\?\\\(\)'\"\*[\]<>#\/\+-]+?(CC)\s?:|From\s*:[\w @\.:,;\&\?\\\(\)'\"\*[\]<>#\/\+-]+?(To)\s?:"
    ...
    def _get_disclaimer(self):
        ...
        disclaimer_text=""
        if pattern_disclaimer is not None:
