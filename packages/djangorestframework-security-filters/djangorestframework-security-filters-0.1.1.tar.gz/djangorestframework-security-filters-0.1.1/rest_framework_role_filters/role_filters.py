...
class RoleFilterGroup:
    def __init__(self, role_filters):
        ...
        for role_filter in role_filters:
            if(isinstance(role_filter.role_id, list)):
                for role_id in role_filter.role_id:
                    self.role_filters[role_id] = role_filter
            else:
                self.role_filters[role_filter.role_id] = role_filter
                #self.role_filters = {role_filter.role_id: role_filter for role_filter in role_filters}

    def get_role_filter(self, role_id):
        filter = next((x for x in role_id if self.role_filters.get(x)), None)
        return self.role_filters.get(filter)
    ...
    def get_allowed_actions(self, role_id, request, view, obj=None):
        try:
            allowed_actions = self.trigger_filter(
                "get_allowed_actions", role_id, request, view, obj=obj)
        except TypeError:
            allowed_actions = self.trigger_filter(
                "get_allowed_actions", role_id, request, view)
            warnings.warn(
