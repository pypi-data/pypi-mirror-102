...
class RoleFilterMixin:
    ...
    def initial(self, request, *args, **kwargs):
        ...
        allowed_actions = self.role_filter_group.get_allowed_actions(
            self.role_id, request, self)
        if self.action not in allowed_actions:
            self.permission_denied(
                request, message="action={} not allowed for role={}".format(
                    self.action, self.role_id)
            )

    def get_queryset(self):
        queryset = super().get_queryset()
        filtered_queryset = self.role_filter_group.get_queryset(
            self.role_id, self.request, self, queryset)
    ...
    def check_object_permissions(self, request, obj):
        ...
        allowed_actions = self.role_filter_group.get_allowed_actions(
            self.role_id, request, self, obj=obj)
        if self.action not in allowed_actions:
            self.permission_denied(
                request, message="action={} not allowed for role={}".format(
                    self.action, self.role_id)
