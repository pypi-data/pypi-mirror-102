...
class ExplanationPackager(object):
    ...
    FILE_FIELDS = ['project_directory', 'user_predict_function_import_path', 'requirements_file', 'df', ]
    ...
    def __init__(self, model: 'ArthurModel', **kwargs):
        ...
        if self._model.input_type == InputType.NLP and self.explanation_algo != ArthurExplainer.LIME:
            logger.warning("Only lime is supported for NLP explainabilty but shap was passed. "
    ...
    def contains_file_fields(self) -> Tuple[bool, bool]:
        ...
        checks.append(self.df is not None)
