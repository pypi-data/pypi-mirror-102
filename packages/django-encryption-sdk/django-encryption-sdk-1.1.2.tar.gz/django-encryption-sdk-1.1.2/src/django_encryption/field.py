...
class DataKeeper(object):
    ...
    def plain(self):
        ...
        return self._cipher_text if self._cipher_text is None else crypto_client().decrypt(self._cipher_text)
    ...
    def cipher(self):
        ...
        if self._plain_text is not None:
            self._cipher_text = crypto_client().encrypt(self._plain_text)
            return self._cipher_text
        return None
    ...
    def mask(self, mask_type=None):
        ...
        获取掩码信息
        """
        plain_info = self._plain_text if self._plain_text is not None else crypto_client().decrypt(self._cipher_text)
...
class DataKeeperCharField(CharField):
    ...
    def __init__(self, *args, mask_type="3**4", edk_key='default', **kwargs):
        """
        mask_type: [num1] (*|**) [num2]
        num1:前num1位明文显示
        (*|**): *表示固定三个*，(即***)，**表示按遮挡长度显示*
