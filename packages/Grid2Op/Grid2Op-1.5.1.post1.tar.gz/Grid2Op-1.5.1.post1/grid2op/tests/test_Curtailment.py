...
class TestCurtailmentEnv(HelperTests):
    ...
    def test_change_curtailment(self):
        ...
        gen_id = 2
        ratios_max = [0.25, 0.24, 0.23, 0.18, 0.29, 0.19, 0.05, 0.2, 0.3, 1.0]  # don't change it
    ...
    def test_curtailment_mw_to_ratio(self):
        ...
        assert np.array_equal(act.curtail, np.array([-1., -1., 0.2857143, -1., -1., -1.], dtype=dt_float))
        # test i can modify the feature a second time
        act.curtail = act.curtailment_mw_to_ratio([(gen_id, 2.0 * amount_max_mw)])
        assert np.array_equal(act.curtail, np.array([-1., -1., 2.0 * 0.2857143, -1., -1., -1.], dtype=dt_float))
        ...
        assert np.array_equal(act.curtail, np.array([-1., -1., 2.0 * 0.2857143, 0.14285715, -1., -1.], dtype=dt_float))
        # test i cannot end up with results above 1
        act.curtail = act.curtailment_mw_to_ratio([(gen_id2, act.gen_pmax[gen_id2] + 10.)])
        assert np.array_equal(act.curtail, np.array([-1., -1., 2.0 * 0.2857143, 1.0, -1., -1.], dtype=dt_float))

        # test the property "curtail_mw"
        act2 = self.env1.action_space()
        act2.curtail_mw = [(gen_id, amount_max_mw)]
        assert np.array_equal(act2.curtail, np.array([-1., -1., 0.2857143, -1., -1., -1.], dtype=dt_float))
        assert np.array_equal(act2.curtail_mw, np.array([-1., -1., amount_max_mw, -1., -1., -1.], dtype=dt_float))
        # test i can modify the feature a second time
        act2.curtail_mw = [(gen_id, 2.0 * amount_max_mw)]
        assert np.array_equal(act2.curtail, np.array([-1., -1., 2.0 * 0.2857143, -1., -1., -1.], dtype=dt_float))
        assert np.array_equal(act2.curtail_mw, np.array([-1., -1., 2.0 * amount_max_mw, -1., -1., -1.], dtype=dt_float))
        # test i modify another gen, it does not erase the first one
        act2.curtail_mw = [(gen_id2, 0.5 * amount_max_mw)]
        assert np.array_equal(act2.curtail, np.array([-1., -1., 2.0 * 0.2857143, 0.14285715, -1., -1.], dtype=dt_float))
        assert np.array_equal(act2.curtail_mw, np.array([-1., -1., 2.0 * amount_max_mw, 0.5 * amount_max_mw, -1., -1.],
                                                        dtype=dt_float))
        # test i cannot end up with results above 1
        act2.curtail_mw = [(gen_id2, act2.gen_pmax[gen_id2] + 10.)]
        assert np.array_equal(act2.curtail, np.array([-1., -1., 2.0 * 0.2857143, 1.0, -1., -1.], dtype=dt_float))
        assert np.array_equal(act2.curtail_mw, np.array([-1., -1., 2.0 * amount_max_mw, act2.gen_pmax[gen_id2], -1., -1.],
                                                        dtype=dt_float))
