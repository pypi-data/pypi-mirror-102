...
def readCellData_to_pointData(caseDir, timename, fieldNames,MeshData):
    ...
    for fieldName in fieldNames:
        ...
        for i,dataName in zip(range(0,len(field)),field.keys()):
            ...
            fielddata=field[dataName][validCells_index] # only extract field values associated with polygons
            index_cell=0
            for i in range(0,len(fielddata)):
                for j in range(0,polygon2triangle[i]):
                    celldata.SetValue( index_cell, fielddata[i] )
    ...
    return {'pointData':pointdata,'cellData':field}
...
def getDataOnPatch_cellZones(caseDir,timeName,patchName,cellZoneNames,fieldNames):
    ...
    for cellZoneName in cellZoneNames:
        ...
        ind_Cells[cellZoneName]=(cellNames_patch == cellZoneName)
    return ind_Cells,fields
