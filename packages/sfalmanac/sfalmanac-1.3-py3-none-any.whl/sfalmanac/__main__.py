...
from sfalmanac.lib import eventtables as eventtables
...
##        config.closeLOG()     # close log after the for-loop
        ...
        for yearint in range(int(yearfr),int(yearto)+1):
            ...
            outfile = open(fn + ".tex", mode="w", encoding="utf8")
            outfile.write(eventtables.maketables(first_day,183,df180,df360,dfcm))
            outfile.close()
            stop = time.time()
            msg = "execution time = {:0.2f} seconds".format(stop-start) # msg = "execution time = %0.2f seconds" %(stop-start)
            print(msg)
...
##        config.closeLOG()
        ...
        outfile = open(fn + ".tex", mode="w", encoding="utf8")
        outfile.write(eventtables.maketables(first_day,3,df180,df360,dfcm))
        outfile.close()
        makePDF(fn)
        tidy_up(fn)
