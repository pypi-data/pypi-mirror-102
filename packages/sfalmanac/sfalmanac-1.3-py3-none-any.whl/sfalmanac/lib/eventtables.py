...
'''
        lasthemisph = hemisph
        twi = alma_skyfield.twilight(date, i, hemisph, True)
        moon, moon2 = alma_skyfield.moonrise_set2(date,i,hemisph)
...
'''.format(datestr)
    p = alma_skyfield.planetstransit(date, True)
    m = m + r'''Venus & {} & {} \\
'''.format(p[0],p[1])
    m = m + r'''Mars & {} & {} \\
'''.format(p[2],p[3])
...
def equationtab(date):
    ...
    n = 0
    while n < 2:
        gham, decm, degm, HPm, GHAupper, GHAlower, ghaSoD, ghaEoD = alma_skyfield.moonGHA(d, True)
...
'''
    ...
    d = date
    for k in range(2):
        eq = alma_skyfield.equation_of_time(d,d + datetime.timedelta(days=1),UpperLists[k],LowerLists[k],True,True)
        tab = tab + r'''{} & {} & {} & {} & {} & {} & {}({}\%) \\
...
'''.format(timedelta, date.strftime("%Y %B %d"),(date+datetime.timedelta(days=1)).strftime("%b. %d"), rightindent)
    ...
    page = page + twilighttab(date2,dfcm)
...
def maketables(first_day, pagenum, df180, df360, dfcm):
