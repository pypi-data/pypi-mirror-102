...
class EngineTest(unittest.TestCase):
    ...
    def test_loops(self):
        ...
        fox_file = join(current_dir, 'tests/input/test_loops_fox')
        transform(for_test_path, {})
        self.assertTrue(compare_files(output_path, fox_file))
        remove(join(current_dir, 'tests/input/test_loops_fox')) # Удаление файлы, созданного во время теста

    def test_variable(self):
        current_dir = getcwd()
        for_test_path = join(current_dir, 'tests/input/test_variable')
        output_path = join(current_dir, 'tests/output/test_variable')
        fox_file = join(current_dir, 'tests/input/test_variable_fox')
        transform(for_test_path, {
            'test_var': 'Hello, World!'
        })
        self.assertTrue(compare_files(output_path, fox_file))
        remove(join(current_dir, 'tests/input/test_variable_fox')) # Удаление файлы, созданного во время теста

    def test_all(self):
        current_dir = getcwd()
        for_test_path = join(current_dir, 'tests/input/test_all')
        output_path = join(current_dir, 'tests/output/test_all')
        fox_file = join(current_dir, 'tests/input/test_all_fox')
        transform(for_test_path, {
            'var': 'Hello, World!'
        })
        self.assertTrue(compare_files(output_path, fox_file))
        remove(join(current_dir, 'tests/input/test_all_fox')) # Удаление файлы, созданного во время теста
