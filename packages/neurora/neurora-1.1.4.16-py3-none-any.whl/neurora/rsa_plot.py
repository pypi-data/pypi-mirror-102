...
def plot_tbyt_decoding_acc(acc, start_time=0, end_time=1, chance=0.5, p=0.05, cbpt=True, color='r', lim=[0.4, 0.8],
                           figsize=[6.4, 3.6], x0=0, fontsize=16):
    ...
    if cbpt == True:
        ps = clusterbased_permutation_1d_1samp_1sided(acc, level=chance, p_threshold=p)
    else:
        ps = np.zeros([nts])
        chance = 0.5
        for t in range(nts):
