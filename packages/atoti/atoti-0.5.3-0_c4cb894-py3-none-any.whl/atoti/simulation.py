...
class Scenario:
    ...
    def _prepare_mapping(self, row: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if _PRIORITY in prepared_row:
            ...
            if not isinstance(priority, (int, float)):
                raise ValueError(
                    f"Value {priority} provided for {_PRIORITY} is of incorrect type, "
    ...
    def append(self, *rows: Row):
        ...
        prepared_df = pd.DataFrame(prepared_rows, columns=self.columns)
        self.load_pandas(prepared_df)

    @doc(STORE_IADD_DOC, **_SCENARIO_DOC_KWARGS)
    def __iadd__(self, row: Row):
        prepare_func = (
            self._prepare_mapping if isinstance(row, Mapping) else self._prepare_tuple
        )
        prepared_row = prepare_func(cast(Any, row))
        self._java_api.insert_in_store(
            store=self._simulation,
            scenario_name=None,
            row=prepared_row,
        )
        return self
    ...
    def head(self, n: int = 5) -> pd.DataFrame:
        ...
        dataframe = self._java_api.get_store_dataframe(
            self._simulation,
            0,
            keys=[level.name for level in self._simulation.levels],
        )
