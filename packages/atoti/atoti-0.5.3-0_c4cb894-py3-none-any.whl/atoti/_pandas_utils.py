...
def pandas_to_temporary_csv(
    ...
    *,
    ...
    Args:
        ...
        sep: The separator to use when writting the csv.
...
def _flatten_dataframe(dataframe: pd.DataFrame):  # type: ignore
    ...
    for col in dataframe.columns.to_flat_index():
        if isinstance(col, tuple):
            ...
            new_columns.append(COLUMN_LEVEL_SEPARATOR.join(map(str, col)))
        else:
...
def get_csv_sep_for_pandas_read_load(
    ...
    """Get seperator for CSv an display a warning if the user specify one.
