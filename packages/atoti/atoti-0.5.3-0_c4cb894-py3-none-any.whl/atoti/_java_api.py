...
from ._vendor.atotipy4j.java_collections import JavaArray, ListConverter
...
class JavaApi(metaclass=ApiMetaClass):
    ...
    def delete_role(self, role_name: str):  # pylint: disable=no-self-use
        raise MissingPluginError("plus")

    def upsert_role(  # pylint: disable=no-self-use
        self, role_name: str, *restrictions: Any
    ):
        raise MissingPluginError("plus")
    ...
    def insert_in_store(
        ...
        *,
        ...
        """Insert a row on a store scenario."""
        # Check the type of the row
        if isinstance(row, dict):
            java_row = to_java_map(self.gateway, row)
            self.java_api.insertOnStoreBranch(store.name, scenario_name, java_row)
            return

        if isinstance(row, tuple):
            java_row_array = to_java_object_array(self.gateway, row)
            self.java_api.insertOnStoreBranch(store.name, scenario_name, java_row_array)
            return
    ...
    def get_store_dataframe(
        ...
        for name, data_type in store._types.items():
    ...
    def register_aggregation_function(
        ...
        self.java_api.registerUserDefinedAggregateFunction(
    ...
    def value_measure(
        ...
        *,
        ...
        """Create a new store value measure and return its name."""
        java_required_levels = (
            to_java_string_array(
                self.gateway, convert_level_in_description(required_levels)
            )
        ...
        return self.java_api.createValueMeasure(
            cube.name,
            measure_name,
            store_name,
            column_name,
    ...
    def do_create_simulation(
        ...
        """Create a family of simulations for the cube and return the name of its backing store."""
        # Replace None values
        if not multiply:
            multiply = []
        if not replace:
    ...
    def create_bucketing(
        ...
        jlevels = to_java_string_array(
            self.gateway,
            [lvl._java_description for lvl in columns_seq],
        )
    ...
    def get_user(  # pylint: disable=no-self-use
        self, username: str, *, authentication_type: str
    ) -> Optional[Any]:
        raise MissingPluginError("plus")
    ...
    def create_user(  # pylint: disable=no-self-use
        self, user: Any, *, authentication_type: str
    ):
        raise MissingPluginError("plus")

    def update_user_roles(  # pylint: disable=no-self-use
        self, username: str, *, roles: Collection[str], authentication_type: str
    ):
        raise MissingPluginError("plus")

    def change_user_password(  # pylint: disable=no-self-use
        self, username: str, *, password: str, authentication_type: str
    ):
        raise MissingPluginError("plus")
