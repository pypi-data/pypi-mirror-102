...
PARQUET_KWARGS = {
    "path": """path: The path to the Parquet file or directory.
                If a path pointing to a directory is provided, all of the files with the ``.parquet`` extension in the directory and subdirectories will be loaded into the same store and, as such, they are all expected to share the same schema.
                The path can contain glob parameters (e.g. ``path/to/directory/**.*.parquet``) and will be expanded correctly.
                Be careful, when using glob expressions in paths, all files which match the expression will be loaded, regardless of their extension.
                When the provided path is a directory, the default glob parameter of ``**.parquet`` is used."""
}

QUANTILE_DOC = """Return a measure equal to the requested quantile {what}.

    Here is how to obtain the same behavior as `these standard quantile calculation methods <https://en.wikipedia.org/wiki/Quantile#Estimating_quantiles_from_a_sample>`_:
...
STORE_CREATION_KWARGS = {
    ...
    "partitioning": """partitioning : The description of how the data will be split across partitions of the store.
                ...
                Joined stores can only use a sub-partitioning of the store referencing them.
