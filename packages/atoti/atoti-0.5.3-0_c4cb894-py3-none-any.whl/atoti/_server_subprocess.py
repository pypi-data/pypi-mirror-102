...
from typing import TYPE_CHECKING, Any, Collection, Optional, Tuple
...
class ServerSubprocess:
    ...
    def _start(self, **kwargs: Any) -> Tuple[Popen, int]:
    ...
    def _create_subprocess(self, jar: Path) -> Popen:
        ...
        if self._config.url_pattern:
            program_args.append(f"-Dserver.url_pattern={self._config.url_pattern}")

        if self._config.max_memory:
            program_args.append(f"-Xmx{self._config.max_memory}")

        program_args.extend(self._config.java_args or [])
