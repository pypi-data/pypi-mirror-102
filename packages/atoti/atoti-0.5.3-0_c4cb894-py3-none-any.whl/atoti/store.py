...
from ._file_utils import _split_path_and_pattern, _validate_glob_pattern, _validate_path
...
class Store(ReprJsonable):
    ...
    def _check_if_sampling_policy_is_respected(self):
        ...
        if self._java_api.should_warn_for_store_sampling_policy(self):
            ...
            msg += _get_warning_message(self._sampling_mode)
    ...
    def append(self, *rows: Row, in_all_scenarios: bool = False):
        ...
        self.load_pandas(rows_df, in_all_scenarios=in_all_scenarios)

    @doc(STORE_IADD_DOC, **_DOC_KWARGS)
    def __iadd__(self, row: Row) -> Store:
        """Add a single row to the store."""
        self._java_api.insert_in_store(store=self, scenario_name=self.scenario, row=row)
        return self
...
def _create_store(java_api: JavaApi, name: str) -> Store:
    ...
    store._check_if_sampling_policy_is_respected()
    return store
...
class StoreScenarios:
    ...
    def load_csv(
        ...
        * The contents of the :guilabel:`Base` directory are loaded into the base scenario.
        * Two new scenarios are created: :guilabel:`Scenario1` and :guilabel:`Scenario2`, containing respectively
        ...
        """
        _validate_path(scenario_directory_path)
