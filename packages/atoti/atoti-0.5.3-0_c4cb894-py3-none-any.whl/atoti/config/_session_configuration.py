...
def _resolve_metadata_db(metadata_db: Union[Path, str]) -> str:
    ...
    return str(Path(metadata_db).absolute())
...
class SessionConfiguration(Configuration):
    ...
    def _create(cls, data: Mapping[str, Any]):
        ...
        logging = _pop_subconfig("logging", LoggingConfiguration, data_dict)
...
def merge_config(
    ...
    return SessionConfiguration(
        ...
        logging=_get_or_fallback(instance2.logging, instance1.logging),
...
def create_config(
    *,
    ...
    Args:
        ...
        logging: The parameters describing how the session logs will be handled.
        ...
        same_site:
            ...
            Setting it to ``none`` requires the session to be served in HTTPS so an *url_pattern* starting with ``https://`` must also be passed.
        ...
        url_pattern: The pattern used to build the URL accessed through :attr:`atoti.session.Session.url`.
            The following placeholder replacements will be made:
                ...
                * ``{protocol}``: ``http`` or ``https`` depending on whether the *https* parameter was defined or not.
    ...
    return SessionConfiguration(
        ...
        metadata_db=_resolve_metadata_db(metadata_db) if metadata_db else None,
        port=port,
        roles=roles,
        same_site=same_site,
        sampling_mode=sampling_mode,
