...
class AttrDict(vBiWh):
 def __init__(self,*args,**kwargs):
  vBiWG(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and vBiWy(socket,'IPV6_V6ONLY'):
...
class Half(vBiWM):
 def __init__(self,opt,sock,addr,vBiWN):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.vBiWN=vBiWN
  self.name='peer' if self.vBiWN else 'client'
  self.queue=[]
  self.dest=vBiWK
  self.err=vBiWK
  self.ready=vBiWc
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':vBiWc,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':vBiWc,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':vBiWc,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':vBiWc,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
