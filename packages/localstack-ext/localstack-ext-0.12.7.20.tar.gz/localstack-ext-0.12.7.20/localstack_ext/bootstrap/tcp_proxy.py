...
class AttrDict(uLmAB):
 def __init__(self,*args,**kwargs):
  uLmAf(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and uLmAn(socket,'IPV6_V6ONLY'):
...
class Half(uLmAj):
 def __init__(self,opt,sock,addr,uLmAx):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.uLmAx=uLmAx
  self.name='peer' if self.uLmAx else 'client'
  self.queue=[]
  self.dest=uLmAX
  self.err=uLmAX
  self.ready=uLmAO
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':uLmAO,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':uLmAO,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':uLmAO,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':uLmAO,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
