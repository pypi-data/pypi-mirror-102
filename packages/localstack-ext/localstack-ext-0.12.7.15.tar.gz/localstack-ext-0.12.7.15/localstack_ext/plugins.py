...
def register_localstack_plugins():
 ...
 if jytoB([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if 'No module named' not in jytoY(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return jytoF(os.environ.get('LOCALSTACK_API_KEY'))
