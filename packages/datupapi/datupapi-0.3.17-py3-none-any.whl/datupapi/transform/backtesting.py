...
class Backtesting(Config):
    ...
    def compute_tracked_bias(self, df_back, use_location=False):
        """
            ...
            if use_location:
                for location in df_back['Location'].unique():
                    for item in df_back['Item'].unique():
                        mask =  (df_back['Location'] == location) & (df_back['Item'] == item)
                        if df_back[mask].empty == False:
    ...
    def compute_tracked_bias_error(self, df_back, error_types=['WMAPE', 'MASE'], use_location=False):
        """
                    ...
                    for error_type in error_types:
                        if error_type == 'sMAPE':
                            ...
                            self.logger.exception(
                                f'No valid forecast error name. Choose from sMAPE, WMAPE, MASE, WAPE, RMSE')
                    df_back = df_back.replace([np.inf, -np.inf], np.nan)
                    df_back = df_back.fillna(0)
        except KeyError as err:
            self.logger.exception(f'Invalid column name. Please check dataframe metadata: {err}')
