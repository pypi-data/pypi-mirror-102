...
class Forecasting(Config):
    ...
    def compute_errors_from_backtests(self, df_forecast, df_backtest, error_types=['WMAPE', 'MASE'], use_location=False):
        """
        ...
        try:
            if use_location:
                df_errors = df_backtest.groupby(['Item', 'Location'], as_index=True) \
                                       .agg(error_dict) \
                                       .applymap(lambda x: round(x, 2)) \
                                       .reset_index(drop=False)
                df_forecast = pd.merge(df_forecast, df_errors, on=['Item', 'Location'], how='left')
            else:
    ...
    def compute_interval_per_backtests(self, df_forecast, q_backtest, backtest_names=None, datalake_path_=None, use_location=False):
        """
        ...
        try:
            if use_location:
                for back_ix, backtest in enumerate(backtest_names):
                    ...
                    for location in df_back['Location'].unique():
                        for item in df_back['Item'].unique():
                            mask_back = (df_back['Item'] == item) & (df_back['Location'] == location)
                            mask_fore = (df_forecast['Item'] == item) & (df_forecast['Location'] == location)
    ...
    def compute_naive_forecast(self, df_forecast, use_location=False):
        """
        Return a dataframe attaching the back suggested interval and forecast per date and item record
        ...
        try:
            if use_location:
                for location in df_forecast['Location'].unique():
                    for item in df_forecast['Item'].unique():
                        mask = (df_forecast['Location'] == location) & (df_forecast['Item'] == item)
