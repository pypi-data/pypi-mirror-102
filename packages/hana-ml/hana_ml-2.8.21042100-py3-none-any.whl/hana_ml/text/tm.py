...
#pylint: disable=superfluous-parens, too-many-branches, no-else-return
'''
...
def text_classification(pred_data, ref_data=None, k_nearest_neighbours=None, thread_ratio=None, lang='EN', index_name=None):
    """
    ...
    if conn_context.is_cloud_version():
        ...
        if isinstance(ref_data, DataFrame):
            tf_analysis_result, doc_term_freq, doc_category = tf_analysis(ref_data)
        else:
        ...
        if index_name is None:
            index_name = "TM_CATEGORIZE_KNN_INDEX_{}".format(ref_data.name.replace('-', '_').upper())
        ...
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        result_tab = "#TM_CATEGORIZE_KNN_{}_TBL".format(unique_id)
        for row in pred_data.select(pred_data.columns[0]).collect().to_numpy().flatten():
            ...
            SELECT '{6}' ID, * FROM TM_CATEGORIZE_KNN(
                ...
                FROM "{5}"."{2}")""".format(pred_data.filter(""""{cond}"='{rowid}'""".format(cond=pred_data.columns[0], rowid=row)).select(pred_data.columns[1]).select_statement,
                                            ...
                                            schema,
                                            row)
            conn_context.sql(sel_statement).save(result_tab, append=True)
        return conn_context.table(result_tab)
...
def _get_basic_func(func, pred_data, ref_data, top, threshold, lang, index_name):
    conn_context = pred_data.connection_context
    tf_analysis_result = None
    doc_term_freq = None
    doc_category = None
    if conn_context.is_cloud_version():
        ...
        mater_tab = '{}_{}_TBL'.format(func, ref_data.name.replace('-', '_').upper())
        ref_data.save(mater_tab, force=True)
        schema = conn_context.get_current_schema()
        conn_context.add_primary_key(mater_tab, ref_data.columns[0], schema)
        if index_name is None:
...
def get_related_doc(pred_data, ref_data=None, top=None, threshold=None, lang='EN', index_name=None):
    """
 ...
 """
    if pred_data.connection_context.is_cloud_version():
        return _get_basic_func("PAL_TMGETRELATEDDOC", pred_data, ref_data, top, threshold, lang, index_name)
    else:
        return _get_basic_func("TM_GET_RELATED_DOCUMENTS", pred_data, ref_data, top, threshold, lang, index_name)

def get_related_term(pred_data, ref_data=None, top=None, threshold=None, lang='EN', index_name=None):
    """
    ...
    if pred_data.connection_context.is_cloud_version():
        return _get_basic_func("PAL_TMGETRELATEDTERM", pred_data, ref_data, top, threshold, lang, index_name)
    else:
        return _get_basic_func("TM_GET_RELATED_TERMS", pred_data, ref_data, top, threshold, lang, index_name)

def get_relevant_doc(pred_data, ref_data=None, top=None, threshold=None, lang='EN', index_name=None):
    ...
    """
    if pred_data.connection_context.is_cloud_version():
        return _get_basic_func("PAL_TMGETRELEVANTDOC", pred_data, ref_data, top, threshold, lang, index_name)
    else:
        return _get_basic_func("TM_GET_RELEVANT_DOCUMENTS", pred_data, ref_data, top, threshold, lang, index_name)

def get_relevant_term(pred_data, ref_data=None, top=None, threshold=None, lang='EN', index_name=None):
    """
    ...
    """
    if pred_data.connection_context.is_cloud_version():
        return _get_basic_func("PAL_TMGETRELEVANTTERM", pred_data, ref_data, top, threshold, lang, index_name)
    else:
        return _get_basic_func("TM_GET_RELEVANT_TERMS", pred_data, ref_data, top, threshold, lang, index_name)

def get_suggested_term(pred_data, ref_data=None, top=None, threshold=None, lang='EN', index_name=None):
    """
    ...
    """
    if pred_data.connection_context.is_cloud_version():
        return _get_basic_func("PAL_TMGETSUGGESTEDTERM", pred_data, ref_data, top, threshold, lang, index_name)
    else:
        return _get_basic_func("TM_GET_SUGGESTED_TERMS", pred_data, ref_data, top, threshold, lang, index_name)
