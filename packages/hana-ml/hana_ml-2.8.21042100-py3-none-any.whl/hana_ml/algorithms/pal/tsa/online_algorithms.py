...
#pylint: disable=too-many-lines, line-too-long, too-many-instance-attributes, too-many-locals, too-many-statements, too-many-branches
import logging
import uuid
...
class OnlineARIMA(PALBase):
        ...
        **kwargs : keyword arguments, optional
        ...
        try:
            with conn.connection.cursor() as cur:
                cur.execute('CREATE LOCAL TEMPORARY COLUMN TABLE {} ("KEY" NVARCHAR(100), "VALUE" NVARCHAR(5000))'.format(input_model_tbl))
                if self.model_ is not None:
                    cur.execute('INSERT INTO {} SELECT * FROM ({})'.format(input_model_tbl, self.model_.select_statement))
                    if kwargs:
                        if 'learning_rate' in kwargs:
                            if self.model_ is not None:
                                learning_rate_ = self._arg('learning_rate', kwargs['learning_rate'], float)
                                with conn.connection.cursor() as cursor:
                                    cursor.execute("UPDATE {} SET VALUE='{}' WHERE KEY='lrate'".format(input_model_tbl,
                                                                                                       learning_rate_))
                        if 'epsilon' in kwargs:
                            if self.model_ is not None:
                                epsilon_ = self._arg('epsilon', kwargs['epsilon'], float)
                                with conn.connection.cursor() as cursor:
                                    cursor.execute("UPDATE {} SET VALUE='{}' WHERE KEY='epsilon'".format(input_model_tbl,
                                                                                                         epsilon_))
    ...
    def predict(self, forecast_length=None):
        """
