...
class DataFrame(object):#pylint: disable=too-many-public-methods
    ...
    def min(self):
        ...
        if len(self.columns) > 1:
            ...
            return self.agg(agg_list).collect().iloc[0]
        else:
            return self.agg([('min', self.columns[0], self.columns[0])]).collect().iat[0, 0]
    ...
    def max(self):
        ...
        if len(self.columns) > 1:
            ...
            return self.agg([('max', self.columns[0], self.columns[0])]).collect().iat[0, 0]
    ...
    def sum(self):
        ...
        if len(self.columns) > 1:
            ...
            return self.agg([('sum', self.columns[0], self.columns[0])]).collect().iat[0, 0]
    ...
    def median(self):
        ...
        if len(self.columns) > 1:
            ...
            return self.agg([('median', self.columns[0], self.columns[0])]).collect().iat[0, 0]
    ...
    def mean(self):
        ...
        if len(self.columns) > 1:
            ...
            return self.agg([('avg', self.columns[0], self.columns[0])]).collect().iat[0, 0]
...
def _check_geo_cols(**kwargs):
    ...
    geo_cols attribute to be a dictionary to properly transform SAP HANA binary results from Spatial columns into WKT.
...
def create_dataframe_from_shapefile(
    ...
    # The generated spatial column always is called SHAPE
    return DataFrame(
        connection_context,
