...
from Hapi.saintvenant import HBC
...
IF.ReadBoundaryConditionsTable(wpath + "/inputs/1d/topo/BonundaryConditions-segment24.txt")
...
for i in range(River.NoSeg):
    ...
    for step in range(s, e):
        ...
        if Q.sum(axis=1).values.max() > River.D1['MinQ']:
            ...
            River.RoutedQ[step_ind,River.Segments.index(SubID)] = q[Sub.XSno-1,River.TS-1]
