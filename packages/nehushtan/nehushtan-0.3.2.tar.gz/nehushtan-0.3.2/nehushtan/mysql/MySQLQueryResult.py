...
class MySQLQueryResult:
    ...
    def get_column_from_fetched_rows_as_tuple(self, field_key):
        """
        Since 0.3.2
        """
        if self._status != constant.MYSQL_QUERY_STATUS_QUERIED:
    ...
    def create_error_result(error_message: str):
        return MySQLQueryResult(MYSQL_QUERY_ROW_TYPE_UNKNOWN).set_status(constant.MYSQL_QUERY_STATUS_ERROR).set_error(
            error_message)
