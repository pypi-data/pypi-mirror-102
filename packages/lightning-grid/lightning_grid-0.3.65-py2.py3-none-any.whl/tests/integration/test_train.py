...
def monkey_patch_get_credentials(self):
    return {'getUserCredentials': [{'credentialId': CREDENTIAL_ID, 'provider': 'aws', 'defaultCredential': True}]}
...
def monkey_patch_wrong_creds(self):
    return {'getUserCredentials': [{'credentialId': 'wrong-cred', 'provider': 'aws', 'defaultCredential': True}]}
...
def test_train_fails_with_no_arguments(monkeypatch):
    ...
    monkeypatch.setattr(grid.Grid, 'get_credentials', monkey_patch_get_credentials)
    monkeypatch.setattr(grid.Grid, '_check_user_github_token', monkey_patch_token_check)
...
def test_train_fails_if_credentials_dont_exist(monkeypatch):
    ...
    result = RUNNER.invoke(cli.train,
                           ['--grid_credential', 'test-fail', '--grid_cpus', 1, '--', 'tests/data/script.py'])
...
def test_train_fails_if_wrong_credential(monkeypatch):
    ...
    result = RUNNER.invoke(cli.train, ['--grid_credential', CREDENTIAL_ID, '--', 'tests/data/script.py'])
...
def test_train_fails_with_multiple_non_default_credentials(monkeypatch):
    ...
    monkeypatch.setattr(grid.Grid, 'get_credentials', monkey_patch_get_credentials_multiple_no_default)
...
def test_train_fails_if_config_not_found(tmp_path, monkeypatch):
    ...
    result = RUNNER.invoke(cli.train,
                           ['--grid_credential', CREDENTIAL_ID, '--grid_config', 'no-file', 'tests/data/script.py'])
...
def test_train_fails_if_description_is_too_long(monkeypatch):
    ...
    result = RUNNER.invoke(cli.train, [
        '--grid_description', description, '--grid_credential', CREDENTIAL_ID, '--grid_cpus', 1, '--',
...
def test_train_default_cred(monkeypatch):
    ...
    monkeypatch.setattr(grid.Grid, 'get_credentials', monkey_patch_get_credentials_multiple)
    monkeypatch.setattr(grid.Grid, '_check_github_repo_accessible', monkey_patch_github_accessible)

    result = RUNNER.invoke(cli.train, ['--ignore_warnings', 'tests/data/script.py'])
...
def test_train_fails_if_github_repo_not_accessible(monkeypatch):
    ...
    monkeypatch.setattr(grid.Grid, 'get_credentials', monkey_patch_get_credentials)
    monkeypatch.setattr(grid.Grid, '_check_user_github_token', monkey_patch_token_check)
    monkeypatch.setattr(grid.Grid, '_check_github_repo_accessible', monkey_patch_github_not_accessible)
    ...
    assert launch_url == os.getenv('GRID_URL', env.DEFAULT_GRID_URL).replace("/graphql", "") + '/#/settings?tabId=integrations'
...
def test_train(monkeypatch):
    ...
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_description', description, '--grid_strategy', 'grid_search',
...
def test_train_with_nickname(monkeypatch):
    ...
    description = 199 * '-'  # 200 characters is the limit.
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_description', description, '--grid_instance_type', '2_cpu_4gb',
        '--grid_strategy', 'grid_search', '--grid_trials', "1", '--ignore_warnings', 'tests/data/script.py'
    ])
    assert result.exit_code == 0
    assert not result.exception
...
def test_train_with_framework(monkeypatch):
    ...
    env.DEBUG = True
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_description', 'test', '--grid_strategy', 'grid_search',
        '--grid_trials', "1", '--grid_framework', 'lightning', '--ignore_warnings', 'tests/data/script.py'
    ])
    assert result.exit_code == 0
    assert not result.exception
