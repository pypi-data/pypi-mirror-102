    ...
    for obj_field, related_field, foreign_field in url_args:
        if not related_field:
            if not hasattr(model, obj_field):
                print(
                    f"""
                    Warning : trying to acces non existent url arg {obj_field}
                    in {model.__name__} when generating detail menu
                """
                )
                continue
            ...
            assert hasattr(obj, related_field), f"{obj} has not {related_field} field"
            assert hasattr(
                model, foreign_field
            ), f"{model} has not {foreign_field} field"
            kwargs["param"] = foreign_field
            kwargs["pk"] = getattr(obj, related_field)
...
def get_model_fields(model, fields=["__all__"], verbose=True):
    """Retourne les champs / attributs du modèle suivant une liste de chaîne de caractères.

    Args:
        model {GenericModel}: le modèle
        fields ([str], optional): Liste des champs. Defaults to ['__all__'].
        verbose (bool, optional): Si vraie renvoie un tuple. Defaults to True.

    Returns:
        [field]: liste des attributs du modèle
    """
    ...
    def process_prop_field(field):
        if field != "pk":
            ...
            if field in model.VERBOSE_PROPERTIES:
                appended = (
                    field if not verbose else field,
                    model.VERBOSE_PROPERTIES[field],
                )
            else:
                print(
                    f"""
