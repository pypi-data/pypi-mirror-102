...
class BottleClient:
    ...
    def inject_genome(self, genome: Dict) -> Dict:
        ...
        *genome* must be either a JSON-serializable object or a file-like object.
        ...
        response = requests.post(f'{self.url}genome/inject', verify=self.secure, headers=self.headers, json={'genome': genome}).json()
    ...
    def ping(self, nodes: List = None) -> List[Dict[str, Any]]:
        ...
        if nodes:
            results = []
            for node in nodes:
                response = requests.get(f'{self.url}{self.genome.primitive_map[node]}/ping', verify=self.secure,
                                        headers=self.headers).json()
                if 'error' in response or response["message"] != 'okay':
                    self.system_failures.append({node: response})
                    print("System Failure:", {node["name"]: response})
    ...
    def define_mood(self, mood_list: List, nodes: List = None) -> List:
        ...
        *mood_data* should be of the format:
        ...
        for node in nodes:
            ...
            if 'error' in response or response["message"] != 'defined-mood':
                self.system_failures.append({node: response})
                print("System Failure:", {node["name"]: response})
                if len(mood_data) == 1 and self.summarize_for_single_node:
                    return response['message']
    ...
    def set_mood(self, mood_data: Dict, nodes: List = None) -> List:
        ...
        *mood_data* should be of the format:
        ...
        for node in nodes:
            ...
            if 'error' in response or response["message"] != 'set-mood':
                self.system_failures.append({node: response})
                print("System Failure:", {node["name"]: response})
                if len(mood_data) == 1 and self.summarize_for_single_node:
                    return response['message']
