...
from .annotations import (Array, BooleanArray, IntegralArray, RealArray, RealNumeric, Shape,
                          ShapeLike)
...
class Generator:
    ...
    def gamma(self, gamma_shape: RealNumeric, shape: Shape = ()) -> Tuple[RealNumeric, Generator]:
        new_g, this_g = self.split()
        return jax.random.gamma(this_g.key, gamma_shape, shape), new_g

    def normal(self, std_dev: RealNumeric, shape: Shape = ()) -> Tuple[RealNumeric, Generator]:
        new_g, this_g = self.split()
