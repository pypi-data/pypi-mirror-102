...
class ParamEditor(QtWidgets.QDockWidget):
  ...
  def __init__(self, parent=None, paramList: t.List[t.Dict] = None, saveDir: FilePath=None,
    ...
    def niceStateLoader(loadState='Default'):
      ...
      :param loadState:
        ...
        limits: [Default]
      """
      if loadState in self.savedStates and loadState != self.lastAppliedName:
  ...
  def _mkMetaTree(self):
    ...
    """
    self._metaTree.setHeaderHidden(True)
    oldSzHint = self._metaTree.sizeHint
    def newSzHint():
      baseHint = oldSzHint()
      baseHint.setHeight(int(baseHint.height() * 1.1))
      return baseHint
    self._metaTree.sizeHint = newSzHint
