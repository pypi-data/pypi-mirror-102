...
class GadgethiAuthenticationStandardDecryption():
	...
	def HMAC256_digest(self,secret,string,mode='base64'):
		...
		if mode != 'base64':
			...
			b64 = b64encode(bytes.fromhex(h.hexdigest())).decode()
			return b64
	def accepting_time_range(time_shift,interval):
		localtime = int(datetime.datetime.utcnow().timestamp()) + (time_shift*60*60)
		if localtime - interval < int(time) < localtime + interval:
			...
			return False
	def encryption_comparsion(secret):
		encryption_result = self.HMAC256_digest(self.secret,self.key+str(localtime))
		if encryption_result == self.HMAC256_result:
			...
			return False
	def authentication_decryption(time_shift=8,interval=30,secret='gadgethi'):
		if self.encryption_comparsion(time_shift,interval) and self.accepting_time_range(secret):
