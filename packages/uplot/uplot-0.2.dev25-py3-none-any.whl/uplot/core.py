...
def _jet(n=256):
    ones = np.ones(n)
    zeros = np.zeros(n)
    incr = np.linspace(0., 1., n)
    decr = incr[::-1]

    b2t = np.vstack([zeros, incr, ones]).T  # 001 -> 011
    t2g = np.vstack([zeros, ones, decr]).T  # 011 -> # 010
    g2y = np.vstack([incr, ones, zeros]).T  # 010 -> 110
    y2r = np.vstack([ones, decr, zeros]).T  # 110 -> 100

    return np.vstack([b2t, t2g, g2y, y2r])

jet = ListedColormap(_jet(n=256))
