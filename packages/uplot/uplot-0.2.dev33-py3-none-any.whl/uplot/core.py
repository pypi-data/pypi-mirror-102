...
jet = LinearSegmentedColormap.from_list('ujet', colors=jet_rgb / 255.)
...
class Line(Geometry):  # noqa
    ...
    def _to_mpl(self, figure, axis):
        ...
        for i, column in enumerate(data.columns):
            ...
            # TODO are we in love with camel_case here?
            line_width = self.get_style('line_width', index=i)

            line_style = self.get_style('line_style', index=i)
            if line_style is not None:
                # TODO turn this into a function
                if line_style == 'longdash':
                    line_style = eval(mpl_line_styles[line_style])
                else:
                    line_style = mpl_line_styles[line_style]
            ...
            axis.plot(x, y, label=label, color=color, linestyle=line_style, linewidth=line_width, marker=marker)
...
# Horizontal line
class HorizontalLine(Geometry):  # noqa
    """
    Add a horizontal line to the plot
    """

    # Initialize class instance
    def __init__(self, y, style=None):
        self._y = y
        super().__init__(None, style)  #

    # Add the horizontal line to matplotlib
    # noinspection PyShadowingNames
    def _to_mpl(self, figure, axis):  # TODO why do we have figure argument if it is never used?
        # Get y
        y = self._y

        label = self.get_style('label')
        color = self.get_style('color')

        # TODO are we in love with camel_case here?
        line_width = self.get_style('line_width')

        line_style = self.get_style('line_style')
        if line_style is not None:  # noqa
            # TODO turn this into a function
            if line_style == 'longdash':
                line_style = eval(mpl_line_styles[line_style])
            else:
        ...
        marker = self.get_style('marker')
        if marker is not None:
            marker = mpl_markers[marker]

        axis.axhline(y, label=label, color=color, linestyle=line_style, linewidth=line_width, marker=marker)
...
# Horizontal line convenience function
def hline(y=None, style=None):
    """
    Convenience function to create a horizontal line at a given `y` value.

    Parameters
    ----------
    y : number
        Value at which to place horizontal line.
    style : dict
        If provided, list of style elements.
    ...
    -------
    HorizontalLine
        Instance of HorizontalLine object.
    """

    if not isinstance(y, (int, float)):
        raise AttributeError('y must be a number')

    style = _coerce_style(style, defaults={'line_style': 'longdash', 'color': 'black', 'line_width': 1.})
