        ...
        if self.n_shifts is None:
            ...
            for v in range(self.n_levels):
                self.values[slice(0, self.n_shifts, 1), v], self.bin[v] = np.split(
                    self.unique[v] * math.ceil(self.n_shifts / self.n_unique[v]), [self.n_shifts])
            # self.__next__()
    ...
    def __next__(self):
        ...
        if self.n_shifts is None:
            ...
            for v in range(self.n_levels):
                ...
                self.values[slice(0, self.n_shifts, 1), v], self.bin[v] = np.split(np.random.permutation(
                    np.append(self.values[slice(0, self.n_shifts, 1), v], self.bin[v], axis=0)),
