...
class ImageLoader:
    ...
    def __init__(
            ...
            inputs_axes_of_image=None, inputs_axis_of_samples=None,
        ...
        self.format_images = cp_directory.get_extension(directory_0, point=False).lower()
        ...
        self.shape_images = shape_image_0
        self.n_dims_images = self.shape_images.__len__()

        self.n_dims_samples = self.n_dims_images + self.H
        self.n_dims_batch = self.n_dims_samples + 1

        self.n_dims_directories_eb = self.H + 1

        if inputs_axes_of_image is None:
            self.inputs_axes_of_image = np.arange(
                self.n_dims_batch - self.n_dims_images, self.n_dims_batch, 1, dtype='i')
        else:
            try:
                len(inputs_axes_of_image)
                if isinstance(inputs_axes_of_image, np.ndarray):
                    self.inputs_axes_of_image = inputs_axes_of_image
                else:
                    self.inputs_axes_of_image = np.asarray(inputs_axes_of_image, dtype='i')
            except TypeError:
                self.inputs_axes_of_image = np.asarray([inputs_axes_of_image], dtype='i')

        self.inputs_axes_of_image[self.inputs_axes_of_image < 0] += self.n_dims_batch

        self.inputs_axes = np.arange(0, self.n_dims_batch, 1, dtype='i')

        if inputs_axis_of_samples is None:
            self.inputs_axis_of_samples = 0
            while self.inputs_axis_of_samples in self.inputs_axes_of_image:
                self.inputs_axis_of_samples += 1
        elif inputs_axis_of_samples in self.inputs_axes_of_image:
            if inputs_axes_of_image is None:
                ...
                self.inputs_axes_of_image[self.inputs_axes_of_image <= self.inputs_axis_of_samples] -= 1
            else:
                raise ValueError('inputs_axis_of_samples, inputs_axes_of_image')
        else:
            self.inputs_axis_of_samples = inputs_axis_of_samples
        ...
        inputs_axes_of_non_intra = np.sort(np.append(self.inputs_axis_of_samples, self.inputs_axes_of_image))
        ...
        #     self.batch_axes[cp_array.samples_in_arr1_are_not_in_arr2(
        #         self.batch_axes, self.inputs_axes_of_image)])
        ...
        self.shape_batch[self.inputs_axes_of_image] = self.shape_images
        ...
        # self.shape_images = tuple(self.shape_images)

        # self.shape_samples = tuple(np.append(self.n_conditions_directories_intra, self.shape_images, axis=0).tolist())
        # self.shape_batch = tuple(np.append([self.batch_size], self.shape_samples, axis=0).tolist())

        if self.return_inputs_eb:

            self.inputs_eb = torch.empty(tuple(self.shape_batch.tolist()), dtype=tensor_0.dtype, device=self.device)
        else:
            self.inputs_eb = None
        ...
        for i in self.inputs_axes:
            if i in self.inputs_axes_of_image:
                f += 1
                ...
                if self.combinations_directories_intra.shape[0] > 0:
                    ...
                    for j in range(self.combinations_directories_intra.shape[0]):
                        ...
                        if self.return_inputs_eb:
                            ...
                            #  if inputs_axes_of_image[0] > inputs_axes_of_image[1]
