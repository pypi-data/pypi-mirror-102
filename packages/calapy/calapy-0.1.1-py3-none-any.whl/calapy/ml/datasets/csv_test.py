...
class CSVLoaderTest:
    ...
    def __init__(
        ...
        self.n_dims_data = self.n_dims_batch
        ...
        self.shape_data = np.copy(self.shape_batch)
        self.shape_data[self.batch_axis_inputs] = self.n_samples

        if self.return_inputs_eb:
            self.inputs = torch.empty(tuple(self.shape_data.tolist()), dtype=torch.float32, device=self.device)
        else:
            self.inputs = None
        ...
        self.shape_directories = np.copy(self.shape_directories_eb)
        self.shape_directories[self.batch_axis_directories] = self.n_samples

        if self.return_relative_directories_eb:
            self.relative_directories = np.empty(self.shape_directories, dtype='O')
        else:
            self.relative_directories = None

        self.relative_directories_eb = None

        if self.return_absolute_directories_eb:
            self.absolute_directories = np.empty(self.shape_directories, dtype='O')
        else:
            self.absolute_directories = None
