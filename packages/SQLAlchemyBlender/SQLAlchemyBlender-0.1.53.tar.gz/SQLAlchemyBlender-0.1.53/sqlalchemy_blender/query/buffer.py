...
class QueryBuffer:
    ...
    def build(self, query, queryargs, entity=None):
        ...
        if queryargs.sortkey in keys:
            ...
            if queryargs.descending is True and queryargs.sortkey in columns(entity, strformat=True):
                query = query.order_by(desc(column))
            elif queryargs.descending is False:
    ...
    def apply(self, pending=False, inactive=False, query=None, entity=None):
        entity = entity
        if not entity:
            if hasattr(self.basequery, '_entity_zero'):
                entity = self.basequery._entity_zero()
            else:
                entity = self.basequery._entity_from_pre_ent_zero()
        ...
        if self.queryargs.sortkey in keys:
            column = getattr(entity.attrs, self.queryargs.sortkey)
        ...
        for item, value in self.queryargs.search.items():
            if item in base_fields:
                self.basequery = self.basequery.filter(getattr(self.model, item).like(value))
            else:
                raise ValueError(f'Field <{item}> does not exist in the requested resource. Please check inputs.')
        ...
        self.prepaged = self.basequery.count()
