...
class DoubleRangeWithUnit(WellsBaseModel):
    class Config:
        allow_population_by_field_name = True

    min: float = Field(..., example=123.123)
    max: float = Field(..., example=153.123)
    unit: Optional[str] = Field(None, example="meter")
...
class NPT(WellsBaseModel):
    ...
    measured_depth: DoubleWithUnit = Field(..., alias="measuredDepth")
...
class NPTFilter(WellsBaseModel):
    ...
    measured_depth: Optional[DoubleRangeWithUnit] = Field(None, alias="measuredDepth")
...
class EventIngestion(WellsBaseModel):
    ...
    npt_ingestion: Optional[List[NPT]] = Field(None, alias="nptIngestion")
...
class WellboreIngestion(WellsBaseModel):
    ...
    event_ingestion: Optional[EventIngestion] = Field(None, alias="eventIngestion")
