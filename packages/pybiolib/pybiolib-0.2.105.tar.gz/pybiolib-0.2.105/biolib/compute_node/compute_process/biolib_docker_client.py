...
class BiolibDockerClient():
    ...
    def pull_image(self, image_uri, access_token, image_lock: threading.Lock(), ecr_proxy, enclave_ecr_token):
        try:
            image_lock.acquire()
            threading.Thread(target=self._pull, args=(image_uri, access_token, image_lock, ecr_proxy,
    ...
    def _pull(self, image_uri, access_token, image_lock, ecr_proxy, enclave_ecr_token):
        try:
            start_time = time.time()
            ...
            try:
                ...
                if enclave_ecr_token:
                    auth_config = {'username': 'AWS', 'password': enclave_ecr_token}
                else:
