...
from fameio.source.tools import arg_handling_convert_results, log_and_raise, set_up_logger
...
def read_proto_file(proto_file_name):
    """Reads protobuf `DataStorage` of given `proto_file_name` and returns its `output`"""
...
def run(file_path, config):
    ...
    if len(proto_buffer.agentType) == 0:
        log_and_raise('Provided protobuf file contains no data in output section.')
