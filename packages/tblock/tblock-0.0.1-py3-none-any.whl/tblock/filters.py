...
def subscribe_to_filter(filter_id: str, db_path: str, tmp_dir: str, permissions: str = None,
    ...
    if not new_filter.exists and source is None:
        ...
        if force or prompt_user(f'You are about to subscribe to filter "{filter_id}"'):
            ...
            db.close()
            update_filter(filter_id, db_path, tmp_dir, verbosity, force)
        else:
            db.close()
...
def update_remote_repo(repo_url: str, repo_mirror: str, version_file: str,
    ...
    Args:
        ...
        repo_mirror (str): The URL of the remote repo index mirror, in case main index is inaccessible
    ...
    try:
        ...
        if verbosity:
            ...
            print(f' => Get mirror: {repo_mirror}')
        try:
            download_file(repo_mirror, tmp_index)
        except (requests.exceptions.ConnectionError, urllib3.exceptions.MaxRetryError, ConnectionRefusedError,
                urllib3.exceptions.NewConnectionError):
            if verbosity:
                print(' => Failed to get mirror')
