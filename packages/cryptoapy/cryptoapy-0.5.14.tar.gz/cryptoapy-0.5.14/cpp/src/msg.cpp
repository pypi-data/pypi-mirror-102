...
void CryptMsg::encrypt_streaming(BYTE **s, DWORD *slen) throw(CSPException)
{
    ...
    if(!CryptMsgGetParam(
        ...
        /* Size of the blob*/
        delete[] ei.rgpRecipients;
        throw CSPException("CryptMsg.encrypt_data_streaming: Couldn't get encrypted data size");
    }
    LOG("   calculated message size\n");
    *s = (BYTE *) malloc(*slen);
    if(!CryptMsgGetParam(
                hmsg,                      /* Handle to the message*/
                CMSG_CONTENT_PARAM,        /* Parameter type*/
        ...
        free((void *)*s);
        delete[] ei.rgpRecipients;
        throw CSPException("CryptMsg.encrypt_data_streaming: Couldn't get encrypted data");
    }
...
//
    ...
    if(!CryptMsgGetParam(
        ...
        delete[] ei.rgCertEncoded;
        delete[] ei.rgSigners;
        throw CSPException("CryptMsg.sign_data_streaming: Couldn't get signed data size");
    }
    LOG("   calculated message size\n");
    *s = (BYTE *) malloc(*slen);
    if(!CryptMsgGetParam(
        ...
        delete[] ei.rgCertEncoded;
        delete[] ei.rgSigners;
        throw CSPException("CryptMsg.sign_data_streaming: Couldn't get signed data");
    }
    LOG("   got message data\n");
    delete[] ei.rgCertEncoded;
    delete[] ei.rgSigners;
