    ...
    async def __connect(self):
        async with websockets.connect(self.WEBSOCKET_ENDPOINT) as conn:
            ...
            while True:
                ...
                await self.on_response(data_json)

    async def send_ping(self):
        await self.__conn.ping()

    async def send_field(self, ticket: str = str(uuid.uuid4()), type_fields: list[dict] = [{'type': 'ticker', 'codes': [Market.KRW_BTC]}], format: str = 'DEFAULT'):
        field = [
            {
                'ticket': ticket
            },
