...
class ReportBuilder:
    ...
    def create_template(self):
        ...
        web_file = open(f'{template_folder}/reports/{self.name_to_snake_case()}.html', 'w')
        web_file.write('''
        {% extends 'web_template_base.html' %}
        {% load static %}
        ''')
        pdf_file = open(f'{template_folder}/reports/pdf/{self.name_to_snake_case()}_pdf.html', 'w')
        pdf_file.write('''
    ...
    def create_views(self):
        file_path = os.path.join(settings.BASE_DIR,settings.MAIN_APP, 'core','views.py')
        view_file = open(file_path, 'a+')
        view_file.write(
