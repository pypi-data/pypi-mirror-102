...
class DockerBuild(object):
    def __init__(self, dockerfile, build_root, include, docker_repo, workdir=None, ignore=None, tag=None, default_ignore=None, include_version_file=False, variables=None, fetch=False, no_ignore_override=False, quiet=False, _raw_build_root=None, safe_dockerignore=False):
        ...
        self.quiet = quiet
        self.safe_dockerignore = safe_dockerignore
    ...
    def backup_dockerignore(self, dryrun, path):
        bak = None
        if os.path.exists(path):
            bak = os.path.join(self.build_root, '.dockerignore.tmp.{}'.format(time.time()))
            utils.rename(dryrun, path, bak)
        return bak

    def restore_dockerignore(self, dryrun, bak, path):
        if bak is None:
            utils.unlink(dryrun, path)
        else:
            utils.rename(dryrun, bak, path)

    def write_dockerignore(self, dryrun, lines, path):
        for line in lines:
        ...
        utils.write(dryrun, path, contents)
...
'''.format(include_path, include, include_path)
            ...
            match = re.search(r'^(.*)#docker-buildtool:variable ([\w-]*)$', line)
...
""".format(self.workdir))
    ...
    def write_dockerfile_and_dockerignore(self, dryrun):
        ...
        dockerignore_path = (custom_dockerfile.name + '.dockerignore' if self.safe_dockerignore else os.path.join(self.build_root, '.dockerignore'))
        ...
        if not self.no_ignore_override:
            bak = self.backup_dockerignore(dryrun, dockerignore_path)
            self.write_dockerignore(dryrun, ignore, dockerignore_path)
        else:
            bak = None
        return custom_dockerfile, dockerignore_path, bak
    ...
    def run(self, dryrun=False, docker_args=[]):
        ...
        try:
            custom_dockerfile, dockerignore_path, backup_dockerignore = self.write_dockerfile_and_dockerignore(dryrun)
            ...
            if versionfile:
                try:
                    ...
                    pass
            if not self.no_ignore_override:
                self.restore_dockerignore(dryrun, backup_dockerignore, dockerignore_path)
