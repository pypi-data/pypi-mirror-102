...
def remote_build(dryrun: bool, build, docker_args: List[str], s3_location=None, run_docker_build=True):
    ...
    remote_temp = None
    dockerfile = None
    dockerignore_bak = None
    try:
        dockerfile, dockerignore_path, dockerignore_bak = build.write_dockerfile_and_dockerignore(dryrun)
        ...
        included_files_path = dockerignore_to_files(dryrun, dockerignore_path, build_root)
        ...
        user = os.getenv('OPENAI_SSH_USER') or os.getenv('OPENAI_USER')
        if build_root[-1] != '/':
            build_root += '/'
        ...
        if dockerfile:
            dockerfile.close()
        if dockerignore_bak is not None:
            build.restore_dockerignore(dryrun, dockerignore_bak, dockerignore_path)
