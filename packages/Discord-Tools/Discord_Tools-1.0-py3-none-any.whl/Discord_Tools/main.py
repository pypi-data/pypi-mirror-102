...
class DT:    
    def __init__(self,usrnm,pswrd):
        ...
        # op.headless = True
        # op.add_argument("user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.128 Safari/537.36")
    ...
    def GetMessagesInView(self):
        ...
        for i in msg_element_list:
            if(i["class"][0].startswith("message")):
                ...
                print(i.find("h2" , {"class" : "header-23xsNx"}).find("span").find("time").get("aria-label"))
