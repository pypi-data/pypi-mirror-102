...
class AMXTelnet:
	...
	async def _telnet_scan_master(self, master, debug):
		...
		global scanned_systems
		if debug: log(msg=f"scanning: {master['full_name']}")
			...
			with Telnet(master['master_ip'], timeout=10) as tn:
				...
				while True:
					...
					if master['master_login_failure'] is None:
						if debug > 0:
							log(msg=f" login success {master['full_name']}")
						tn = await self._poll_master(tn)
						if debug > 0:
							log(msg=f"polled {master['full_name']}")
					...
					# failure, try amx default
						...
						log(msg=f" trying {self.user_name} {master['full_name']}")
		...
		# instead of .close(), but NI needs 'exit'
			...
			log(msg=f'{__name__}.py: _telnet_scan_master() NX false connection error')

		except Exception as e:
			if master is not None:
				log(msg=f"{__name__}.py: _telnet_scan_master() {master['full_name']} "
					f"{master['master_ip']} {e}")
			else: log(msg=f"{__name__}.py: _telnet_scan_master() no master: {master} {e}")
