import asyncio
from logging import error, warning, info, warn
import asyncio
import re
...
class AMXTelnet:
	...
	async def _telnet_master_login(self, tn, master):
		...
		if 'Welcome to' in login_result.decode():
			...
			info(f"amxtelnet.py successfully logged into {master['full_name']}")
			return master
	...
	async def _telnet_scan_master(self, master, debug):
		...
		info(f"scanning: {master['full_name']}")
		...
		# instead of .close(), but NI needs 'exit'
			...
			error(f'{__name__}.py: _telnet_scan_master() NX false connection error')

		except Exception as e:
			if master is not None:
				error(f"{__name__}.py: _telnet_scan_master() {master['full_name']} {master['master_ip']} {e}")
			else: error(f"{__name__}.py: _telnet_scan_master() no master: {master} {e}")
