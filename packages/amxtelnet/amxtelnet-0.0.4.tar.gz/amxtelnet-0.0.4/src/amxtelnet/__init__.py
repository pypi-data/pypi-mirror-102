import asyncio
from logging import ERROR, INFO, WARNING, log
import asyncio
import re
...
class AMXTelnet:
	...
	def __init__(self, user_name, password, alt_username, alt_password, output_path) -> None:
		if user_name:
		...
		else: self.alt_username = 'administrator'

		if alt_password:
			self.alt_password = alt_password
		else: self.alt_password = 'password'

		if output_path:
			self.output_path = output_path
		else: self.output_path = 'telnet responses/'
	...
	def _write_to_file(self, master, telnet_text):
		...
		telnet_response_path = f"{self.output_path}"
		if not path.exists(telnet_response_path):
			mkdir(telnet_response_path)
		...
		with open(f"{telnet_response_path}{master['full_name']} telnet.txt", 'w+') as f:
	...
	async def _telnet_scan_master(self, master, debug):
		from amxtelnetlib import Telnet

		log(level=INFO, msg=f"scanning: {master['full_name']}")
		...
		try:
			with Telnet(master['master_ip'], timeout=10) as tn:
				...
				tn.read_until(b'\r\nLogin : ')
				master = await self._telnet_master_login(tn, master, self.user_name, self.password)

				# check login results
				while True:
					# success
					if master['master_login_failure'] is None:
						log(level=INFO, msg=f" login success {master['full_name']}")
						tn = await self._poll_master(tn)
						log(level=INFO, msg=f"polled {master['full_name']}")
					...
					# failure, try amx default
						...
						log(level=WARNING, msg=f" trying {self.user_name} {master['full_name']}")
					...
					# default didn't work either, move on to the next system
						...
						log(level=ERROR, msg=
							f"""Unable to login to {master['full_name']} {master['master_ip']}
								after {master['master_failed_attempts']} failed attempts"""
						)
						tn.close()
						break

				self._write_to_file(master, telnet_text)

		# NX masters throw this error when using 'exit'
		# instead of .close(), but NI needs 'exit'
		except ConnectionResetError:
			log(level=ERROR, msg=f'{__name__}.py: _telnet_scan_master() NX false connection error')

		except Exception as e:
			if master is not None:
				log(level=INFO, msg=f"{__name__}.py: _telnet_scan_master() {master['full_name']} "
					f"{master['master_ip']} {e}")
			else: log(level=WARNING, msg=f"{__name__}.py: _telnet_scan_master() no master: {master} {e}")
	...
	async def telnet_master_to_txt(self, systems, simultaneous=65535, debug=0):
		import time
		...
		# make sure we don't start getting negative numbers
		if master_count <= simultaneous:
			# this is redundant when =, but we still need to set skip to True
			simultaneous = master_count
			skip_remainder = True
		...
		for i in range(master_chunks):
			...
			for x in range(scan_start, scan_start + simultaneous):
				tasks.append(self._telnet_scan_master(systems[x], debug))
			log(level=INFO, msg=
				f'\ntelnet scan {scan_start + 1}-{scan_end} of {master_count} AMX systems\n')
		...
		if not skip_remainder:
			...
			for i in range(scan_start, scan_end):
				tasks.append(self._telnet_scan_master(systems[i], debug))
			log(level=INFO, msg=
				f'\ntelnet scan {scan_start + 1}-{scan_end} of {master_count} AMX systems\n')
			await asyncio.gather(*tasks)

		elapsed = time.perf_counter() - start
		log(level=INFO, msg=f"\namxtelnet scan completed in {elapsed:0.2f} seconds\n")
...
class ParseAMXResponse:
		...
		for i, line in enumerate(full_text):
			if master_line in line:
				...
				try:
					...
					log(level=ERROR, msg=f"amxtelnet.py _parse_master_telnet({telnet_master['full_name']}) tp_firmware {e}")
					pass
	...
	async def read_telnet_txt(self) -> list:
