...
class Score:
    ...
    def blue(self) -> tuple:
        ...
        def 매출액증가율평가() -> tuple:
            ...
            logger.info(f'평균 : {mean}')
            #logger.info(f'표준편차 : {std}')
            p += 0 if mean < 0 else round(mean / 10)
...
def report_one(code: str) -> str:
    ...
    def red_str(all_dict: dict, red_p: tuple) -> str:
        red, 괴리율 = red_p
    ...
    def mil_str(all_dict: dict, mil_p: tuple) -> str:
        mil, MAX = mil_p
    ...
    def blue_str(all_dict: dict, blue_p: tuple) -> str:
        neg_p, NEG_MAX, pos_p, mean = blue_p
    ...
    def c108_str(all_dict: dict) -> str:
        s = ''
        for i, c108_dict in enumerate(all_dict['c108']):
