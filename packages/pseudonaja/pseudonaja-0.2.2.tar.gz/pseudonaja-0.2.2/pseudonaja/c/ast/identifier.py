...
class ArrayIdentifierDecl(IdentifierDecl):
    ...
    def interpret(self):
        ...
        if isinstance(self.__start_idx, str):
            # make sure this has been defined

            if self.__start_idx in pcint.PInterpreter.symbols:

                constant =  pcint.PInterpreter.symbols[self.__start_idx]

                if not isinstance(constant, Constant):
                    # raise exception index name is not a constant
                    raise SyntaxError(f"Array index '{self.__start_idx}' is not a constant")

                else:
                    self.__start_idx = constant.value

            else:
                # raise exception: index constant undefined
                raise SyntaxError(f"Array index constant '{self.__start_idx}' undefined")


        if isinstance(self.__end_idx, str):
            # make sure this has been defined

            if self.__end_idx in pcint.PInterpreter.symbols:

                constant =  pcint.PInterpreter.symbols[self.__end_idx]

                if not isinstance(constant, Constant):
                    # raise exception index name is not a constant
                    raise SyntaxError(f"Array index '{self.__end_idx}' is not a constant")

                else:
                    self.__end_idx = constant.value

            else:
                # raise exception: index constant undefined
                raise SyntaxError(f"Array index constant '{self.__end_idx}' undefined")
