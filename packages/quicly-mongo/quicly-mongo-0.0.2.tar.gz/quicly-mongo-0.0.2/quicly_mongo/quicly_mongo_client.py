...
class QuiclyMongoClient(object):
    def __init__(self, host='localhost', port=27017, username=None, password=None, database='admin', current_time=None):
        if isinstance(host, str):
            if '://' in host:
                py_mongo_client = QuiclyMongoClient.create_py_mongo_client_from_url(host)
            else:
                py_mongo_client = QuiclyMongoClient.create_py_mongo_client(host, port, username, password)
        else:
