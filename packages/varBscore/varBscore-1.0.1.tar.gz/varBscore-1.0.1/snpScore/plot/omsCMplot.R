...
# added out.name
# modified manhattan plot y.axis label
...
)
{
	...
	if("d" %in% plot.type){
		...
		if(file.output){
			if(file=="jpg")	jpeg(paste(out.name,".jpg",sep=""), width = 9*dpi,height=7*dpi,res=dpi,quality = 100)
			if(file=="pdf")	pdf(paste(out.name,".pdf",sep=""), width = 9,height=7)
			if(file=="tiff")	tiff(paste(out.name,".tiff",sep=""), width = 9*dpi,height=7*dpi,res=dpi)
			par(xpd=TRUE)
		}else{
		...
		for(i in 1:R){
			...
			if(outward==TRUE){
				...
				if(cir.legend==TRUE){
					...
					text(-r/15,r+H*(i-0.75)+cir.band*(i-1),round(Max*0.25,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
					text(-r/15,r+H*(i-0.5)+cir.band*(i-1),round(Max*0.5,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
					text(-r/15,r+H*(i-0.25)+cir.band*(i-1),round(Max*0.75,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
					text(-r/15,r+H*(i-0)+cir.band*(i-1),round(Max*1,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
				}
				...
				if(cir.chr==TRUE){
					...
					if(is.null(chr.labels)){
						for(i in 1:length(ticks)){
							...
							text(ticks1[i],ticks2[i],chr.ori[i],srt=angle,font=1,cex=cex.axis)
						}
					}else{
						for(i in 1:length(ticks)){
							angle=360*(1-(ticks-round(band/2))[i]/TotalN)
							text(ticks1[i],ticks2[i],chr.labels[i],srt=angle,font=1,cex=cex.axis)
						}
					}
				}else{
			...
			if(outward==FALSE){
				...
				if(cir.legend==TRUE){
					...
					segments(0,r+H*(i-0)+cir.band*(i-1),H/20,r+H*(i-0)+cir.band*(i-1),col=cir.legend.col,lwd=1.5)
					circle.plot(myr=r+H*(i-0)+cir.band*(i-1),lwd=0.5,add=TRUE,col='grey')
					text(-r/15,r+H*(i-0.25)+cir.band*(i-1),round(Max*0.25,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
					text(-r/15,r+H*(i-0.5)+cir.band*(i-1),round(Max*0.5,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
					text(-r/15,r+H*(i-0.75)+cir.band*(i-1),round(Max*0.75,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
					text(-r/15,r+H*(i-1)+cir.band*(i-1),round(Max*1,round.n),adj=1,col=cir.legend.col,cex=cir.legend.cex,font=1)
				}
	...
	if("m" %in% plot.type){
		if(multracks==FALSE){
				...
				print(paste("Rectangular_Manhattan Plotting ",taxa[i],"...",sep=""))
					...
					if(is.null(ylim)){
						if(Max>100){
						  ...
						  axis(2,at=seq(0,100,10),cex.axis=cex.axis,font=1,labels=(seq(0,100,10)))
						  legend.y <- tail(seq(0,(Max+1),ceiling((Max+1)/10)),1)
						}
					}else{
						if(ylim[2]>1){
			...
			for(i in 1:R){
				...
				if(i == R){
					if(is.null(chr.labels)){
						axis(1, at=c(0,ticks),cex.axis=cex.axis*xn,font=1,labels=c("",chr.ori),padj=(xn-1)/2)
					}else{
				...
				if(is.null(ylim)){
					...
					if(ylim[2]>1){
						axis(2,at=seq(min(ylim),(ylim[2]),ceiling((ylim[2])/10)),cex.axis=cex.axis*xn,font=1,labels=seq(min(ylim),(ylim[2]),ceiling((ylim[2])/10)))
					}else{
						axis(2,at=seq(min(ylim),ylim[2],10^(-ceiling(-log10(ylim[2])))),cex.axis=cex.axis*xn,font=1,labels=seq(min(ylim),ylim[2],10^(-ceiling(-log10(ylim[2])))))
					}
				}
				if(!is.null(threshold)){
					if(sum(threshold!=0)==length(threshold)){
