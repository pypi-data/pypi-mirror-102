...
def aux_logger(in_name: str):
    ...
    in_name = in_name[-1]
    name_length = len(in_name)
    if name_length > 33:
        while name_length > 30:
            if '.' not in in_name:
                break
            in_name = in_name.split('.', maxsplit=1)
            in_name = in_name[-1]
...
def _each_run_file_rotate_handler(in_file_name, in_back_up):
    ...
    formatter = CustomFormatter(fmt='{asctime} |{levelname:^13}|{threadName:^14}|{lineno:^7}| {name:<39}|{funcName:^35}||--> {message}', datefmt=_date_format, style='{')
...
def _stdout_handler():
    ...
    formatter = CustomFormatter(fmt='{asctime} |{levelname:^13}| {name:<39}|{funcName:^35}||--> {message}', datefmt=_date_format, style='{')
