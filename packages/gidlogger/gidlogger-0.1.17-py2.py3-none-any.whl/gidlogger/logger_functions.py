...
def _each_run_file_rotate_handler(in_file_name, in_back_up):
    ...
    formatter = CustomFormatter(fmt='{asctime} |{levelname:^11}|{threadName:^14}|{lineno:^5}| {name:<50}|{funcName:^35}||--> {message}', datefmt=_date_format, style='{')
...
def _stdout_handler():
    ...
    formatter = CustomFormatter(fmt='{asctime} |{levelname:^11}| {name:<50}|{funcName:^35}||--> {message}', datefmt=_date_format, style='{')
