...
class Connection:
    ...
    async def _requests_handler(self, uid, flags, chunks_count, curr_chunk, data):
    ...
    async def listen_loop(self):
        ...
        while True:
            try:
                data_raw: bytes = await self._reader.readuntil(models.BIONIC_POSTMAGIC)
            except asyncio.IncompleteReadError:
                logger.warning("Read failed, closing connection!")
                self._writer.close()
