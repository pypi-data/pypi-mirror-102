...
class _Y(object):
        ...
        if this_line.startswith("@") or this_line_prev.startswith("@") or this.decorator:
            ...
            def real_decorator(function):
                ...
                def wrapper(*args, **kwargs):
                    ...
                    if this.show_enter:
                        this.do_output("{context}called {function_arguments}{traceback}".format(context=context, function_arguments=function_arguments, traceback=this.traceback()))
    ...
    def traceback(self):
        if self.show_traceback:
            ...
            return result + '\n'.join(wrap_indent + '  File "' + entry.filename + '", line ' + str(entry.lineno) + ", in " + entry.name + "\n" + wrap_indent+ "    " +entry.line for entry in traceback.extract_stack()[:-2])
        else:
            return ""
