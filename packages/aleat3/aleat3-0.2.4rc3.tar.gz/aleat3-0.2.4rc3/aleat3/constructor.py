...
class Aleatoryous:
    ...
    def __init__(self, mode="random", extras=None):
        ...
        if mode == "random":
            ...
            mode = "aleatory.coin" if m == 1  else "aleatory.dice"
            mode = "aleatory.roulette" if m == 3 else mode
            extras = LIST_EXAMPLE if extras is None and mode == "aleatory.roulette" else extras# add a default list if doesn't exists and is required
            del(m)
