...
from ..view.raw_models import HFIndexPrice
...
class ResearchNAVReader:
    ...
    def _read_for_NAVData(file_path: str) -> pd.DataFrame:
        ...
        for one in tar:
            ...
            p_name = one.split('/')[-1].split('.')[0]
            key_name = file_path.split('融智-')[-1].split('.')[0]
            ...
            df['index_id'] = asset_info[asset_info.real_name == p_name].real_id.array[0]
