    ...
    def get_stat_result(dates:             pd.core.series.Series,
        ...
        if len(dates) > 2:
            ...
            if frequency == '1D':            
                ...
                dts = BasicDataApi().get_trading_day_list(start_date=sr.index[0], end_date=sr.index[-1]).datetime
                sr_annual_vol = sr.reindex(dts).ffill()
                sr_ret_annual_vol = sr_annual_vol.pct_change(1).iloc[1:]
            elif frequency == '1W':
                period_1m = 4
                period_3m = period_1m * 3
                period_6m = period_1m * 6
            ...
            annual_vol = sr_ret_annual_vol.std(ddof=1) * np.sqrt(period_1y)
            _begin_date = sr.index[0]
