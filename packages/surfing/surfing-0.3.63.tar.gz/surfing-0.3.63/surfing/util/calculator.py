...
class Calculator:
    ...
    @staticmethod
    def get_product_rolling_corr(fund_navs, windows=20, frequency='周度', index_id='hs300'):
        try:
            basic = BasicDataApi()
            ...
            result = Calculator.get_rolling_corr(fund_navs, windows, frequency)
    ...
    @staticmethod
    def get_product_rolling_beta(fund_navs, windows=20, frequency='周度', index_id='hs300'):
        try:
            if frequency == '日度':
                rule = '1D'
                windows = int(windows / 1)
