...
class FOFDataManagerLite(metaclass=Singleton):
    ...
    def _calc_virtual_net_value(manager_id, fof_id, fund_list, asset_allocation=None):
        ...
        for row in nav.itertuples():
            ...
            if df.empty:
                continue
            v_nav = FOFDataManagerLite._do_calc_v_net_value(row.net_asset_value, df.unit_total, row.acc_unit_value, df.water_line, incentive_fee_ratio[row.fund_id], v_nav_decimals[row.fund_id])
