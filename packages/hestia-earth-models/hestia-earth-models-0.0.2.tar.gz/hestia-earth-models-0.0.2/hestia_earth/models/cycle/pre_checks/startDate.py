...
from hestia_earth.models.utils import _is_in_days
...
def _should_run(cycle: dict):
    should_run = _is_in_days(cycle.get('endDate')) \
...
def run(cycle: dict):
    cycle['startDate'] = _run(cycle) if _should_run(cycle) else cycle.get('startDate')
    return cycle
