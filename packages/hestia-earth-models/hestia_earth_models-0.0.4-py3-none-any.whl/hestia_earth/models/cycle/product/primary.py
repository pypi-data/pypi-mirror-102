...
def _product(term: dict):
    logger.info('model=%s, value=%s', MODEL, term.get('@id'))
...
def _run(products: list):
    ...
    return None if primary is None else _product(primary.get('term', {}))
...
def run(cycle: dict):
    ...
    return _run(products) if _should_run(products) else None
