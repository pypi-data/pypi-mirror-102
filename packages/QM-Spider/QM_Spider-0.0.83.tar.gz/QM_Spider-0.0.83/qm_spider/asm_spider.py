    ...
    def asm_consume(self):
        ...
        if '异常' not in res_status:
            ...
            if float(today_credits_num) != yes_credits_num:
                yes_run_num = round(today_credits_num - yes_credits_num, 2)  # 昨日消耗的；
                # now_yue_num = budget_amount - spent_value  # 限制消耗金额减去当前总消耗
                now_yue_days = int(math.floor(today_credits_num / yes_run_num))  # 预估还可消耗天数；
            else:
                # 如果今日昨日一样，代表没消耗的，就可以跳过；
                yes_run_num = '空'
                now_yue_days = '∞∞∞'
            ...
            push_text = "### %s-七麦科技-ASM信用额度提醒 \n\n**今日可用的信用额度**：[%s(USD)](%s)\n\n**昨日可用的信用额度**：%s(USD)\n\n**昨日消耗信用额度**：%s(USD)\n\n**预估剩余可用天数**：**%s天**" % (self.today_date, today_credits_num, asm_link, yes_credits_num, yes_run_num, now_yue_days)
            push_text_list.append([push_title, push_text])
            # 完毕后读取当前数据进行去重，
