...
#                         if tag['type'] == 'snippet':
#                             self.add_snippet(tag, datamodel, mappings)
#                             
#     def add_snippet(self, snippet_tag, datamodel, mappings):
#         """
#         Snippets are already fetched into datamodel['snippets].
#         Now, lets annotate them with the uuid/content_hash
# 
#         Example snippet_tag:
#         {
#           'id': 'a4dd8000-a422-75b9-570d-bd0baf32cf08', 
#           'name': 'snippet: mailings-for-trials-benchmark', 
#           'display-name': 'Snippet: mailings-for-trials-benchmark', 
#           'type': 'snippet', 
#           'snippet-name': 'mailings-for-trials-benchmark', 
#           'snippet-id': 1
#         }
#         """
#         snip_id = snippet_tag['snippet-id']
#         snippet = datamodel['snippets'][snip_id]
#         if snip_id not in mappings['snippets']:
#             mappings[{"model": 'snippets', "id": snip_id}] = self.versioned_mapping(snippet)
        ...
        for model in models:
            for k, v in source_map[model].items():
                if v[resolve_key] in property_to_id[model]:
                    ...
                    unmapped[model][k] = v
        print("resolve_with.property_to_id: {}".format(property_to_id['snippets']))
        print("resolve_with.unmapped: {}".format(unmapped['snippets']))
        return unmapped
