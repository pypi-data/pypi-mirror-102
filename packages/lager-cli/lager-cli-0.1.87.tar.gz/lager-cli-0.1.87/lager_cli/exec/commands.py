...
def _run_command_host(section, path, cmd_to_run, mount, extra_args, debug, interactive, tty):
    """
...
def _run_command(section, path, cmd_to_run, mount, extra_args, debug, interactive, tty):
    ...
    if ci_env == CIEnvironment.HOST:
        return _run_command_host(section, path, cmd_to_run, mount, extra_args, debug, interactive, tty)
...
def exec_(ctx, cmd_name, extra_args, command, save_as, warn, mount, interactive, tty):
    """
    ...
    returncode = _run_command(section, path, cmd_to_run, mount, extra_args, ctx.obj.debug, interactive, tty)
    ctx.exit(returncode)
